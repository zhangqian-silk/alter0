...
ENABLE_THRESHOLD_RECONCILE="${ENABLE_THRESHOLD_RECONCILE:-true}"
...
if [ "$ENABLE_THRESHOLD_RECONCILE" = "true" ]; then
  THRESHOLD_HISTORY_PATH="$THRESHOLD_HISTORY_PATH" \
    THRESHOLD_RECONCILE_OUTPUT_PATH="output/cost/threshold-reconcile-latest.json" \
    "$ROOT_DIR/scripts/check-cost-threshold-reconcile.sh"
fi

...
test.describe("Terminal route", () => {
  ...
  test("keeps the mobile terminal workspace in the dynamic viewport grid while the composer stays in the footer", async ({ page, request }) => {
    ...
    const readShellMetrics = async () => page.evaluate(() => {
      ...
      const workspace = document.querySelector("[data-runtime-workspace='terminal']");
      const workspaceBody = document.querySelector("[data-runtime-view='terminal'] .runtime-workspace-body");
      const mobileHeader = document.querySelector("[data-runtime-mobile-variant='terminal']");
      ...
      return {
        ...
        shellTop: appShell.getBoundingClientRect().top,
        shellScreenTop: appShell.getBoundingClientRect().top - viewport.offsetTop,
        shellHeight: appShell.getBoundingClientRect().height,
        workspaceTop: workspace.getBoundingClientRect().top,
        workspaceBodyTop: workspaceBody.getBoundingClientRect().top,
        workspaceBodyScreenTop: workspaceBody.getBoundingClientRect().top - viewport.offsetTop,
        workspaceHeight: workspace.getBoundingClientRect().height,
        workspaceBodyHeight: workspaceBody.getBoundingClientRect().height,
        mobileHeaderPosition: window.getComputedStyle(mobileHeader).position,
        mobileHeaderTop: mobileHeaderRect.top,
        mobileHeaderBottom: mobileHeaderRect.bottom,
        composerBottom: composer.getBoundingClientRect().bottom,
        composerScreenBottom: composer.getBoundingClientRect().bottom - viewport.offsetTop,
    ...
    expect(Math.abs((metrics?.shellTop ?? 0) - (baseline?.shellTop ?? 0))).toBeLessThanOrEqual(2);
    ...
    expect(metrics?.workspaceHeight ?? 0).toBeLessThanOrEqual(baseline?.workspaceHeight ?? Number.POSITIVE_INFINITY);
    ...
    expect(Math.abs((nextMetrics?.shellTop ?? 0) - (baseline?.shellTop ?? 0))).toBeLessThanOrEqual(2);
    ...
    expect(nextMetrics?.workspaceHeight ?? 0).toBeLessThanOrEqual(baseline?.workspaceHeight ?? Number.POSITIVE_INFINITY);

...
func TestTravelWorkspaceChatCreatesGuideFromMasterAgentReply(t *testing.T) {
	...
	if got := orchestrator.lastMessage.Metadata[execdomain.AgentToolsMetadataKey]; got != `["codex_exec","search_memory","read_memory","write_memory"]` {
		t.Fatalf("expected travel-master tool set, got %+v", orchestrator.lastMessage.Metadata)
	}
	if got := orchestrator.lastMessage.Metadata["alter0.skills.include"]; got != `["memory"]` {

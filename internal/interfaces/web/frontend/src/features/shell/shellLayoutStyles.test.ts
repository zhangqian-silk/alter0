...
describe("shell layout stylesheet", () => {
  ...
  it("keeps the mobile runtime composer in normal grid flow during viewport refreshes", () => {
    ...
    const mobileComposerBlocks = Array.from(stylesheet.matchAll(
      /@media \(max-width: (?:1280|760)px\) \{[\s\S]*?\.runtime-workspace-body(?:\[data-runtime-view="(?:conversation|chatRuntime)"\])? > \.runtime-composer-shell[^{]*\{([^}]*)\}/g,
    ));
  ...
  it("keeps the mobile runtime header fixed while preserving the first workspace grid row footprint", () => {
    ...
    expect(stylesheet).toMatch(
      /@media \(max-width: 760px\) \{[\s\S]*?\.runtime-workspace-body\[data-runtime-view="chatRuntime"\]\s*\{[\s\S]*?--runtime-mobile-header-fixed-height:\s*48px;[\s\S]*?grid-template-rows:\s*var\(--runtime-mobile-header-fixed-height\) minmax\(0, 1fr\) auto;/,
    );
    expect(stylesheet).toMatch(
  ...
  it("keeps the conversation message area as a continuous borderless workbench surface", () => {
    const currentDirectory = dirname(fileURLToPath(import.meta.url));
    const stylesheet = readFileSync(resolve(currentDirectory, "../../styles/shell.css"), "utf8");

    expect(stylesheet).toMatch(
      /\[data-runtime-view="conversation"\] \.runtime-workspace-head\s*\{[\s\S]*?border-bottom-color:\s*transparent;[\s\S]*?background:\s*#f6f7f9;/,
    );
    expect(stylesheet).toMatch(
      /\[data-runtime-view="conversation"\] \.runtime-workspace-screen\s*\{[\s\S]*?border:\s*0;[\s\S]*?border-radius:\s*0;[\s\S]*?background:\s*#f6f7f9;/,
    );
    expect(stylesheet).not.toMatch(
      /\[data-runtime-view="conversation"\] \.runtime-workspace-screen\s*\{[^}]*background:\s*#fff;/,
    );
    expect(stylesheet).toMatch(
  ...
  it("keeps narrow roots on dynamic viewport units without fixed document locks", () => {
    ...
    expect(stylesheet).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?html,\s*body,\s*#frontend-root\s*\{[\s\S]*?min-height:\s*100dvh;[\s\S]*?overflow-x:\s*hidden;[\s\S]*?overflow-x:\s*clip;[\s\S]*?overscroll-behavior-x:\s*none;/,
    );
    expect(stylesheet).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?#frontend-root\s*\{[\s\S]*?min-height:\s*100dvh;/,
    );
  ...
  it("uses the same compact mobile header controls throughout the shared mobile shell", () => {
    const currentDirectory = dirname(fileURLToPath(import.meta.url));
    const stylesheet = readFileSync(resolve(currentDirectory, "../../styles/shell.css"), "utf8");
    const stateMachine = stylesheet.slice(stylesheet.lastIndexOf("/* Workbench responsive state machine */"));
    ...
    expect(stateMachine).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\.runtime-workspace-mobile-header \.runtime-workspace-mobile-action,\s*\.runtime-workspace-mobile-header \.runtime-workspace-mobile-action\.is-quiet,\s*\.runtime-workspace-mobile-header \.runtime-workspace-mobile-action\.is-primary\s*\{[\s\S]*?border:\s*0;[\s\S]*?background:\s*transparent;[\s\S]*?box-shadow:\s*none;/,
    );
    expect(stateMachine).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\.runtime-workspace-mobile-header \.runtime-workspace-mobile-action svg\s*\{[\s\S]*?width:\s*18px;[\s\S]*?height:\s*18px;/,
    );
  ...
  it("keeps the Chat composer inset and compact at the final cascade layer", () => {
    const currentDirectory = dirname(fileURLToPath(import.meta.url));
    const stylesheet = readFileSync(resolve(currentDirectory, "../../styles/shell.css"), "utf8");
    const guard = stylesheet.slice(
      stylesheet.lastIndexOf("/* Final Chat composer floor and three-step inset contract */"),
    );

    expect(guard).toContain("/* Final Chat composer floor and three-step inset contract */");
    expect(stylesheet.lastIndexOf("/* Final Chat composer floor and three-step inset contract */")).toBeGreaterThan(
      stylesheet.lastIndexOf("/* Final Chat mobile scroll and composer stability guard */"),
    );
    expect(guard).toMatch(
      /@media \(max-width: 760px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-composer-shell,\s*\[data-runtime-composer-view="conversation"\] \.runtime-composer-shell\s*\{[\s\S]*?padding:\s*6px max\(16px, env\(safe-area-inset-right\)\) calc\(8px \+ env\(safe-area-inset-bottom\)\) max\(16px, env\(safe-area-inset-left\)\);[\s\S]*?box-sizing:\s*border-box;[\s\S]*?background:\s*#f6f7f9;/,
    );
    expect(guard).toMatch(
      /@media \(max-width: 760px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-composer-form,\s*\[data-runtime-composer-view="conversation"\] \.runtime-composer-form\s*\{[\s\S]*?min-height:\s*98px;[\s\S]*?padding:\s*10px 14px 8px;[\s\S]*?gap:\s*4px;[\s\S]*?width:\s*var\(--runtime-conversation-rail\);[\s\S]*?max-width:\s*var\(--runtime-conversation-rail\);[\s\S]*?border-radius:\s*18px !important;/,
    );
    expect(guard).toMatch(
  ...
  it("uses phone-sized transparent header actions throughout the 1280px transition shell", () => {
    const currentDirectory = dirname(fileURLToPath(import.meta.url));
    const stylesheet = readFileSync(resolve(currentDirectory, "../../styles/shell.css"), "utf8");
    const stateMachine = stylesheet.slice(stylesheet.lastIndexOf("/* Workbench responsive state machine */"));

    expect(stateMachine).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\.runtime-workspace-mobile-header \.runtime-workspace-mobile-action\s*\{[\s\S]*?width:\s*36px;[\s\S]*?min-width:\s*36px;[\s\S]*?height:\s*36px;[\s\S]*?min-height:\s*36px;[\s\S]*?padding:\s*0;[\s\S]*?border:\s*0;[\s\S]*?background:\s*transparent;[\s\S]*?box-shadow:\s*none;/,
    );
  });

  it("derives the conversation empty state, timeline, and composer from one final rail contract", () => {
    const currentDirectory = dirname(fileURLToPath(import.meta.url));
    const stylesheet = readFileSync(resolve(currentDirectory, "../../styles/shell.css"), "utf8");
    const finalRailIndex = stylesheet.lastIndexOf("/* Final conversation rail contract */");
    const legacyConversationIndex = stylesheet.lastIndexOf('@media (max-width: 1100px)');
    const conversationContract = stylesheet.slice(finalRailIndex);
    const stateMachine = stylesheet.slice(stylesheet.lastIndexOf("/* Workbench responsive state machine */"));

    expect(finalRailIndex).toBeGreaterThan(legacyConversationIndex);
    expect(stylesheet.slice(0, finalRailIndex)).not.toMatch(
      /\[data-runtime-view="conversation"\] \.conversation-empty-state\s*\{[^}]*?(?:width:\s*100%;|max-width:\s*none;|justify-items:\s*stretch;|text-align:\s*left;)/,
    );
    expect(stylesheet.slice(0, finalRailIndex)).not.toMatch(
      /@media \(max-width: 760px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-workspace-(?:panel|screen)\s*\{[\s\S]*?padding:/,
    );
    expect(conversationContract).toMatch(
      /\[data-runtime-view="conversation"\],\s*\[data-runtime-composer-view="conversation"\]\s*\{[\s\S]*?--runtime-conversation-gutter:\s*clamp\(24px,\s*4vw,\s*42px\);[\s\S]*?--runtime-conversation-rail:\s*min\(100%,\s*960px\);/,
    );
    expect(conversationContract).toMatch(
      /\[data-runtime-view="conversation"\] \.runtime-workspace-screen\s*\{[\s\S]*?padding:[^;]*var\(--runtime-conversation-gutter\)[^;]*;/,
    );
    expect(conversationContract).toMatch(
      /\[data-runtime-view="conversation"\] \.runtime-workspace-panel\s*\{[\s\S]*?padding:\s*0 0 14px;/,
    );
    expect(conversationContract).not.toMatch(
      /\[data-runtime-view="conversation"\] \.runtime-workspace-panel\s*\{[^}]*?var\(--runtime-conversation-gutter\)/,
    );
    expect(conversationContract).toMatch(
      /\[data-runtime-view="conversation"\] \.runtime-timeline\s*\{[\s\S]*?width:\s*var\(--runtime-conversation-rail\);[\s\S]*?margin-inline:\s*auto;/,
    );
    expect(conversationContract).toMatch(
      /\[data-runtime-view="conversation"\] \.conversation-empty-state\s*\{[^}]*?width:\s*min\(100%,\s*620px\);[^}]*?justify-self:\s*center;[^}]*?margin-inline:\s*auto;/,
    );
    expect(conversationContract).toMatch(
      /\[data-runtime-view="conversation"\]\[data-runtime-empty-state="true"\],\s*\[data-runtime-composer-view="conversation"\]\[data-runtime-empty-state="true"\]\s*\{[\s\S]*?--runtime-conversation-rail:\s*min\(100%,\s*620px\);/,
    );
    expect(conversationContract).toMatch(
      /\[data-runtime-view="conversation"\] \.runtime-composer-shell,\s*\[data-runtime-composer-view="conversation"\] \.runtime-composer-shell\s*\{[\s\S]*?padding:[^;]*var\(--runtime-conversation-gutter\)[^;]*;/,
    );
    expect(conversationContract).toMatch(
      /\[data-runtime-view="conversation"\] \.runtime-composer-form,\s*\[data-runtime-composer-view="conversation"\] \.runtime-composer-form\s*\{[\s\S]*?width:\s*var\(--runtime-conversation-rail\);[\s\S]*?max-width:\s*var\(--runtime-conversation-rail\);[\s\S]*?justify-self:\s*center;/,
    );
    expect(stateMachine).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\[data-runtime-view="conversation"\],\s*\[data-runtime-composer-view="conversation"\]\s*\{[\s\S]*?--runtime-conversation-gutter:\s*clamp\(16px,\s*3vw,\s*28px\);[\s\S]*?--runtime-conversation-rail:\s*min\(100%,\s*920px\);/,
    );
    expect(stateMachine).not.toMatch(
      /\[data-runtime-view="conversation"\] \.runtime-timeline,\s*\[data-runtime-view="chatRuntime"\] \.runtime-timeline\s*\{[\s\S]*?var\(--runtime-content-column\)/,
    );
    expect(stateMachine).not.toMatch(
      /\n\s+\.runtime-composer-form\s*\{[\s\S]*?var\(--runtime-composer-column\)/,
    );
    expect(stateMachine).toMatch(
      /\[data-runtime-view="chatRuntime"\] \.runtime-timeline\s*\{[\s\S]*?width:\s*var\(--runtime-content-column\);[\s\S]*?max-width:\s*var\(--runtime-content-column\);[\s\S]*?margin-inline:\s*auto;/,
    );
    expect(stateMachine).toMatch(
      /\[data-runtime-composer-view="chatRuntime"\] \.runtime-composer-form\s*\{[\s\S]*?width:\s*var\(--runtime-composer-column\);[\s\S]*?max-width:\s*var\(--runtime-composer-column\);[\s\S]*?justify-self:\s*center;/,
    );
    expect(stateMachine).not.toMatch(/@media \(max-width: (?:1100|1024|900|820)px\)[\s\S]*?--runtime-conversation-rail/);
    expect(conversationContract).toMatch(
      /@media \(max-width: 760px\) \{[\s\S]*?\[data-runtime-view="conversation"\],\s*\[data-runtime-composer-view="conversation"\]\s*\{[\s\S]*?--runtime-conversation-gutter:\s*16px;[\s\S]*?--runtime-conversation-rail:\s*100%;/,
    );
    const conversationTimelineBlocks = Array.from(conversationContract.matchAll(
      /\[data-runtime-view="conversation"\] \.runtime-timeline\s*\{([^}]*)\}/g,
    )).map((match) => match[1]).join("\n");
    const conversationComposerBlocks = Array.from(conversationContract.matchAll(
      /\[data-runtime-view="conversation"\] \.runtime-composer-form,\s*\[data-runtime-composer-view="conversation"\] \.runtime-composer-form\s*\{([^}]*)\}/g,
    )).map((match) => match[1]).join("\n");
    expect(conversationTimelineBlocks).not.toContain("width: 100%;");
    expect(conversationComposerBlocks).not.toContain("width: 100%;");
  });

  it("keeps Chat composer bottoms to three spacing profiles with a colored desktop and transition floor", () => {
    const currentDirectory = dirname(fileURLToPath(import.meta.url));
    const stylesheet = readFileSync(resolve(currentDirectory, "../../styles/shell.css"), "utf8");
    const stateMachine = stylesheet.slice(stylesheet.lastIndexOf("/* Workbench responsive state machine */"));
    const floorGuard = stylesheet.slice(
      stylesheet.lastIndexOf("/* Final Chat composer floor and three-step inset contract */"),
    );

    expect(floorGuard).toContain("/* Final Chat composer floor and three-step inset contract */");
    expect(floorGuard).toMatch(
      /@media \(min-width: 1281px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-workspace-body,\s*\.runtime-workspace-body\[data-runtime-view="conversation"\],\s*\[data-runtime-view="conversation"\] \.runtime-workspace-panel,\s*\.runtime-workspace-body\[data-runtime-view="conversation"\] > \.runtime-workspace-panel\s*\{[\s\S]*?background:\s*#f6f7f9;/,
    );
    expect(floorGuard).toMatch(
      /@media \(min-width: 1281px\) \{[\s\S]*?\.chat-pane\[data-route="chat"\]\.page-mode,\s*\.runtime-workspace-shell\[data-runtime-view="conversation"\]\s*\{[\s\S]*?background:\s*#f6f7f9;/,
    );
    expect(floorGuard).toMatch(
      /@media \(min-width: 1281px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-workspace-head\s*\{[\s\S]*?background:\s*#f6f7f9;/,
    );
    expect(floorGuard).toMatch(
      /@media \(min-width: 1281px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-workspace-screen,\s*\.runtime-workspace-body\[data-runtime-view="conversation"\] \.runtime-workspace-screen\s*\{[\s\S]*?background:\s*#f6f7f9;/,
    );
    expect(floorGuard).not.toMatch(/background:\s*linear-gradient\(180deg, #ffffff 0%, #ffffff 72%, #f6f7f9 100%\);/);
    expect(floorGuard).toMatch(
      /@media \(min-width: 1281px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-composer-shell,\s*\[data-runtime-composer-view="conversation"\] \.runtime-composer-shell\s*\{[\s\S]*?padding:\s*12px var\(--runtime-conversation-gutter\) calc\(18px \+ env\(safe-area-inset-bottom\)\);[\s\S]*?background:\s*linear-gradient\(180deg, rgba\(246, 247, 249, 0\.78\) 0%, #f6f7f9 100%\);/,
    );
    expect(stateMachine).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\.runtime-workspace\s*\{[\s\S]*?padding:\s*0;/,
    );
    expect(stateMachine).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-composer-shell,\s*\[data-runtime-composer-view="conversation"\] \.runtime-composer-shell\s*\{[\s\S]*?padding:\s*10px var\(--runtime-conversation-gutter\) calc\(14px \+ env\(safe-area-inset-bottom\)\);[\s\S]*?background:\s*#f6f7f9;/,
    );
    expect(stateMachine).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-workspace-screen,\s*\.runtime-workspace-body\[data-runtime-view="conversation"\] \.runtime-workspace-screen\s*\{[\s\S]*?background:\s*#f6f7f9;/,
    );
    expect(stateMachine).not.toMatch(/background:\s*linear-gradient\(180deg, #ffffff 0%, #ffffff 70%, #f6f7f9 100%\);/);
    expect(floorGuard).toMatch(
      /@media \(max-width: 760px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-workspace-body,\s*\.runtime-workspace-body\[data-runtime-view="conversation"\],\s*\[data-runtime-view="conversation"\] \.runtime-workspace-panel,\s*\.runtime-workspace-body\[data-runtime-view="conversation"\] > \.runtime-workspace-panel,\s*\[data-runtime-view="conversation"\] \.runtime-workspace-screen,\s*\.runtime-workspace-body\[data-runtime-view="conversation"\] \.runtime-workspace-screen\s*\{[\s\S]*?background:\s*#f6f7f9;/,
    );
    expect(floorGuard).toMatch(
      /@media \(max-width: 760px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-composer-shell,\s*\[data-runtime-composer-view="conversation"\] \.runtime-composer-shell\s*\{[\s\S]*?padding:\s*6px max\(16px, env\(safe-area-inset-right\)\) calc\(8px \+ env\(safe-area-inset-bottom\)\) max\(16px, env\(safe-area-inset-left\)\);[\s\S]*?background:\s*#f6f7f9;/,
    );
    expect(floorGuard).not.toMatch(/@media \(max-width: (?:1100|1024|900|820)px\)[\s\S]*?runtime-composer-shell/);
  });
  ...
  it("keeps mobile conversation empty states on the shared conversation rail", () => {
    const currentDirectory = dirname(fileURLToPath(import.meta.url));
    const stylesheet = readFileSync(resolve(currentDirectory, "../../styles/shell.css"), "utf8");
    const finalRail = stylesheet.slice(stylesheet.lastIndexOf("/* Final conversation rail contract */"));
    const floorGuard = stylesheet.slice(
      stylesheet.lastIndexOf("/* Final Chat composer floor and three-step inset contract */"),
    );

    expect(finalRail).toMatch(
      /\[data-runtime-view="conversation"\] \.conversation-empty-state\s*\{[\s\S]*?width:\s*min\(100%,\s*620px\);[\s\S]*?justify-self:\s*center;[\s\S]*?margin-inline:\s*auto;/,
    );
    expect(floorGuard).toMatch(
      /@media \(max-width: 760px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-composer-shell,\s*\[data-runtime-composer-view="conversation"\] \.runtime-composer-shell\s*\{[\s\S]*?padding:\s*6px max\(16px, env\(safe-area-inset-right\)\) calc\(8px \+ env\(safe-area-inset-bottom\)\) max\(16px, env\(safe-area-inset-left\)\);[\s\S]*?background:\s*#f6f7f9;/,
    );
  ...
  it("keeps runtime mobile shells and long chatRuntime content from creating page-level horizontal overflow", () => {
    ...
    expect(horizontalGuard).toMatch(
      /@media \(max-width: 760px\) \{[\s\S]*?\[data-runtime-view="conversation"\],\s*\.runtime-workspace-body\[data-runtime-view="conversation"\],\s*\[data-runtime-view="conversation"\] \.runtime-workspace,\s*\[data-runtime-view="conversation"\] \.runtime-workspace-body,\s*\[data-runtime-view="conversation"\] \.runtime-workspace-panel,\s*\[data-runtime-view="conversation"\] \.runtime-workspace-screen\s*\{[\s\S]*?min-width:\s*0;[\s\S]*?max-width:\s*100%;[\s\S]*?overflow-x:\s*hidden;/,
    );
    expect(horizontalGuard).toMatch(
      /@media \(max-width: 760px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-timeline\s*\{[\s\S]*?min-width:\s*0;[\s\S]*?max-width:\s*var\(--runtime-conversation-rail\);[\s\S]*?overflow-x:\s*hidden;/,
    );
    expect(horizontalGuard).toMatch(
      /@media \(max-width: 760px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-composer-shell,\s*\[data-runtime-composer-view="conversation"\] \.runtime-composer-shell\s*\{[\s\S]*?min-width:\s*0;[\s\S]*?max-width:\s*100%;[\s\S]*?overflow-x:\s*hidden;/,
    );
    expect(horizontalGuard).toMatch(
      /@media \(max-width: 760px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-composer-form,\s*\[data-runtime-composer-view="conversation"\] \.runtime-composer-form\s*\{[\s\S]*?min-width:\s*0;[\s\S]*?max-width:\s*var\(--runtime-conversation-rail\);[\s\S]*?overflow-x:\s*hidden;/,
    );
    expect(stylesheet).toMatch(
  ...
  it("uses one shared transition shell below the wide desktop breakpoint so navigation remains reachable", () => {
    ...
    expect(stylesheet).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\.runtime-workspace-body\[data-runtime-view="settings"\],\s*\.runtime-workspace-body\[data-runtime-view="conversation"\],\s*\.runtime-workspace-body\[data-runtime-view="chatRuntime"\]\s*\{[\s\S]*?grid-template-rows:\s*48px minmax\(0, 1fr\) auto;[\s\S]*?overflow:\s*hidden;/,
    );
    expect(stylesheet).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\.runtime-workspace-shell\[data-runtime-view="settings"\] \.runtime-workspace-body,\s*\.runtime-workspace-shell\[data-runtime-view="conversation"\] \.runtime-workspace-body,\s*\.runtime-workspace-shell\[data-runtime-view="chatRuntime"\] \.runtime-workspace-body[\s\S]*?\{[\s\S]*?grid-template-rows:\s*48px minmax\(0, 1fr\) auto;[\s\S]*?overflow:\s*hidden;/,
    );
    expect(stylesheet).toMatch(
    ...
    expect(stylesheet).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\.runtime-workspace-mobile-header\s*\{[\s\S]*?grid-template-columns:\s*36px minmax\(0,\s*1fr\) 36px;[\s\S]*?min-height:\s*48px;[\s\S]*?padding:\s*6px 10px;/,
    );
    expect(stylesheet).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\.runtime-workspace-mobile-header \.runtime-workspace-mobile-title\s*\{[\s\S]*?display:\s*inline-flex;[\s\S]*?min-width:\s*0;[\s\S]*?width:\s*100%;/,
    );
    expect(stylesheet).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\.runtime-workspace-mobile-header \.runtime-workspace-mobile-actions\s*\{[\s\S]*?display:\s*flex;[\s\S]*?justify-content:\s*flex-end;/,
    );
    expect(stylesheet).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\.runtime-workspace-mobile-header \.runtime-workspace-mobile-action\s*\{[\s\S]*?width:\s*36px;[\s\S]*?min-width:\s*36px;[\s\S]*?height:\s*36px;[\s\S]*?min-height:\s*36px;[\s\S]*?padding:\s*0;/,
    );
    expect(stylesheet).toMatch(
  ...
  it("keeps keyboard isolation and overlay portal behavior aligned with the shared 1280px mobile shell", () => {
    const currentDirectory = dirname(fileURLToPath(import.meta.url));
    const isolationStylesheet = readFileSync(resolve(currentDirectory, "../../styles/runtimeKeyboardIsolation.css"), "utf8");
    const rootStylesheet = readFileSync(resolve(currentDirectory, "../../styles/root.css"), "utf8");
    const shellStylesheet = readFileSync(resolve(currentDirectory, "../../styles/shell.css"), "utf8");
    const responsiveStateMachineLayer = shellStylesheet.slice(shellStylesheet.lastIndexOf("/* Workbench responsive state machine */"));

    expect(isolationStylesheet).toContain("@media (max-width: 1280px)");
    expect(isolationStylesheet).not.toContain("@media (max-width: 1100px)");
    expect(rootStylesheet).toContain("@media (max-width: 1280px)");
    expect(rootStylesheet).not.toContain("@media (max-width: 1100px)");
    expect(responsiveStateMachineLayer).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\.workbench-mobile-overlay-portal\s*\{[\s\S]*?position:\s*fixed;[\s\S]*?inset:\s*0;[\s\S]*?z-index:\s*120;[\s\S]*?pointer-events:\s*none;/,
    );
    expect(responsiveStateMachineLayer).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\.workbench-mobile-overlay-portal \.primary-nav\s*\{[\s\S]*?position:\s*fixed;[\s\S]*?top:\s*0;[\s\S]*?bottom:\s*0;[\s\S]*?visibility:\s*visible;[\s\S]*?pointer-events:\s*auto;[\s\S]*?transform:\s*translate3d\(0,\s*0,\s*0\);/,
    );
  });

  it("keeps runtime rails explicit through the 1280px transition state", () => {
    const currentDirectory = dirname(fileURLToPath(import.meta.url));
    const stylesheet = readFileSync(resolve(currentDirectory, "../../styles/shell.css"), "utf8");
    const responsiveStateMachineLayer = stylesheet.slice(stylesheet.lastIndexOf("/* Workbench responsive state machine */"));

    expect(responsiveStateMachineLayer).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\.app-shell,\s*\.app-shell\.info-mode\s*\{[\s\S]*?--runtime-content-gutter:\s*clamp\(16px,\s*3vw,\s*28px\);[\s\S]*?--runtime-content-column:\s*min\(100%,\s*920px\);[\s\S]*?--runtime-composer-column:\s*min\(100%,\s*920px\);/,
    );
    expect(responsiveStateMachineLayer).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\[data-runtime-view="chatRuntime"\] \.runtime-timeline\s*\{[\s\S]*?width:\s*var\(--runtime-content-column\);[\s\S]*?margin-inline:\s*auto;/,
    );
    expect(responsiveStateMachineLayer).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\[data-runtime-composer-view="chatRuntime"\] \.runtime-composer-form\s*\{[\s\S]*?width:\s*var\(--runtime-composer-column\);[\s\S]*?justify-self:\s*center;/,
    );
    expect(responsiveStateMachineLayer).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-timeline\s*\{[\s\S]*?width:\s*var\(--runtime-conversation-rail\);[\s\S]*?max-width:\s*var\(--runtime-conversation-rail\);/,
    );
    expect(responsiveStateMachineLayer).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\[data-runtime-view="conversation"\] \.runtime-composer-form,\s*\[data-runtime-composer-view="conversation"\] \.runtime-composer-form\s*\{[\s\S]*?width:\s*var\(--runtime-conversation-rail\);[\s\S]*?max-width:\s*var\(--runtime-conversation-rail\);/,
    );
    expect(responsiveStateMachineLayer).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\.chat-pane\[data-route="chat"\]\.page-mode \.nav-toggle,\s*\.chat-pane\[data-route="chat"\]\.page-mode \.panel-toggle,\s*\.chat-pane\[data-route="chat"\]\.page-mode \.mobile-new-chat,\s*\.chat-pane\[data-route="chatRuntime"\]\.page-mode \.nav-toggle,\s*\.chat-pane\[data-route="chatRuntime"\]\.page-mode \.panel-toggle,\s*\.chat-pane\[data-route="chatRuntime"\]\.page-mode \.mobile-new-chat\s*\{[\s\S]*?display:\s*none;/,
    );
  });
  ...
  it("keeps desktop Runtime and General settings from inheriting the compact mobile card layout", () => {
    ...
    expect(shellLayer).toMatch(
      /@media \(max-width: 1280px\) \{[\s\S]*?\.runtime-workspace-mobile-header \.runtime-workspace-mobile-action\s*\{[\s\S]*?width:\s*36px;[\s\S]*?min-width:\s*36px;[\s\S]*?height:\s*36px;[\s\S]*?justify-self:\s*center;/,
    );
  });

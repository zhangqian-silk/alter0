...
- `features/conversation-runtime` 承载 `chat` 运行态：`ConversationRuntimeProvider` 负责会话、消息流、SSE 收口、任务轮询、草稿恢复、短 hash URL 恢复和模型/能力项选择；历史 `chat` 本地 snapshot、服务端 registry 与 Session history 在加载时迁移到当前 Chat 会话结构。`ConversationWorkspace` 接入共享 `RuntimeWorkspacePage` 组装「会话列 + 主时间线工作区 + Composer + Inspector」运行页，并在窄屏下统一输出 `Menu / 标题 / New` 操作行；不再渲染 Skill 选择器、Deliverables、Session Profile 或独立 Skill 面板

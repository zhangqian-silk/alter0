...
- `features/conversation-runtime` 承载 `chat / agent-runtime` 的运行态：`ConversationRuntimeProvider` 负责会话、消息流、SSE 收口、任务轮询、草稿恢复和模型/能力项选择，`Chat / Agent Runtime` 的模型选择都会补充内置 `Codex` provider，Agent Runtime 的 Agent 选择会排除 `Alter0/main` 主助手，并把当前 Agent 私有 Skill 合成为锁定启用项、只把公有 Skill 暴露为可选项；`ConversationWorkspace` 通过共享 `RuntimeWorkspaceFrame` 组装 terminal-style 的「会话列 + 主时间线工作区 + Composer + Inspector」视图，并在窄屏下提供 `Menu / Sessions / New` 操作行与会话抽屉；会话抽屉头部左侧固定显示两行 `Sessions + 总数` 文案，右侧 `New / Hide` 复用紧凑按钮规格；会话侧栏按最近时间分组为 `Today / Yesterday / Earlier`，条目采用卡片式标题、时间元信息、底部短 hash 与尾侧更多按钮的共享结构，其中 `agent-runtime` 会额外在标题下方显示具体 Agent 名称的小字标签

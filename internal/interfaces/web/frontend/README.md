...
- `features/conversation-runtime` 承载 Chat 对话运行态 UI：`ConversationRuntimeProvider` 复用 runtime session 数据模型和状态机，并通过 `/api/chat/sessions` 负责会话、输入提交、会话恢复轮询、草稿恢复、短 hash URL 恢复和模型/能力项选择。`ConversationRuntimeProvider` 额外维护同一 SPA 工作台内的 24 小时运行态内存缓存，保存 active session、文本草稿、附件草稿、过程披露过滤、完整消息快照和轻量会话信息快照。`ConversationWorkspace` 接入共享 `RuntimeWorkspacePage` 组装「会话列 + 主时间线工作区 + Composer + Inspector」Chat 工作区，并在窄屏下统一输出 `Menu / 标题 / New` 操作行；不再渲染 Skill 选择器、Deliverables、Session Profile 或独立 Skill 面板
- `features/shell` 负责主导航、共享 copy、Chat 工作区骨架与 React 管理页：`components/RuntimeWorkspacePage.tsx` 提供Chat 工作区骨架、导航持有的会话列表、工作区 body 与 slot 化头部/正文/底部区域，并把当前Chat 工作区的 count、列表 body 与 `New` 动作绑定给主导航；`PrimaryNav.tsx` 稳定持有 `Sessions / New` 公共 chrome，`WorkbenchApp.tsx` 在Chat 工作区 rail 数据尚未注册时提供禁用态 `New` fallback rail，并缓存最近一次有效 rail body；`RuntimeWorkspaceHeader.tsx` 统一输出Chat 工作区的标题、状态按钮与 `Details` 按钮；`ReactManagedRouteBody` 只分派 Settings 管理页

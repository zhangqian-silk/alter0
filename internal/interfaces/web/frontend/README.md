...
- `features/conversation-runtime` 承载 `chat / agent-runtime` 的运行态：`ConversationRuntimeProvider` 负责会话、消息流、SSE 收口、任务轮询、草稿恢复和模型/能力项选择，`ConversationWorkspace` 通过共享 `RuntimeWorkspaceFrame` 组装 terminal-style 的「会话列 + 主时间线工作区 + Composer + Inspector」视图，并在窄屏下提供 `Menu / Sessions / New` 操作行与会话抽屉；会话侧栏按最近时间分组为 `Today / Yesterday / Earlier`，条目采用正文列加尾侧轻量删除动作的线性列表关系

...
- `src/app` 承载单一 React 工作台壳层：`WorkbenchApp` 负责 `/chat`、`/terminal` 与其他控制页 canonical path 路由、语言切换、主导航折叠/抽屉和运行页/控制页分派；`/terminal` 暂时保留为兼容 path，但与 `/chat` 一样进入 `RuntimeRouteHost`。`routeState.ts` 负责路由解析、派发与 `session_id` 短 hash query 协调，`WorkbenchContext.tsx` 暴露当前 route/language/navigate 以及移动端主导航状态
- `features/conversation-runtime` 承载共享对话运行态 UI：`ConversationRuntimeProvider` 复用 Terminal session API 的数据模型和状态机，并按传入 route 分别通过 `/api/chat/sessions` 与 `/api/terminal/sessions` 使用独立 owner 存储来负责会话、输入提交、会话恢复轮询、草稿恢复、短 hash URL 恢复和模型/能力项选择；`RuntimeRouteHost` 会把 `/chat` 与 `/terminal` 都挂载到 Conversation runtime，但保留真实 `route` 作为 API、缓存和草稿命名空间。历史 `chat` 本地 snapshot 与 session `turns` 在加载时迁移到当前 Chat 会话结构，不再维护 Chat 专用消息发送链路。`ConversationRuntimeProvider` 额外维护同一 SPA 工作台内的 24 小时运行态内存缓存，并为 `chat / terminal` 分别写入独立 `sessionStorage / localStorage` key：active session、文本草稿、附件草稿、过程披露过滤、完整消息快照和轻量会话信息快照互不复用；旧合并快照仅作为迁移 fallback 读取。`ConversationWorkspace` 接入共享 `RuntimeWorkspacePage` 组装「会话列 + 主时间线工作区 + Composer + Inspector」运行页，并在窄屏下统一输出 `Menu / 标题 / New` 操作行；不再渲染 Skill 选择器、Deliverables、Session Profile 或独立 Skill 面板
- `features/shell` 负责主导航、共享 copy、运行页骨架与 React 管理页：`components/RuntimeWorkspacePage.tsx` 提供兼容运行页骨架、导航持有的会话列表、工作区 body 与 slot 化头部/正文/底部区域，并把当前运行页的 count、列表 body 与 `New` 动作绑定给主导航；`PrimaryNav.tsx` 稳定持有 `Sessions / New` 公共 chrome，`WorkbenchApp.tsx` 在运行页 rail 数据尚未注册时提供禁用态 `New` fallback rail，并按 route 缓存最近一次有效 rail body；`RuntimeWorkspaceHeader.tsx` 统一输出运行页的标题、状态按钮与 `Details` 按钮；`ReactManagedRouteBody` 只分派 Settings 管理页。`ReactManagedTerminalRouteBody.tsx` 保留为独立 Terminal 兼容组件与测试对象，继续维护旧 Terminal 控制器、附件与缓存逻辑，但当前 `/terminal` Shell 入口不再挂载该组件

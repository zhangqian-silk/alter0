...
- Runtime Resolver 的执行器选择：默认 Codex、显式 Provider/Claude 走 Claude Code、显式 Codex 走 Codex、Claude 失败直接返回错误。
- runtime、skill、MCP、memory、runtime context prompt 与 metadata 组装。
- 工作区解析、session repo clone、repo root 模式。
- 既有 Agent runtime Skill/ReAct/Codex 执行源、模型选择、委派、Memory 工具的兼容测试。

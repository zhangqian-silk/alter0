...
func TestExecuteAgentInjectsSelectedTravelSkillFromControlCatalog(t *testing.T) {
	...
	source := &stubSkillSource{items: []controldomain.Capability{
		{
			...
			Enabled: true,
			Metadata: map[string]string{
				skillFilePathKey:    "docs/skills/travel/SKILL.md",
				skillDescriptionKey: "Reusable city guide workflow.",
				skillGuideKey:       "Read docs/skills/travel/SKILL.md before composing a city guide.",
			},
		},
	}}
	service := NewServiceWithSkills(processor, source, nil)
	...
	result, err := service.ExecuteAgent(context.Background(), shareddomain.UnifiedMessage{
		...
		TraceID:     "t-travel-skill",
		Metadata: map[string]string{
			skillIncludeFilterKey: `["travel"]`,
		},
	})
	if err != nil {
		t.Fatalf("ExecuteAgent() error = %v", err)
	...
	travelSkill := skillContext.Skills[0]
	if travelSkill.ID != "travel" {
		t.Fatalf("skill id = %q, want travel", travelSkill.ID)
	}
	if travelSkill.FilePath != "docs/skills/travel/SKILL.md" {
		t.Fatalf("travel skill file path = %q, want docs/skills/travel/SKILL.md", travelSkill.FilePath)
	}
}

...
- `Chat / Agent Runtime / Terminal` 进入已有内容会话时默认定位到消息时间线或 Terminal 输出区底部，优先展示最新上下文；该初始定位仅在会话进入或切换时生效，不得在同一会话持续更新、轮询刷新或 Process 展开期间覆盖用户的历史阅读滚动。
...
- `Chat / Agent Runtime / Terminal` 的当前活动会话必须稳定反映到 URL query，作为刷新恢复的第一优先级事实来源：`chat_session_id` 对应 Chat，`agent_session_id` 对应 Agent Runtime，`terminal_session_id` 对应 Terminal；切换会话后 query 立即更新，刷新或直接打开当前链接时优先按该参数恢复对应会话，不得退回其他会话。
...
- 直连 Codex 的 Chat / Agent 会话会在各自工作区下额外维护 `.alter0/codex-runtime/`、`.alter0/codex-runtime/codex-home/` 与 `.alter0/codex-runtime/thread.json`；Terminal 会话会在 `.alter0/workspaces/terminal/sessions/<terminal_session_id>/codex-home/` 下维护独立 `CODEX_HOME`。

...
- `Chat` 前端缓存需保留当前已加载会话的完整消息：同一工作台内的 8 小时运行态缓存不得裁剪消息；浏览器侧还需维护 30 天长期 `localStorage` 快照，用于刷新、重开或 `sessionStorage` 丢失时优先恢复首屏。长期缓存只作为本地加速与断网前置恢复，服务端 Session history 与后续单会话详情回源仍是最终事实源。
...
- 运行时执行过程需以统一 `RuntimeTraceEvent` 数据模型承载，并覆盖 Terminal turn、Terminal input 结果与会话历史持久化。Terminal turn 摘要需直接输出 `runtime_trace_events` 作为 Chat / Terminal 过程展示的 canonical 数据；事件详情通过 `session_id / turn_id / event_id` 索引。事件类型、来源、角色、生命周期、状态、block 与 action 信息只能来自底层 provider、工程 adapter 或 alter0 自身确定生成的字段，不允许通过自然语言内容 heuristic 猜测。前端优先消费结构化事件，并仅按 `RuntimeTraceEvent.kind` 执行过程披露过滤，不依赖解析自然语言过程文本。
...
- 移动端 Web Shell 使用 `VisualViewport` 驱动的 `--mobile-viewport-height`、`--keyboard-offset` 与 `--keyboard-composer-offset` 协调壳体和输入区：浏览器工具栏切换时壳体继续贴合可视区域，软键盘弹起时主工作区保持稳定高度，底部 Composer 只按可见底边偏移移动；移动端根文档禁止页面级滚动，workspace header、正文滚动区、空态、阅读定位条、命令候选和配置面板不消费键盘偏移，不出现底部空白、内容裁切、整页上移或输入区重复上移。
...
- `Chat / Terminal` 的移动端键盘弹出链路需保留浏览器原生输入框聚焦手势：首次触摸主输入框时不得在 `pointerdown / touchstart` 捕获阶段取消默认行为或抢先手动 `focus()`；程序化 `preventScroll` 聚焦仅用于 slash command、创建新会话后回到 Composer 等非直接输入框触摸场景。输入框保持真实焦点期间，若移动端浏览器在键盘动画中改写页面级滚动、运行页祖先容器或正文滚动容器，运行页必须在 `window.scroll`、`VisualViewport resize/scroll` 与短延迟复核中锚回聚焦前位置，避免公共操作行、workspace header 或空态正文被整页顶出可视区；键盘动画不得造成页面整体分辨率/可视区域突变。
...
- 输入框聚焦且软键盘已确认弹出后，移动端 `VisualViewport` 在键盘动画中短暂回报完整高度时，不得立即清空 `--keyboard-offset` 或重置 `--mobile-viewport-height`；若浏览器同时回报收缩后的 `height` 与正 `offsetTop`，仍需按收缩高度保持键盘占位，不能用 `height + offsetTop` 抵消正文遮挡；Composer 贴底位移需在短过渡窗口内忽略临时 `offsetTop` 以避免键盘抬起过程突跳，窗口结束或稳定事件到来后再从键盘占位中扣除 `offsetTop`，避免浏览器已平移可视窗口后再次上移输入区。
...
- Chat / Terminal 单会话详情默认只返回最新 `20` 个 turns，并按约 `256KiB` 的 turns 页预算控制单次响应体；长会话通过 `turn_limit` 与 `turn_before` 按批次读取。分页响应必须带 `turns_paging`，其中包含数量边界与 `byte_limit / approx_bytes`；前端刷新、轮询和恢复时按 turn/message id 合并新片段，不得因轻量响应覆盖已加载历史。

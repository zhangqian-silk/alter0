...
- Web Shell 在桥接阶段仍需保持稳定 legacy DOM 契约；其中主导航的当前路由高亮、折叠态与语言感知文案，以及 Session Pane、ChatWorkspace 头部动作区、Session 历史空态提示/可访问标签、路由页头部标题/副标题的历史折叠、会话入口、菜单与页面文案由 React 壳层直接维护，不能因为壳层状态更新而清空会话列表、消息区、route body、runtime host 或 legacy runtime 直接写入的标题挂载节点。

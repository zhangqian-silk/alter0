...
| R-038 | 对话历史面板折叠能力 | planned | 在 Chat 页面支持“最近会话”历史区折叠/展开，减少侧栏占用并提升长对话阅读空间 |
| R-039 | 页面滚动隔离（侧栏固定） | planned | 全站页面默认固定侧边栏并禁止其随主页面滚动；仅在侧栏内容受高度限制发生溢出时允许侧栏自身内部滚动 |
| R-040 | Cron 可视化配置与触发会话归档 | planned | 支持以可视化方式配置 Cron 与任务（周期/固定时间点），实时展示 cron 表达式；Cron 触发后创建专属会话并可在会话历史中查看 |
| R-041 | 任务执行前复杂度预判与同步/异步分流 | planned | 用户消息进入执行器前先做复杂度预估；若创建异步任务则先同步返回可跳转任务详情的任务卡片（含任务 ID 与简要描述），任务完成后向用户主动同步结果 |
| R-042 | 异步任务执行模块与前端全量执行明细 | planned | 建立专门异步执行模块并限制最多 5 个任务并发执行；前端任务模块按当前界面风格展示任务执行明细，并完整回传终端运行细节（如“已运行 xxx”） |
| R-043 | Environments 关键配置统一管理 | planned | 完善 Environments 模块，支持对任务并发等关键运行参数进行可视化配置、校验与持久化，并将当前可配置项统一纳入同一管理入口 |
| R-044 | Channels 迁移至 Settings 模块 | planned | 前端导航将 `Channels` 从 `Control` 分组迁移到 `Settings` 分组，保持页面能力与接口不变并兼容原有直达路由 |

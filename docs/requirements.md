...
- `Chat` 前端缓存需保留当前已加载会话的完整消息：同一工作台内的 8 小时运行态缓存不得裁剪消息；浏览器侧还需维护 30 天长期 `localStorage` 完整消息快照，用于刷新、重开或 `sessionStorage` 丢失时优先恢复首屏；同时维护轻量会话信息快照，用于完整消息缓存写入失败或被清理时恢复会话列表与活动会话元数据。长期缓存只作为本地加速与断网前置恢复，服务端 Session history 与后续单会话详情回源仍是最终事实源。

...
- 四键阅读定位条仅保留在 `Agent` 运行态消息区与 `Terminal` 对话区，采用右侧箭头四键承载 `回到顶部 / 上一条 / 下一条 / 回到底部`；普通 `Chat` 消息区与其他托管 route 页不展示该控件，定位目标按当前视口中的可见消息块或 Terminal turn 动态计算。Terminal 移动端四键定位条固定停靠在工作区右侧、输入区上沿之上，四键统一使用独立圆形触达面。

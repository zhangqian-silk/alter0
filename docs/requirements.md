...
- `Chat / Terminal` 的移动端键盘弹出链路需保留浏览器原生输入框聚焦手势：首次触摸主输入框时不得在 `pointerdown / touchstart` 捕获阶段取消默认行为或抢先手动 `focus()`；程序化 `preventScroll` 聚焦仅用于 slash command、创建新会话后回到 Composer 等非直接输入框触摸场景。输入框保持真实焦点期间不得用程序化 `window.scrollTo` 抢占浏览器键盘动画，不允许首次弹出软键盘时把公共操作行顶出可视区，也不允许因 `VisualViewport` 首次收缩造成页面整体分辨率/可视区域突变。
...
- 输入框聚焦且软键盘已确认弹出后，移动端 `VisualViewport` 在键盘动画中短暂回报完整高度时，不得立即清空 `--keyboard-offset` 或重置 `--mobile-viewport-height`；若浏览器同时回报收缩后的 `height` 与正 `offsetTop`，仍需按收缩高度保持键盘占位，不能用 `height + offsetTop` 抵消键盘偏移；Composer 与正文区需保持上一帧稳定键盘占位，直到视口恢复状态稳定。
...
- Runtime 设置页支持在线实例启动时间与 commit hash 展示、运行时重启、远端 master 快进同步、仅在二次确认后丢弃 Git 已跟踪本地改动的重启前同步策略、候选二进制构建、readyz 探活与失败回滚。旧运行参数配置页、环境变量可视化配置、队列、终端 shell、记忆路径参数均不再对用户暴露。

...
- `Chat / Terminal` 的移动端键盘弹出链路需保留浏览器原生输入框聚焦手势：首次触摸主输入框时不得在 `pointerdown / touchstart` 捕获阶段取消默认行为或抢先手动 `focus()`；程序化 `preventScroll` 聚焦仅用于 slash command、创建新会话后回到 Composer 等非直接输入框触摸场景。输入框保持真实焦点期间，运行页只同步 composer 贴底偏移和遮挡高度，不主动改写页面级滚动、运行页祖先容器或正文滚动容器的 `scrollTop`，避免前端滚动补偿与软键盘动画叠加成可见突变；键盘动画不得造成页面整体分辨率/可视区域突变。

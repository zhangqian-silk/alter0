...
- `Chat / Agent Runtime` 首页 Composer、会话卡片与 `Details` 面板需维持同一套浅色 terminal-runtime 表面系统：输入框作为单一紧凑主输入面，桌面与移动端都要控制输入高度、底部留白和发送按钮体量，发送按钮直接复用 Terminal 的紧凑 icon submit 皮肤；PC 端上传、发送、状态、详情、短 hash 与弹窗动作统一采用低圆角矩形，不使用胶囊按钮或胶囊标签；meta 与发送按钮收敛到同一底部工具行，会话卡片和详情面板不再退回旧式轻表单或松散卡片观感；空态工作区禁止保留可拖拽滚动，不得把头部操作行或输入区顶出可视区。

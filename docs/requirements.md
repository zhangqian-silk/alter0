...
- `Chat` 消息提交统一使用 `POST /api/chat/sessions/{session_id}/input`，复用 runtime 数据模型与状态机但与 Terminal 默认 session 列表分开存储；`/terminal` 兼容 path 复用 Conversation runtime UI，但继续通过 `route=terminal` 接入 `/api/terminal/sessions` 前端 owner。前端不再接入 `/api/messages`、`/api/messages/stream`、消息 SSE、SSE parser、本地 `Thinking` 过程步骤或后台 Task API。发送后由当前 owner session 状态标记 `busy`，执行完成或失败后，再通过返回的 session 或当前 owner 的 session 详情恢复写回 assistant 正文与结构化步骤。

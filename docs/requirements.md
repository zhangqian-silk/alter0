...
- `Chat / Agent Runtime / Terminal` 统一展示右侧四键阅读定位条，承载 `回到顶部 / 上一条 / 下一条 / 回到底部`；定位目标按当前视口中的可见消息块或 Terminal turn 动态计算。`回到底部` 只在最后一条内容的底边仍位于视口外时显示，不得因为消息区尾部仅剩空白或 padding 继续保留伪底部跳转。移动端四键定位条固定停靠在工作区右侧、输入区上沿之上，四键统一使用独立圆形触达面。

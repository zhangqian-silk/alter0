...
- Web Shell 在桥接阶段仍需保持稳定 legacy DOM 契约；其中主导航的当前路由高亮、折叠态、导航 tooltip 与语言感知文案，以及 Session Pane、会话卡片列表、ChatWorkspace 头部动作区、欢迎区文案、欢迎区 target picker、欢迎区/消息区显隐、消息列表 DOM、运行时 controls/note/sheet DOM、Session 历史空态提示/可访问标签、路由页头部标题/副标题的历史折叠、会话入口、菜单与页面文案由 React 壳层直接维护；导航抽屉、会话抽屉、导航折叠同步、会话历史折叠同步、主导航跳转、新建会话入口、欢迎区快捷提示、会话聚焦、会话删除与语言切换统一通过 React 发出的结构化 bridge 事件交给 legacy runtime 执行业务流转，当前会话标题、副标题、欢迎区描述与 target picker 通过 chat workspace snapshot bridge 回写到 React，会话卡片列表、空态与加载错误通过 session pane snapshot bridge 回写到 React，欢迎区/消息区显隐、空态与消息 HTML 快照通过 message region snapshot bridge 回写到 React，运行时 controls、错误提示、移动端 runtime sheet 与滚动位置通过 chat runtime snapshot bridge 回写到 React，`channels / skills / mcp` 三类卡片页的加载态、错误态与卡片 HTML 通过 route body snapshot bridge 回写到 React；不能因为壳层状态更新而清空其余复杂 route body。

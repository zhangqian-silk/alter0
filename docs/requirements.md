...
- 请求断开或页面刷新后，前端优先静默回源当前会话详情，而不是立即把同一请求重发一遍。该恢复链路即使在服务端集合接口已返回当前会话摘要、但尚未附带完整消息时也必须继续触发。transport failure 与详情回读失败均不得自行把会话收敛为失败态；只有后端详情或增量事件返回失败事实时才展示会话失败。本地缓存中残留的旧版 `streaming / Load failed` 客户端占位不得继续作为会话事实。
...
- `Chat` 的会话状态交互以服务端业务状态为准：请求提交期间仅使用非持久化 Composer `submitting` 锁，收到服务端 input 接受结果后，当前活动会话和左侧对应会话项才按后端状态进入处理中。transport failure 只在 Composer 工具栏显示短暂弱提示并静默回读详情；结构化 HTTP 后端错误可在同一位置显示安全服务端消息。两类请求错误都不得改写 session、turn、assistant 消息、会话排序或浏览器会话缓存；失败、中断和删除只由服务端会话详情或明确增量事件决定。
...
- `Chat` 发送输入时生成 `client_request_id` 并随 input 提交，但在服务端确认前不得把 optimistic user message、`local_running` 或其他客户端推测写入 route 状态与浏览器会话缓存；服务端对应 turn 回传同一 `client_request_id` 完成对账。提交锁与弱提示属于 Composer 临时状态，不参与会话合并或持久化。会话运行态只能通过共享 runtime controller 接收服务端 input、updates、列表和详情；这些响应必须先校验目标 session、请求代次、`update_id` 与内容 `updated_at`，旧响应在任何 message/turn 合并前整体拒绝。summary 只更新列表字段，title/pin 变化不得推进内容 `updated_at` 或重写内容缓存；detail/event 才能推进内容。
...
- Chat 会话态统一为 `ready / busy / failed / exited / interrupted`，执行态在 turn/runtime event 维度维护 `running / completed / failed / interrupted`；服务端已接受请求后的 runtime 执行失败由后端使 session 与 turn 同步进入 `failed`，但浏览器 transport failure、HTTP 后端拒绝或解析错误不得由前端映射成会话失败。failed session 仍可接受下一条输入并重新进入 busy。运行态退出、失败或中断后保留历史；Composer 只由后端真实 busy 状态或当前非持久化 submitting 锁定。

...
- Environments 桌面端默认采用高密度配置表 + 详情面板：左侧表格展示状态、Key、Value、Type 与 Module，右侧详情面板展示 apply mode、effective value、source 与 description。
...
- `web` 前端静态资源只覆盖 Web Shell 前端构建；`/api/*`、登录态、健康检查和共享后端能力仍由主运行时提供。
...
- Session 级测试服务的标准部署入口为 `scripts/deploy_test_service.sh`，它负责启动或注册工作区服务，并调用共享运行时的 workspace service 注册接口。

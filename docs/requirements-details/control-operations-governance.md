...
| `SessionPreviewRegistration` | 会话短哈希域名到工作区前端构建的绑定关系 |
...
- Web Shell 桥接阶段由 React 直接承接 `agent`、`channels`、`skills`、`mcp`、`models`、`environments` 与 `cron-jobs` 七类控制台路由体的读取、加载、空态与错误态渲染；其中 `agent` 路由已由 React 承接托管 Agent 列表、表单编辑、保存、删除与进入运行页入口；legacy runtime 不再托管这些页面的主要 DOM。
...
- `GET /api/control/previews`、`GET /api/control/previews/{session_id}`、`PUT /api/control/previews/{session_id}`、`DELETE /api/control/previews/{session_id}` 管理会话前端预览注册表。
...
- 会话预览域名固定使用 `sha1(session_id)` 的前 8 位短哈希，格式为 `https://<session_short_hash>.alter0.cn`。
- 控制面通过 `PUT /api/control/previews/{session_id}` 注册当前会话工作区的前端构建目录，要求 `repository_path` 指向已构建的 git 工作区。
- 删除或重绑预览时，通过 `DELETE /api/control/previews/{session_id}` 或再次 `PUT` 完成更新。

### 分发规则

- 当请求 Host 命中已注册短哈希域名时，共享运行时直接分发该会话工作区的 `/chat` HTML 与 `static/dist/assets`、`static/dist/legacy` 资源。
- 预览静态资源只覆盖 Web Shell 前端构建；`/api/*`、登录态、健康检查和共享后端能力仍由主运行时提供。
- 预览注册表需持久化到 `.alter0/session-previews.json`，以便运行时重启后继续保持会话域名绑定。
...
- 会话前端预览的标准部署入口为 `scripts/deploy_session_preview.sh`，它负责构建当前工作区前端产物并调用共享运行时的预览注册接口。
...
- Channel、Capability、Skill、MCP、Agent、Product、Cron、Models、Environments 控制面接口可用。

...
- 控制面默认展示运行时概览条、当前 Codex 管理区、托管账号卡片列表与导入/登录操作侧栏；概览区需展示当前活动 auth 路径、托管数量与当前账号，当前 Codex 管理区需展示 CLI 命令、auth 状态、config 状态、config 路径、当前 profile、当前 model、当前思考深度以及配置来源，操作区需持续显示最近一次登录会话状态与日志。
...
- 托管账号区需按断点切换布局：大屏优先展示当前 Codex 管理区、多列账号卡片与右侧操作栏，中屏切换为全宽账号区加下方双侧栏，窄屏回落为单列卡片；套餐、小时/周额度、当前 model、思考深度与切换按钮不得依赖横向滚动后才可见。

...
10. 移动端信息收纳要求：会话列表默认收纳为可展开的会话面板；当前会话的 `terminal_session_id`、CLI 路径、工作目录等元信息默认折叠，用户按需展开查看并可复制。
   - 工作区头部需提供单行摘要操作栏，至少包含当前会话标题、状态、`Details`、`Close`、`Delete`，避免在小屏上拆成多行松散布局。
...
12. 浏览增强要求：长回复滚动阅读时，当前用户问题需在消息区顶部保持真吸顶提示；吸顶行为必须直接复用原始用户消息节点本身，不得额外构造一份顶栏副本；该节点作为消息滚动容器内的真实吸顶节点参与布局，不得以脱离文档流的覆盖层遮挡后续内容或露出底下信息；吸顶提示与原消息应采用同构的单层消息条结构，不再额外套嵌第二层卡片；吸顶区域本身需提供不透底背景、底部收口阴影与轻量暗色滚动条风格，保证长文阅读时视觉稳定；当 `Process` 或 `Final Output` 进入阅读区顶部时，其头部需继续保持吸顶，方便用户随时点击折叠/展开；当用户离开底部一定距离或有未读输出时，界面需提供一键回到底部入口。
   - 吸顶提示仅作为上下文辅助信息，不得阻挡当前输出区的 `Show more / Show less`、`Process` 折叠等交互按钮点击；用户点击顶部吸顶提示后，界面需直接返回该用户消息在消息流中的原始位置。
...
14. 验收：用户在 `Terminal` 模块创建会话后可连续发送多轮命令与追问；界面持续展示同一会话链路的输入/输出，且会话上下文保持连续；移动端输入过程中页面不会因轮询刷新或输入法候选确认跳回顶部，长输出与会话元信息可按需展开，并支持用户问题真吸顶、点击顶部提示回到原消息位置、`Process / Final Output` 头部吸顶折叠与一键回到底部。

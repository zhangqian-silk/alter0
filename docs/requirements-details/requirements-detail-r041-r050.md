...
12. 浏览增强要求：长回复滚动阅读时，当前用户问题需在消息区顶部保持真吸顶提示；该提示需作为消息滚动容器内的真实吸顶节点参与布局，不得以脱离文档流的覆盖层遮挡后续内容或露出底下信息；当 `Process` 或 `Final Output` 进入阅读区顶部时，其头部需继续保持吸顶，方便用户随时点击折叠/展开；当用户离开底部一定距离或有未读输出时，界面需提供一键回到底部入口。

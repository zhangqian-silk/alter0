...
12. 浏览增强要求：长回复滚动阅读时，当前用户问题需在消息区顶部保持吸顶提示；当 `Process` 或 `Final Output` 进入阅读区顶部时，其头部需继续保持吸顶，方便用户随时点击折叠/展开；当用户离开底部一定距离或有未读输出时，界面需提供一键回到底部入口。
   - 吸顶提示仅作为上下文辅助信息，不得阻挡当前输出区的 `Show more / Show less`、`Process` 折叠等交互按钮点击。
...
14. 验收：用户在 `Terminal` 模块创建会话后可连续发送多轮命令与追问；界面持续展示同一会话链路的输入/输出，且会话上下文保持连续；移动端输入过程中页面不会因轮询刷新或输入法候选确认跳回顶部，长输出与会话元信息可按需展开，并支持用户问题吸顶、`Process / Final Output` 头部吸顶折叠与一键回到底部。

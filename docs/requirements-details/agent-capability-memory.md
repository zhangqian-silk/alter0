...
- 存在可用 Provider 且进入 Agent / ReAct 链路时，Agent 自身吸收 `runtime_context`、`product_context`、`product_discovery`、`skill_context`、`mcp_context`、`memory_context`，只向 Codex 下发当前步骤的纯执行指令。

...
- `travel`：内置旅游 Agent，显示名称为 `Travel Agent`，负责城市行程类任务的专项执行、HTML 攻略产物与只读子域名交付。
...
- `travel` 生成 HTML 攻略时，移动端是主交付视口：默认信息顺序、单列阅读、触控按钮、章节间距和横向溢出控制都必须先满足手机使用；桌面端作为辅助视口通过更宽版心、层级增强或支撑内容分组渐进增强，不得反过来牺牲移动端可读性。
- `travel` 生成 HTML 攻略时，所有涉及路线的信息都应尽量提供路线化表达：总体日程、每日路线、交通指南、步行段、换乘段、轮渡/船行段和地图提示均需在信息足够时生成路线卡片或路线图。路线表达以编号站点、连线路径、分段交通方式、预计步行/公交/轮渡时间和地标提示呈现；若缺少精确地理形态，也必须优先提供保留站点顺序、交通方式、粗略耗时和方向提示的示意路线卡。路线图需作为真实 HTML/CSS/SVG 页面结构存在，不以空白占位图替代。

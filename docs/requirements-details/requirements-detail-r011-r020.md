...
5. Chat 输入区收纳：移动端 `Chat`/`Agent` 输入区默认只保留消息输入、发送主按钮与单一“会话设置”入口；`Provider / Model`、`Tools / MCP`、`Skills` 改为在同一弹层内按需展开，不再常显多组运行时信息、附注文案与字数计数。
6. Terminal 输入稳定性：移动端 `Terminal` 路由在输入框聚焦且键盘弹起期间，后台轮询刷新不得重建输入节点、清空草稿或把页面滚动位置重置到顶部；输入法每次确认词句后，只要输入框仍保持聚焦，前端继续延迟重绘。
7. 验收：在移动端连续执行“打开抽屉 -> 切换页面 -> 输入框聚焦 -> 键盘收起”流程，页面无错位、无遮挡、可稳定关闭覆盖层；`Chat` 输入区不会因运行时信息常显而挤占主要可视空间；在 `Terminal` 输入过程中，轮询刷新与输入法候选确认均不触发回顶或输入抖动，失焦后再统一刷新视图。

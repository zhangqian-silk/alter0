...
4. 前端在流式过程中实时追加渲染，`done` 后收敛为完整消息；`error` 时展示可重试提示。
   - `Chat / Agent` 消息区在高频 `delta`、Agent `Process` 展开收起与任务状态回填期间，不得每次都整段重建整条消息列表；应优先复用未变化的消息节点，并将高频刷新合并到浏览器逐帧节奏内执行。
...
- 验证记录：
  ...
  - 2026-04-02：`Chat / Agent` 消息区改为按消息粒度 patch，并将高频流式刷新合并到 `requestAnimationFrame`；长会话中的流式阅读、过程展开与任务状态回填不再反复整段重建消息列表。

### R-014 移动端真机适配增强

1. 小屏安全区适配：移动端导航抽屉、会话面板、消息区与输入区统一纳入 `env(safe-area-inset-bottom)`，避免底部刘海/手势区遮挡。
2. 键盘跟随：基于 `VisualViewport` 实时计算软键盘占位高度并写入 `--keyboard-offset`，同时同步移动端对话区有效可视高度；`Chat / Agent` 输入区在 iOS/Android 键盘弹起、收起和窗口尺寸变化时持续贴住可见底部。底部键盘偏移仅在输入框实际聚焦且视口缩减达到软键盘阈值时生效，不把浏览器地址栏或底部工具栏的伸缩误判为键盘占位。
3. 路由页贴底：移动端 `page-mode` 路由页需同步消费 `--mobile-viewport-height` 与 `--keyboard-offset`，保证 `Terminal` 与其他信息页在浏览器底部工具栏伸缩、软键盘收起或可视视口回弹后，页面底边立即回贴可见视口，不保留额外底部空白。
4. 遮罩关闭与手势交互：支持点击遮罩、按下 `Escape`、以及在导航/会话抽屉内左滑手势关闭覆盖层。
5. 路由切换一致性：移动端点击侧边栏菜单后立即收起抽屉并进入目标信息页，防止面板残留遮挡主内容。
6. Chat 输入区收纳：移动端 `Chat`/`Agent` 输入区默认只保留消息输入，以及与发送主按钮同排的单一“会话设置”入口；`Provider / Model`、`Tools / MCP`、`Skills` 改为在同一弹层内按需展开，不再常显多组运行时信息、附注文案与字数计数。
7. 运行时文案一致性：`Chat` 输入区底部的 `Model`、`Tools / MCP`、`Skills` 与“会话设置”弹层在当前语言下需保持统一文案口径，不得出现中英混排或同一概念多套称呼并存。
8. 会话设置弹层布局稳定性：在窄宽度和长文案场景下，配置项标题、描述、Provider 标签与计数信息需支持截断或换行，不得发生重叠、覆盖或挤压到不可读；移动端弹层需以独立固定底部面板承载，带遮罩、关闭入口和内部滚动区，不得与发送区发生层叠覆盖；其中 Agent 选项仅展示短摘要，不直接铺出完整 system prompt；连续勾选 Skill / Tool / MCP 时需保持当前滚动位置，不得在每次勾选后回到顶部。
9. Terminal 输入稳定性：移动端 `Terminal` 路由在输入框聚焦且键盘弹起期间，后台轮询刷新不得重建输入节点、清空草稿或把页面滚动位置重置到顶部；输入法每次确认词句后，只要输入框仍保持聚焦，前端继续延迟重绘。
10. 移动端功耗要求：`Chat / Agent / Terminal` 的前端轮询、`VisualViewport` 同步与本地持久化需根据页面可见性、输入聚焦、滚动活跃度自动降频；页面隐藏时不得继续维持高频刷新，恢复前台后需尽快补齐一次状态同步。
11. 验收：在移动端连续执行“打开抽屉 -> 切换页面 -> 输入框聚焦 -> 键盘收起”流程，页面无错位、无遮挡、可稳定关闭覆盖层；`Chat / Agent` 输入区在 `VisualViewport` 高度变化与窗口尺寸切换期间持续贴底、焦点输入框不被键盘遮挡，键盘收起或浏览器工具栏回弹后不残留额外底部空白；移动端 `page-mode` 路由页在浏览器底部工具栏回弹或视口回升后，`Terminal` 与其他信息页底边需立即回贴可见视口；`Chat` 输入区不会因运行时信息常显而挤占主要可视空间，会话设置入口与发送按钮保持同排，文案不出现中英混排，配置弹层中的长标题与说明文案保持可读、不互相重叠，独立底部面板内容可完整滚动且不会被发送区遮挡；在 `Terminal` 输入过程中，轮询刷新与输入法候选确认均不触发回顶或输入抖动，失焦后再统一刷新视图；页面切到后台时移动端不再维持高频轮询。
...
- 验证记录：
  ...
  - 2026-04-03：移动端 `page-mode` 路由页补充消费 `VisualViewport` 高度；`Terminal` 与其他信息页在底部工具栏伸缩、键盘收起或视口回弹后不再残留底部空白。
  - 2026-04-03：移动端 `Chat / Agent` 的“会话设置”改为独立固定底部面板，新增遮罩、关闭按钮与内部滚动区；长配置内容不再与底部发送区发生层叠覆盖。
  - 2026-04-03：`Agent` 选择卡片改为短摘要展示，优先使用 Agent description，并限制为简短可扫读的两行文案；完整 system prompt 不再直接出现在会话设置面板。
  - 2026-04-03：会话设置在连续勾选 Skill / Tool / MCP 时改为保留当前滚动位置；移动端长列表支持连续选择，不会在每次勾选后跳回顶部。
  - 2026-04-03：移动端键盘偏移补偿收紧为“仅在输入框聚焦且软键盘占位达到阈值时生效”；浏览器底部工具栏回弹、地址栏伸缩与键盘收起后不再保留额外底部留白。

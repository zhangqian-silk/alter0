...
9. 验收：在移动端连续执行“打开抽屉 -> 切换页面 -> 输入框聚焦 -> 键盘收起”流程，页面无错位、无遮挡、可稳定关闭覆盖层；`Chat / Agent` 输入区在 `VisualViewport` 高度变化与窗口尺寸切换期间持续贴底、焦点输入框不被键盘遮挡；`Chat` 输入区不会因运行时信息常显而挤占主要可视空间，会话设置入口与发送按钮保持同排，文案不出现中英混排，配置弹层中的长标题与说明文案保持可读、不互相重叠；在 `Terminal` 输入过程中，轮询刷新与输入法候选确认均不触发回顶或输入抖动，失焦后再统一刷新视图。

...
7. Terminal 输入稳定性：移动端 `Terminal` 路由在输入框聚焦且键盘弹起期间，后台轮询刷新不得重建输入节点、清空草稿或把页面滚动位置重置到顶部；输入法每次确认词句后，只要输入框仍保持聚焦，前端继续延迟重绘。
8. 验收：在移动端连续执行“打开抽屉 -> 切换页面 -> 输入框聚焦 -> 键盘收起”流程，页面无错位、无遮挡、可稳定关闭覆盖层；`Chat` 输入区不会因运行时信息常显而挤占主要可视空间，会话设置入口与发送按钮保持同排，文案不出现中英混排；在 `Terminal` 输入过程中，轮询刷新与输入法候选确认均不触发回顶或输入抖动，失焦后再统一刷新视图。
...
- 验证记录：
  ...
  - 2026-03-30：Chat / Agent / Async Task 默认执行目录收敛到仓库根目录；历史版本遗留的 `.alter0/workspaces/sessions/<session_id>` 仍保留删除联动清理。

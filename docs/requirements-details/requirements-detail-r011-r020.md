...
7. 会话设置弹层布局稳定性：在窄宽度和长文案场景下，配置项标题、描述、Provider 标签与计数信息需支持截断或换行，不得发生重叠、覆盖或挤压到不可读；移动端弹层需以独立固定底部面板承载，带遮罩、关闭入口和内部滚动区，不得与发送区发生层叠覆盖；其中 Agent 选项仅展示短摘要，不直接铺出完整 system prompt；连续勾选 Skill / Tool / MCP 时需保持当前滚动位置，不得在每次勾选后回到顶部。

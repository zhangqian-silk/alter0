...
- Terminal 桌面端维持旧版 master-detail 布局关系：左侧会话列表承载标题、最近输出与状态徽标，右侧工作区头部收敛为标题、状态点与 `Details / Close / Delete` 工具栏；Terminal route body 顶部不再额外挂载页面级说明 hero。
...
- 工作区头部收敛为会话标题、状态点和紧凑工具栏；真手机宽度下允许标题与工具栏换行，`Details / Close / Delete` 不得被长标题挤出屏幕。
- 长输出阅读取消消息与区块头部吸顶，保持自然文档流滚动。
- 右侧圆形四键组支持回到顶部、上一条、下一条与回到底部。
- 浏览器底部工具栏伸缩、软键盘收起或视口回弹后，底部输入条立即回贴可见底边。
- 输入框聚焦时仅允许 Composer 自身按键盘偏移上移，不能把工作区整体撑高到可视视口之外；长对话下输入框仍需保持可见、可聚焦、可提交。

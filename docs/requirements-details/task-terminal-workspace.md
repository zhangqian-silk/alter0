...
- Web Shell 中的 Terminal 路由页主体由 React 通过稳定 host 托管，`routeBody` 必须暴露 `data-react-managed-route="true"`，避免 legacy runtime 再走整页 `renderRoute(routeBody)` 加载链。
- Terminal 现有会话恢复、轮询、输入、关闭、删除、step 展开与日志滚动逻辑继续复用 legacy terminal controller，但挂载入口改为 React-managed host 调用的 terminal mount API。

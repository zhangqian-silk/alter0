...
- `POST /api/control/tasks/{task_id}/terminal/input` 为交互式任务追加输入并创建 follow-up Task。
...
- `POST /api/terminal/sessions/{session_id}/input` 发送 Terminal 输入。

### Terminal Web Shell

- Web Shell 中的 Terminal 路由页主体由 React 原生实现，`routeBody` 必须暴露 `data-react-managed-route="true"`，避免其他运行时再次接管该页 DOM。
- Terminal 页面直接请求 `/api/terminal/sessions`、`/api/terminal/sessions/{session_id}`、`/api/terminal/sessions/{session_id}/turns/{turn_id}/steps/{step_id}` 等接口，并在 React 内维护会话恢复、轮询、输入、关闭、删除、step 展开、滚动定位与本地草稿恢复。
- Terminal 的 session pane 容器、workspace 容器与主视图外壳在 React rerender 期间必须保持稳定实例，不能因语言切换、hash 路由变化或壳层状态更新而清空正在运行的终端内容。
- Terminal 运行页需挂载在共享 runtime workspace framework 上：统一复用会话侧栏、workspace body、slot 化头部/正文/底部区域与 backdrop 结构，同时允许 Terminal 继续注入自身的状态 pill、详情面板、Process、跳转四键与 Composer 控件。
...
- `POST /api/terminal/sessions/{session_id}/close` 关闭 Terminal 运行态。
...
- 会话详情响应包含已持久化 turn 摘要；单个 turn/step 明细可按会话、turn 与 step 标识读取。
...
- `Input` 向指定 Terminal 会话追加用户输入，执行完成前会话进入 `busy`。
...
- Terminal 桌面端维持旧版 master-detail 布局关系：左侧会话列表承载标题、最近输出、状态徽标与删除入口，右侧工作区头部收敛为标题、状态点与 `Details / Close` 工具栏；Terminal route body 顶部不再额外挂载页面级说明 hero。
- Terminal React 版继续保留 `terminal-*` DOM class 与数据钩子，但会话栏、工作区容器、工作区头部和窄屏 `Menu / Sessions / New` 操作行需与 Chat / Agent Runtime 复用同一套工作台表面语义，避免同属运行页却出现独立壳层节奏。
...
- Terminal 在 `1100px` 及以下窄屏下由 `ReactManagedTerminalRouteBody` 直接输出 `Menu / Sessions / New` 顶部操作行：`Menu` 打开主导航抽屉，`Sessions` 打开会话抽屉，`New` 直接创建终端会话。
- Terminal 窄屏消息阅读继续由 `terminal-chat-screen` 作为唯一纵向滚动容器；`workbench-main / chat-pane / route-view / route-body / terminal-view` 只负责提供满高和滚动隔离，不允许因高度未闭合或外层 `overflow: hidden` 造成消息页无法滑动。
- 工作区头部收敛为会话标题、状态点和紧凑工具栏；真手机宽度下允许标题与工具栏换行，`Details / Close` 不得被长标题挤出屏幕。

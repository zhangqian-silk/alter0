...
- `POST /api/terminal/sessions/{session_id}/input` 发送 Terminal 输入；请求体除 `input` 外还接受 `attachments[]` 图片载荷，允许仅发送图片。
...
- `Input` 向指定 Terminal 会话追加用户输入，执行完成前会话进入 `busy`；输入支持文本与图片混合载荷，纯图片输入由服务端补齐稳定占位文本。
...
- Terminal 页面与 Task 详情抽屉中的 follow-up 输入条共享图片附件体验：选择图片后立即显示缩略图，支持点击预览与移除，发送后由历史区回显该轮图片。
...
- Terminal 桌面端维持旧版 master-detail 布局关系：左侧会话列表承载标题、最近输出、状态徽标与删除入口，右侧工作区头部收敛为标题、状态点与 `Details` 工具栏；Terminal route body 顶部不再额外挂载页面级说明 hero。
...
- 工作区头部收敛为会话标题、状态点和紧凑工具栏；真手机宽度下允许标题与工具栏换行，`Details` 不得被长标题挤出屏幕。

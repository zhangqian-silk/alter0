...
- 基线：`master@97faefe`
- 仓库同步：已执行 `git fetch --all --prune`，远端残留 `origin/feat/p0-legacy-task-audit-remote-cleanup`，与主线内容一致。
...
| origin/feat/p0-legacy-task-audit-remote-cleanup | 已被新方案覆盖 | 远端分支与 `master` 无内容差异，保留分支不影响主线执行。 |

...
- `ConversationWorkspace.tsx` 在 Chat 时间线滚动容器上同时监听 `scroll` 与原生 `touchstart / touchmove`：普通滚动到顶时先展开本地隐藏批次，已完全展开后调用 `refreshActiveSession()` 回源当前 Chat 会话详情；移动端已处于顶部时，如果继续向下触摸拉动超过固定阈值，同样触发这条补拉链路，避免 `scrollTop` 已经为 0 时浏览器不再派发有效滚动事件。手动补拉由 in-flight guard 限制为同一时间只有一个请求，返回后按既有消息合并规则保留当前已加载历史。

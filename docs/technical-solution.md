...
- `internal/interfaces/web/frontend/src/features/shell` 继续维护主导航、共享 copy、React 管理页和 route surface。`components/PrimaryNav.tsx` 负责路由高亮、导航折叠、tooltip 与语言切换，品牌区只渲染文字 `Alter0`，不再输出图形 logo 节点；`components/ReactManagedRouteBody.tsx` 负责把 `agent / terminal / memory / channels / skills / mcp / models / environments / cron-jobs / sessions / tasks / codex-accounts` 分派到各自 React 页面。`RuntimeWorkspaceHeader.tsx` 负责共享 `Chat / Agent Runtime / Terminal` 的固定 workspace header，只保留会话标题、状态按钮和 `Details` 入口，并把差异内容交给各页传入。`RuntimeWorkspaceShell.tsx` 在窄屏下直接输出运行页顶部 `Menu / Sessions / New` 操作行，并为 session pane 的 `New / Hide sessions` 按钮统一注入图标壳层；`ReactManagedTerminalRouteBody.tsx` 继续保持旧版 `terminal-*` DOM class 契约作为布局皮肤基线，但会话列表、工作区容器、工作区头部与窄屏顶部操作行额外复用 `ConversationWorkspace` 的工作台语义类，确保 Terminal 与 Chat / Agent Runtime 使用同一套表面和头部节奏；Terminal 的会话抽屉列表与 Conversation 一样输出 `role="list"` 语义、卡片式标题、单行时间与短标识元信息，以及尾侧更多按钮，运行状态保留在 workspace header 与 `Details` 摘要中，不再在列表项内额外挂独立状态徽标，详情面板则先复用共享紧凑摘要栅格，再由 Terminal 自己补充会话字段和公有 Skill 选择区；Terminal 的会话列表、详情轮询、step 展开、列表删除、Skill 选择与 Markdown 输出仍全部由 React state 直接维护；控制页、Sessions、Tasks 与 Memory 继续复用统一客户端和共享 surface 样式。
...
- `src/app/routeState.ts` 负责运行页路由与会话 query 协调：保留现有 hash 路由的同时，新增 `chat_session_id`、`agent_session_id`、`terminal_session_id` 三个稳定 query 参数。`ConversationRuntimeProvider.tsx` 在初始化时优先读取 Chat / Agent Runtime 对应参数恢复活动会话，并在会话 focus、创建、删除和服务端恢复后回写 URL；`ReactManagedTerminalRouteBody.tsx` 对 Terminal 采用同一策略。query 缺失或目标会话不存在时，运行页才回退到 `sessionStorage` 快照与服务端列表默认项。
...
- `internal/agent/application/builtin.go` 中的 `travel` 内置 Agent 默认声明 `memory`、`deploy-test-service`、`frontend-design` 与 `artifact-preview` 四个 Skill；Agent Catalog 会把该列表写入运行时 metadata 的 `alter0.skills.include`，新建 Travel Agent 会话直接携带发布、前端设计与静态产物预览规则，用户仍可通过会话级 Skill 面板调整后续消息的公有 Skill 选择。

...
- `ScrollJumpStrip.test.tsx`、`LegacyWebShell.test.tsx` 与 `shellLayoutStyles.test.ts` 共同覆盖共享四键阅读定位条的目标计算、托管 route 页锚点分配、样式可见性和 Chat 消息区集成；`internal/interfaces/web/e2e/chat.spec.ts` 继续验证非 Terminal 托管 route 页的真实浏览器跳转行为。

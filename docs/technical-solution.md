...
- `ConversationWorkspace.tsx` 在 Chat 时间线滚动容器上监听 `scroll`，并在 workspace body 上监听原生 `touchstart / touchmove`：普通滚动到顶时先展开本地隐藏批次，已完全展开后调用 `refreshActiveSession()` 回源当前 Chat 会话详情；移动端触摸手势会在整个正文工作区内记录，若 `touchstart` 时已经在顶部，或同一手势先把滚动容器拉到顶部后继续下拉超过固定阈值，都会触发同一补拉链路，避免 `scrollTop` 已经为 0 时浏览器不再派发有效滚动事件，也避免真机手势落在 workspace body 而非内部 screen 节点时丢失。Composer、移动端头部、详情浮层和会话抽屉区域被排除在补拉手势之外；手动补拉由 in-flight guard 限制为同一时间只有一个请求，返回后按既有消息合并规则保留当前已加载历史。

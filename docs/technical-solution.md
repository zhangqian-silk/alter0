...
- `runtimeKeyboardIsolation.css` 让 workspace grid 的中间行成为唯一正文高度来源，`.runtime-workspace-panel` 不再按 Composer 高度二次计算；`.runtime-workspace-screen` 保持固定 20px 阅读留白，不写入键盘高度、Composer 高度或额外滚动范围。Terminal 的 `.terminal-jump-cluster` 在输入框聚焦且软键盘弹起时由 `ReactManagedTerminalRouteBody.tsx` 直接隐藏，键盘收起后再恢复到输入区上沿之上。
...
- `ConversationWorkspace.tsx` 与 `ReactManagedTerminalRouteBody.tsx` 的主输入框不在移动端 `pointerdown / touchstart` 捕获阶段取消默认行为，首次触摸输入框仍走浏览器原生软键盘手势；运行页不再对首触输入框调用 `focus({ preventScroll: true })`，也不安装 page scroll 锁或 workspace 背景滚动锁。两条运行页仍保留 `focus({ preventScroll: true })` 作为 slash command 补全、创建新会话后回到 Composer 等明确动作后的输入恢复能力。

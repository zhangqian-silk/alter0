...
- `internal/interfaces/web/frontend/src/shared/viewport/mobileViewport.ts` 负责移动端断点、键盘偏移阈值、composer 贴底偏移与 viewport baseline 计算，避免 Chat、Terminal 与 route 页重复维护软键盘占位逻辑。初始同步或页面刷新时若没有输入框聚焦，状态机以 layout viewport 作为 baseline，不把短暂偏小的 visual viewport 报告升级为键盘态；已有 baseline 且 visual viewport 明确键盘收缩时，仍保留键盘诊断偏移。

...
- `internal/interfaces/web/frontend/src/shared/viewport/mobileViewport.ts` 负责移动端断点、键盘偏移阈值、composer 贴底偏移与 viewport baseline 计算。输入框聚焦且 `VisualViewport.height` 相对基线开始收缩时，状态机从同步阈值起写入 `--keyboard-offset`，避免 App Shell 在键盘开场阶段先消费缩短后的视口高度再回弹；Chat、Terminal 与 route 页不重复维护软键盘占位逻辑。
...
- 移动端 App Shell 与 workbench 高度链使用 `--mobile-viewport-height + --keyboard-offset` 的稳定基线表达，由共享 `mobileViewportSync` 同步 `VisualViewport` 计算值；`root.css` 在窄屏下把 `body / #frontend-root` 固定到视口，`shell.css` 同步把 `.app-shell` 固定到视口，键盘动画期间主工作区容器不直接消费实时 `--mobile-viewport-height` 改变高度，避免浏览器工具栏变化、输入聚焦自动平移或软键盘帧更新导致底部留白、内容裁切或可见位移动画。
...
- `WorkbenchApp` 在根壳层安装共享 `mobileViewportSync` controller，把 `VisualViewport` 变化稳定写入 `--mobile-viewport-height / --keyboard-offset / --keyboard-composer-offset`；移动端 `root.css` 在 `1100px` 及以下锁住 `html / body / #frontend-root` 的页面级滚动，并把 `body / #frontend-root` 设为 `position: fixed; inset: 0`；`shell.css` 在同一断点把 `.app-shell` 设为 `position: fixed; inset: 0`。App Shell 与 `workbench-main` 在键盘弹起期间使用 `--mobile-viewport-height + --keyboard-offset` 保持基线高度，避免整个 workbench 被 `visualViewport` 收缩或输入聚焦自动平移带着上移；`useRuntimeComposerViewportSync` 在 Composer 聚焦期间监听 `window.scroll`、`VisualViewport resize/scroll` 并做短延迟复核，当移动端浏览器改写页面级滚动、运行页祖先容器或 workspace 内部滚动容器时立即锚回聚焦前位置，防止工作台被键盘动画整体顶起；Chat / Terminal 移动端 `.runtime-composer-form` 通过 `bottom: var(--keyboard-composer-offset, var(--keyboard-offset))` 贴住可见底边，且 `mobileViewportSync` 会对键盘动画中的临时 `offsetTop` 做短窗口保持与自动复核。`shell.css` 只允许 Composer form 消费 `--keyboard-composer-offset`；`runtime-composer-shell`、`runtime-workspace-screen`、命令候选、配置面板和阅读定位控件保持静态位置，也不直接用实时 `--mobile-viewport-height` 改变主工作区高度。

...
- `internal/interfaces/web/frontend/src/shared/visibility/usePageActivation.ts` 是 Chat / Terminal 的共享页面激活补偿入口，监听 `focus`、`visibilitychange`、`pageshow` 与 `online`，在页面可见时刷新当前 owner 列表并触发 owner 增量轮询，在 `resync_required`、缓存不完整或 user-only 待恢复时触发必要快照回源，通过短 debounce 合并同一恢复阶段的重复事件。
...
- `internal/interfaces/web/frontend/src/features/conversation-runtime` 承载共享对话运行态 UI：`ConversationRuntimeProvider.tsx` 负责复用 Terminal session API 的数据模型和状态机，按 route 通过 `/api/chat/sessions` 或 `/api/terminal/sessions` 完成会话创建/切换/删除/置顶、input 输入提交、结果收口、文本草稿与附件草稿恢复、模型与能力项选择。`RuntimeRouteHost.tsx` 将 `/chat` 与 `/terminal` 都挂载到 `ConversationRuntimeProvider + ConversationWorkspace`，但通过 `key={route}` 和真实 `route` 保证 controller、API owner、active session、缓存和草稿命名空间彼此独立；`/terminal` 不再挂载 `ReactManagedTerminalRouteBody`，共享 Workspace 在 Terminal route 下仍输出 `data-runtime-view/workspace/screen="terminal"`、`data-composer-input="terminal"`、`data-runtime-create-session="terminal"` 与 `data-runtime-session-select` 等稳定别名，供壳层、测试和兼容自动化识别当前 owner。`ConversationWorkspace.tsx` 不再渲染 Skill 选择器、Deliverables、Session Profile 或独立 Skill 面板；历史会话的 `target_type=runtime` 仅作为会话摘要元数据保留。Chat 仍在常规模型 Provider 列表外额外注入前端内置 `Codex` provider，未持久化其他模型选择时默认选中 Codex；模型/Provider、Tools/MCP、Skills 与过程披露过滤按 route 写入独立 `localStorage` 配置，发送时随当前 owner input payload 提交有效 `model_provider_id / model_id / tool_ids / mcp_ids / skill_ids`。运行页会话列表与消息详情通过当前 route 的 session 接口恢复，浏览器继续保存当前 route 的未发送草稿、附件草稿和运行配置；Chat / Terminal / Settings 路由切换、会话切换和 New 创建不再调用 unsaved draft confirm，草稿保留由 route 与 session 维度的缓存承担；Chat owner 与 Terminal owner 的会话列表互不包含。`RuntimeSessionController` 将集合响应标记为 `summary`，将单会话详情、input 响应和 pin 响应标记为 `detail`，owner updates 标记为 `event`；`ConversationRuntimeProvider.normalizeRuntimeSession` 只有在 `detail/event` 携带 turns 时才更新 messages、`messagesLoaded` 和 `detailRevision`，summary 只更新标题、状态、置顶、配置、`activityAt` 与 `revision`。忙碌或可恢复会话优先通过 owner updates 合并增量事件；Provider 将 poll 结果区分为 `appliedAny` 与 `appliedRecoverable`，只有命中本轮 `busy / recoverable` session 的事件才算当前会话有进展。若 updates 返回空事件、事件页只包含其他会话的旧 backlog，或当前待恢复会话的 revision/activity/messages/process steps 均未推进，Provider 会先累计连续无进展次数，只在第 10、20、50 次以及之后每 50 次对这些待恢复 session 执行强制 `hydrateRuntimeSession(..., { force: true })`，只拉最新 bounded detail 并按同一 merge 规则补正文与终态，不触发更早历史分页；当前会话收到新的 busy revision、activity、消息或过程步骤时会重置退避计数，详情仍未收敛为稳定态时不重置退避计数。单会话详情若携带 `turns_paging.has_more_before`，Provider 会先按消息 id 合并当前分页片段，并把 `next_before_turn_id / oldest_turn_id` 暴露给 Workspace；只有 `ConversationWorkspace` 因用户点击或触顶调用 `loadEarlierHistory()` 时，Provider 才请求 `turn_before` 下一页。无重叠片段按消息时间判断前置或追加，避免 page-activation、轮询或输入返回的短 turns 页覆盖已加载历史。Provider 将时间线/会话 workspace 上下文与 Composer 上下文分离，显式 `turn_before` 补页只更新消息集合；Composer 上下文只消费稳定草稿、附件和运行配置字段，避免未变化的输入区随历史合并刷新。Provider 的模块级运行态缓存保留完整已加载消息并按 24 小时 TTL 失效；完整会话消息快照、轻量会话信息快照、active session、文本草稿、附件草稿、运行配置与过程披露过滤均使用 route 独立 `sessionStorage / localStorage` key，旧合并快照只作为读取迁移 fallback。`hydrateRuntimeSession` 在无 `turn_before` 的普通详情恢复中先检查当前 route 缓存，只有 `serverBacked=true`、`messagesLoaded=true`、`revision <= detailRevision` 且没有强制刷新要求时，才直接返回缓存会话并跳过重复详情请求；显式分页补历史、`resync_required`、用户手动刷新、连续增量无相关进展达到退避阈值的 busy 兜底和列表 revision 新于详情缓存时继续回源。
...
- `internal/terminal/application.Service` 的 `Create` 与 `Recover` 只在全局 `sessions` map 读写期间持有服务锁；新建或恢复的 `runtimeSession` 放入 map 后立即释放锁，再执行 `persistSession`、session update hook 与 bounded detail 构建。update hook 可通过 `Get / ListTurns` 回读同一会话以发布 `session.created / session.updated`，但不得与创建/恢复请求形成锁重入。
- `internal/terminal/application.Service.List` 在读取内存 map 前调用 `syncMissingPersistedSessions`，扫描当前 state directory 中内存缺失的 JSON session，并按恢复逻辑补入 map；该同步不覆盖已有内存 session，避免轮询列表把正在运行的 turn 回退到旧文件快照。列表同步与创建/恢复一样在持久化事件发布前释放全局锁，迁移旧记录时只对补入的缺失会话写回。
...
- `ConversationRuntimeProvider.tsx` 的发送链路始终确保存在当前 route owner 的 runtime session，再调用对应 owner 的 input 接口，并用返回的 session、owner 增量轮询或后续详情恢复生成本轮 user / assistant 消息；前端不再创建本地 running process step，也不再使用旧 `/api/messages/stream` 读流协议。发送前只拦截真实运行中的同会话输入，已失败或中断的历史 turn 即使没有 assistant 正文也不会阻断新的 input。收到 input 结果、增量事件、错误或恢复详情后同步收口最终状态，迟到结果不得重新打开已完成消息或覆盖最终正文。
...
- `internal/interfaces/web/server_terminal.go` 的 session collection、item、pin、delete 与 input handlers 由 Chat owner 和独立 Terminal owner 复用；Chat 请求必须走 `/api/chat/sessions` 并落到 `chat` owner，Terminal 请求走 `/api/terminal/sessions` 并落到 `terminal` owner。session payload 的 `pinned` 字段作为前端恢复契约始终输出明确布尔值，取消置顶响应保留 `pinned:false`，避免浏览器本地旧快照把缺字段合并回置顶态。collection handler 通过 `buildTerminalSessionSummary` 输出轻量 summary，删除 `turns / turns_paging` 并补齐 `activity_at` 与微秒级 `revision`；item/input/pin 详情通过 `buildTerminalSessionDetail` 补齐同一可比较字段后再分页返回 turns。当前 Web Shell 的 `/terminal` path 使用共享 Conversation UI，但仍触发 Terminal owner 前端请求。前端将当前 owner 的 `turn.prompt` 映射为用户消息，将 `turn.final_output` 映射为 assistant 正文，将 `turn.runtime_trace_events` 映射为运行过程。

### 实时更新通道

`Chat / Terminal` 会话更新采用 owner 级增量轮询，替代旧长连接与固定高频详情轮询。HTTP 快照仍是最终恢复入口，updates 只承载低成本增量：

```text
POST /api/{chat|terminal}/sessions/{session_id}/input
  -> Terminal session store 持久化 user turn + busy summary
  -> SessionUpdateLog append session.updated / turn.started
  -> POST /api/{chat|terminal}/sessions/updates
...
- 会话标题升级、空白会话唯一性、历史折叠和页面滚动状态属于 Conversation 子域。`internal/terminal/application/session_title.go` 通过评分区分占位标题、低辨识开场、具体任务标题和补充约束：`New`、短 hash 或“拉取仓库 / 分析仓库”等 bootstrap 标题可被后续更具体任务覆盖；已形成的主题型标题不会被“而不是 / 不要 / instead of / rather than”等约束型 prompt 覆盖。持久化恢复时，`store.go` 会对旧版本已写坏的自动标题做一次轻量纠偏，从 turns 中选择最早有效主题 prompt 回写；手动标题保持不变。
...
- `shared/visibility/usePageActivation.ts` 提供运行页共享的 page-activation hook：统一监听 `visibilitychange`、`focus`、`pageshow` 与 `online`，并用短时间去抖吸收同一恢复阶段的连续触发。`ConversationRuntimeProvider.tsx` 与 `ReactManagedTerminalRouteBody.tsx` 都复用这条链路；Conversation 在页面重新变为前台可见时刷新当前 owner 会话列表，并对当前 active server session 强制补拉一次不带 `turn_before` 的最新 bounded detail，用于校准最终正文、状态和 `runtime_trace_events`，随后让下一轮 updates 按 cursor 补齐增量。恢复流程、`resync_required`、用户显式刷新和详情打开都会绕过稳定本地缓存短路；历史分页仍只在用户显式加载更早消息时请求。页面隐藏时停止高频轮询，Terminal 兼容组件则继续按自身状态刷新会话列表与当前活动会话详情。
...
- Web handler 测试覆盖 `/api/chat/sessions/updates` 与 `/api/terminal/sessions/updates`：owner 隔离、`since_event_id` 续接、`limit / byte_limit` 传输预算、ack manifest 裁剪已知 runtime steps、事件窗口过期时返回 `resync_required`、HTTP input 断开不取消后端执行。
- Terminal application 测试覆盖 input 接受后先持久化 `busy` session 与 user turn，再发布 `session.updated / turn.started`；Codex event、完成、失败和中断分别生成幂等 revision 事件；服务重启或运行进程丢失后残留的孤儿 `busy / running` 会话必须在列表/详情读取和继续输入前校准为 `interrupted`，并持久化中断 entry 与 `Interrupted` runtime event。
...
- Terminal session store 的恢复和读路径共同维护运行态事实：持久化状态加载时会把旧 `running` turn 收敛为 `interrupted`；服务进程存活但 CLI worker 丢失时，`List / Get / ListTurns / ListEntries / InputWithAttachments` 在读取或 busy 检查前调用同一中断收口逻辑，追加系统 entry 与 `Interrupted` runtime event 后写回 session JSON。`turnRunning=true` 或仍有 `turnCancel` 的当前进程内运行请求不会被该懒校准处理。
...
- Skill 与 MCP 专用接口需要复用统一 Capability 数据结构；Capability 审计记录生命周期动作，供控制面按类型查询。`internal/control/application.NewServiceWithStore` 在加载持久化 control store 时只接纳当前支持的 `skill` 与 `mcp` capability，遇到旧版本遗留的其他 capability 类型时跳过该条记录并继续初始化；`UpsertCapability` 仍复用领域校验，禁止新写入非支持类型。

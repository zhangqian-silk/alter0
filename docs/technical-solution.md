...
- systemd 基线显式配置 `ALTER0_RUNTIME_ROOT`，并由 `cmd/alter0` 派生 `<runtime_root>/storage`、`workspaces`、`state`、`logs` 与 `output`；`ALTER0_STORAGE_DIR` 仅作为旧配置兼容入口，不能与 runtime root 指向不同存储树。
- 提交签名问题不得通过关闭签名绕过。

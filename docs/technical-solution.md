...
- `features/shell/components/useRuntimeComposerViewportSync.ts` 在移动端为共享 Runtime Composer 主输入框注册 `pointerdown / touchstart` 捕获监听；监听只在目标命中 `data-runtime-composer-input` 时记录页面级滚动、运行页祖先容器和正文滚动容器的当前位置，不取消默认事件、不抢先调用 `focus()`。真实 `focus` 建立后，后续 `window.scroll`、`VisualViewport resize/scroll` 与短延迟复核复用该预捕获锚点，优先抵消移动端浏览器对 textarea 的自动 `scrollIntoView`，避免工作台先被顶起再回弹。
...
- `ConversationWorkspace.tsx` 在 Chat 时间线滚动容器上监听 `scroll`，并在 workspace body 与 `window` capture 链路上监听原生 `touchstart / touchmove`：普通滚动到顶时先展开本地隐藏批次，已完全展开后调用 `refreshActiveSession()` 回源当前 Chat 会话详情；移动端触摸手势会按触点坐标确认发生在消息区内，若 `touchstart` 时已经在顶部，或同一手势先把滚动容器拉到顶部后继续下拉超过固定阈值，都会触发同一补拉链路，避免 `scrollTop` 已经为 0 时浏览器不再派发有效滚动事件，也避免 iOS Safari 弹性下拉只出现在全局触摸链路或真机手势落在 workspace body 而非内部 screen 节点时丢失。Composer、移动端头部、详情浮层和会话抽屉区域被排除在补拉手势之外；手动补拉由 in-flight guard 限制为同一时间只有一个请求，返回后按既有消息合并规则保留当前已加载历史。

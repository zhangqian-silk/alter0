...
- `internal/interfaces/web/frontend/src/features/shell` 继续维护主导航、共享 copy、React 管理页和 route surface。`components/PrimaryNav.tsx` 负责路由高亮、导航折叠、tooltip 与语言切换；`components/ReactManagedRouteBody.tsx` 负责把 `agent / terminal / memory / channels / skills / mcp / models / environments / cron-jobs / sessions / tasks / codex-accounts` 分派到各自 React 页面。`RuntimeWorkspaceHeader.tsx` 负责共享 `Chat / Agent Runtime / Terminal` 的固定 workspace header，只保留会话标题、状态按钮和 `Details` 入口，并把差异内容交给各页传入。`RuntimeWorkspaceShell.tsx` 在窄屏下把运行页顶部操作收口为单一 `Workspace actions` 四宫格入口，面板内集中承载 `Menu / Sessions / New`；`ReactManagedTerminalRouteBody.tsx` 继续保持旧版 `terminal-*` DOM class 契约作为布局皮肤基线，但会话列表、工作区容器、工作区头部与窄屏顶部操作行额外复用 `ConversationWorkspace` 的工作台语义类，确保 Terminal 与 Chat / Agent Runtime 使用同一套表面和头部节奏；Terminal 的会话抽屉列表与 Conversation 一样输出 `role="list"` 语义、当前态 badge、状态徽标、最近输出、短标识与尾侧删除动作，详情面板则先复用共享紧凑摘要栅格，再由 Terminal 自己补充会话字段和公有 Skill 选择区；Terminal 的会话列表、详情轮询、step 展开、列表删除、Skill 选择与 Markdown 输出仍全部由 React state 直接维护；控制页、Sessions、Tasks 与 Memory 继续复用统一客户端和共享 surface 样式。
...
- `ConversationWorkspace.tsx` 与 `ReactManagedTerminalRouteBody.tsx` 不会把 `--keyboard-offset` 透传给其他运行区控件：移动端 `Workspace actions` 入口、紧凑 workspace header 和 Terminal 右侧四键定位条继续留在静态工作区流里，键盘开合期间只有 fixed Composer 自身跟随 `VisualViewport` 贴底移动；Composer 配置面板在 `760px` 及以下改为独立 fixed 浮层并限制最大宽高，避免技能、模型或工具面板重新挤压正文与输入区。
...
- `ConversationWorkspace.tsx` 与 `ReactManagedTerminalRouteBody.tsx` 都已经退化为 controller + route wrapper：统一由 `RuntimeWorkspacePage.tsx` 产出紧凑工作区头部、会话列、时间线、`Details` 面板与 Composer，页面只负责注入会话数据、变体 class 和路由专属交互；`RuntimeComposer.tsx` 现在固定输出 `runtime-composer-body + runtime-composer-toolbar` 两层结构，工具栏左侧收口为单一 `Session` 会话设置入口与附件按钮，运行态配置通过面板内部 tab 切换，右侧只保留提交动作，并通过共享 padding / min-height 令牌保证主输入区拥有稳定可读宽度；`PrimaryNav.tsx` 维护品牌图标位与语言切换，左下角不再额外挂接账号信息区。Conversation 继续保留 `data-conversation-*` 钩子，Terminal 继续保留 `terminal-*` 钩子与布局变体，但公共 DOM 主契约已经收敛到 `runtime-workspace-* / runtime-composer-* / runtime-timeline-*`。

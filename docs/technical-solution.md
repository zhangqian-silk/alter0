...
- `internal/interfaces/web/frontend/src/shared/viewport/mobileViewport.ts` 负责移动端断点、键盘偏移阈值、composer 贴底偏移与 viewport baseline 计算。输入框聚焦且 `VisualViewport.height` 相对基线开始收缩时，状态机从同步阈值起写入 `--keyboard-offset`，避免 App Shell 在键盘开场阶段先消费缩短后的视口高度再回弹；Chat、Terminal 与 route 页不重复维护软键盘占位逻辑。移动端键盘稳定性主要由 viewport 状态机和 `shell.css` 末尾静态层共同收口，不额外引入根壳层固定定位或强制平移逻辑。
...
- `ConversationWorkspace.tsx` 与 `ReactManagedTerminalRouteBody.tsx` 不会把 `--keyboard-offset` 透传给其他运行区控件：移动端顶部操作行与紧凑 workspace header 继续留在静态工作区流里，键盘开合期间只有 `.runtime-composer-form` 跟随 `--keyboard-composer-offset` 贴底移动，`.runtime-composer-shell` 保持静态外壳位置；阅读定位条在主输入框聚焦期间直接停止渲染，待输入框失焦后再恢复；Composer 配置面板在 `760px` 及以下改为独立 fixed 浮层并限制最大宽高，避免技能、模型或工具面板重新挤压正文与输入区。
...
- `shared/viewport/mobileViewport.ts` 对 `VisualViewport.height` 与 `offsetTop` 分开判定：输入框聚焦期间只要回报的原始 `height` 相对 `baselineHeight` 达到同步阈值，状态机就优先使用原始 `height` 计算 `--keyboard-offset`；上一轮键盘占位仍在释放中时继续沿用完整键盘阈值识别真实收缩。`--keyboard-composer-offset` 通常按 `keyboardOffset - offsetTop` 下限为 0 输出，但在最近一次有效键盘对齐后的 `MOBILE_VIEWPORT_COMPOSER_OFFSET_HOLD_MS` 短窗口内会暂时忽略正 `offsetTop`，避免 iOS 键盘抬起动画里 Composer 突然下跳；`mobileViewportSync` 会在该保持窗口结束后补做一次同步，让稳定 `offsetTop` 场景继续扣除偏移，避免部分移动浏览器已平移可视窗口后 Composer 再次上移。
...
- 移动端 App Shell 与 workbench 高度链使用 `--mobile-viewport-height + --keyboard-offset` 的稳定基线表达，由共享 `mobileViewportSync` 同步 `VisualViewport` 计算值；键盘动画期间主工作区容器不直接消费实时 `--mobile-viewport-height` 改变高度，避免浏览器工具栏变化或软键盘帧更新导致底部留白、内容裁切或可见位移动画。
...
- `WorkbenchApp` 在根壳层安装共享 `mobileViewportSync` controller，把 `VisualViewport` 变化稳定写入 `--mobile-viewport-height / --keyboard-offset / --keyboard-composer-offset`；移动端 `root.css` 在 `1100px` 及以下锁住 `html / body / #frontend-root` 的页面级滚动，App Shell 与 `workbench-main` 在键盘弹起期间使用 `--mobile-viewport-height + --keyboard-offset` 保持基线高度，避免整个 workbench 被 `visualViewport` 收缩带着上移；Chat / Terminal 移动端 `.runtime-composer-form` 通过 `bottom: var(--keyboard-composer-offset, var(--keyboard-offset))` 贴住可见底边，且 `mobileViewportSync` 会对键盘动画中的临时 `offsetTop` 做短窗口保持与自动复核。`shell.css` 只允许 Composer form 消费 `--keyboard-composer-offset`；`runtime-composer-shell`、`runtime-workspace-screen`、命令候选、配置面板和阅读定位控件保持静态位置，也不直接用实时 `--mobile-viewport-height` 改变主工作区高度；`useRuntimeComposerViewportSync` 不再对 `window.scroll` 或 workspace scroll 容器做聚焦期锚回，末尾移动端静态层负责把键盘路径上的表面和状态信号去动效化，防止最近视觉层样式覆盖键盘稳定契约。

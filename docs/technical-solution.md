...
2026-07-10 更新：Chat 同步按移动端个人单设备场景收敛。共享 runtime controller 是唯一会话状态提交入口，请求以代次拒绝迟到结果，消息合并前先用内容 `updated_at` 判旧；input 通过 `client_request_id` 对账 optimistic user turn，turn/event/update id 均使用 canonical 字符串。列表只返回六个展示/判鲜字段，详情由应用层原子快照生成，所有会话 API 使用 `Cache-Control: no-store`。页面隐藏时停止轮询；不足 5 分钟回前台只恢复本机未完成 updates，达到 5 分钟、首次进入或手动刷新才拉轻量列表与当前详情。长期缓存保留全部 summary，但只保留当前加最近四个内容快照，并排除 data URL 与 event detail blocks。本段规则替代下文早期“effect 重新合并 ref/state”“长期保存 event detail blocks”以及“成功空列表仍保留服务端会话”的旧描述。
...
- Claude Code 运行前准备 provider profile 与必要上下文；Codex Direct 使用共享活动 `CODEX_HOME`，从用户级目录发现 Skills、读取全局 Memories 配置并直接使用仓库自身 `AGENTS.md`。
- `internal/execution/domain` 使用 `alter0.runtime.thread_title` 作为 CLI Runtime 外部 thread title 的统一元数据键。Codex JSONL 事件解析 `title / thread_title / conversation_title / name` 以及 `thread/session/conversation.title` 等字段并写入该 metadata；Claude Code 在不改变现有 text 输出模式的前提下，从 stdout 中可观测到的结构化 title 行提取同一 metadata，并把该元数据行从最终文本输出中过滤。`internal/execution/application.Service` 将该 metadata 透传到 `ExecutionResult.Metadata`，供上层会话标题同步或审计使用。
- 运行过程统一归一为轻量 `RuntimeTraceEvent`：turn 摘要与 updates payload 只保留 `id / kind / status / text / detail_available / created_at / completed_at / duration_ms`。`kind` 枚举固定为 `important_text / reasoning / plan / tools / commands / system`，不通过自然语言正文、标题、关键词或语言模式推断；结构化 `blocks` 只在 event detail 接口返回，`seq / session_id / turn_id / provider / source / role / lifecycle / title / summary / visibility / raw / action` 不进入轻量协议。
...
- Chat session id 由 `internal/chatruntime/application.Service.Create` 在后端生成，格式为 `c_` 加 16 位小写字母数字，例如 `c_x8k4p9m2q7vd3n6a`。该 id 是唯一 canonical id，同时用于 URL query、列表摘要、详情接口、updates payload、状态文件与工作区路径；前端不再生成临时会话引用，也不再通过会话列表做引用反查。旧 `alter0-chat`、`chat-*` 长 id、前端短 hash 与旧 sessionStorage snapshot key 不迁移，启动时直接丢弃或清理。
...
- `internal/interfaces/web/frontend/src/shared/time/sessionListGroups.ts` 负责把Chat 会话列表按 `Pinned / Expiring Soon / Today / Yesterday / Earlier` 分组；调用方通过 `getPinned` 把置顶会话抽到独立首组，其余会话传入内容 `updatedAt` 作为分组时间，缺少更新时间时才回退到创建时间。分组 helper 内固定复用当前维护策略的 7 天不活跃清理阈值，并把距离阈值还剩不超过 2 天、且尚未超过阈值的未置顶会话归入 `Expiring Soon / 即将清理`，避免 Chat 漂移成不同的分组策略。
...
- `internal/interfaces/web/frontend/src/shared/visibility/usePageActivation.ts` 是 Chat 的共享页面激活入口，监听 `focus`、`visibilitychange`、`pageshow` 与 `online`。Provider 记录隐藏起点并通过 `conversationSyncPolicy.ts` 计算恢复动作：不足 5 分钟时稳定会话零请求，本机 optimistic journal 中未完成的会话恢复 updates；达到 5 分钟时刷新轻量全量列表和当前详情。入口使用短 debounce 合并同一恢复阶段的重复事件。
...
- `ConversationRuntimeProvider.tsx` 的会话状态提交入口统一通过 `setSessionsByRoute` 与 `setActiveSessionByRoute` 先写 `conversationRuntimeCache`、route 长期会话快照和轻量信息快照，再调用 React state 更新；`upsertRuntimeSession` 也必须走 `mergeRuntimeSessions`，不得以迟到 summary 直接替换本地 busy 会话。`sendPrompt` 在 input 请求未完成前追加本地 queued user 消息并置 busy；若发送前没有真实 active session，先创建不带 `title` 的 ChatRuntime session，再把 input 提交给该 session。前端不从 prompt 或 queued user 消息派生标题，当前标题只来自底层 ChatRuntime summary/detail/update 的合并结果。`normalizeRuntimeSession / mergeRuntimeSessions / mergePagedMessages` 继续按消息 id 合并服务端 turns，服务端返回对应 turn 后压缩同文本 queued user 消息并补入 assistant 结果，避免短 turns 页或旧列表摘要覆盖本地已追加时间线。`mergeRuntimeSessionTurnPagingPayload` 使用合并后的本地消息集校准 `turnsPaging`：若本地已确认 `has_more_before=false`，后续最新 bounded 页即使携带 `has_more_before=true`，只要页边界 turn 已存在于本地合并结果，就保持本地分页耗尽并清理 `next_before_turn_id`。消息排序使用稳定比较器：同一 turn 固定 user 在 assistant/Thinking 前，本地 queued user 在首个 busy assistant patch 前，不只按时间戳排序；会话列表排序使用内容 `updatedAt` 与本地已加载消息时间，置顶仍优先于更新时间；pin、配置、查看详情、标题管理不推进服务端 `updated_at`，也不影响普通时间排序。缓存恢复也走同一排序，修正旧快照中的倒序消息。Provider 关闭 `useRuntimeSessionController` 的自动 progressive history，避免稳定会话在后台 `turn_before` 补页后直接刷新 active session。Provider 的缓存写入 effect 必须先把 runtime controller state 与 `sessionsByRouteRef` 通过同一 merge reducer 合并，再写入完整消息快照和轻量信息快照，避免已加载过程 detail 被旧 controller 快照覆盖。
...
- `ConversationRuntimeProvider.tsx` 接受 `chat` conversation route：读取本地活动会话、当前 route 快照、服务端集合、单会话详情和用户手动 focus 时都会使用当前 route owner 的会话模型；`session_id` query 直接保存后端 canonical id，不做二次解析或迁移。`session_id` query 只作为显式恢复输入和用户手动 focus 的输出；当 `/chat` 没有显式 `session_id` 时，Provider 以当前 owner 的服务端集合与本地 canonical 快照合并后的第一条会话作为当前会话，不让其他 route 的活动会话覆盖最新入口。会话列表在前端按置顶优先、内容 `updatedAt` 时间排序与分组，缺少更新时间时回退到创建时间；对尚未进入 Session history 的空白本地会话，pin 请求失败时仍更新本地会话快照，保证当前工作区即时反馈不被 history 缺口吞掉。
...
- `internal/interfaces/web/maintenance.go` 只注册 Session Cleanup，并在启动时通过 Scheduler 显式退役历史 `system-memory-maintenance` 内置任务；会话清理继续移除超过 7 天不活跃且未置顶的会话及其任务、Runtime 和工作区数据。
- `ConversationRuntimeProvider.tsx` 的恢复判定识别本地正在同步的 `local_running / recovering` 会话；恢复流程在要求稳定 assistant 时必须等到详情接口返回非占位 assistant 或失败态后才 upsert。Provider 将自动轮询条件与输入锁条件拆分：`shouldPollRuntimeBackedSession` 只接受前端本机产生的 `local_running / recovering` 同步意图，普通服务端 `busy / running` 摘要不触发后台 updates/detail 轮询；`shouldBlockRuntimeInput` 只检查会话或消息是否仍为 `busy / running / queued / in_progress / local_running / recovering`，不把用于详情补偿的 recoverable error/历史未回答 user 判定复用为输入锁。失败、中断或退出终态允许同一会话继续提交下一条输入；兼容历史 `ready` summary + failed turn 时直接以 turn/message 终态解锁，不使用 `updated_at > finished_at` 作为失败可重试的必要条件。ChatRuntime `finishTurn` 在请求失败时把 session 与 turn 同步标为 `failed` 并记录 `finished_at`，下一条 input 会清理失败摘要并重新进入 busy。
...
- `internal/chatruntime/application.Service` 的 `Create` 与 `Recover` 只在全局 `sessions` map 读写期间持有服务锁；新建或恢复的 `runtimeSession` 放入 map 后立即释放锁，再执行 `persistSession` 与 HTTP 响应所需 bounded detail 构建。运行态事件只通过 `SessionEventHook` 在输入接受、runtime step 追加/更新和 turn 收口时发布，不从持久化 snapshot 反推出增量。直连 Codex JSON 事件会解析外部 thread title 字段并写入 session title，状态文件额外保存 `title_external`；外部 title 更新发布 `session.updated`，后续 prompt 自动标题在 `title_external=true` 时不再覆盖标题，但新的外部 title 事件仍可继续覆盖。
- `internal/chatruntime/application.Service.List` 在读取内存 map 前调用 `syncMissingPersistedSessions`，扫描当前 state directory 中内存缺失且 id 符合 `c_[a-z0-9]{16}` 的 JSON session，并按恢复逻辑补入 map；该同步不覆盖已有内存 session，避免轮询列表把正在运行的 turn 回退到旧文件快照。旧 id 状态文件不进入内存列表、不写回、不参与增量。
...
- 仓库准备发生在 Agent 启动前。准备成功后 ChatRuntime 把 `repo/` 设为 Codex 的 `cmd.Dir`，使仓库 `AGENTS.md` 原生生效；retry 只恢复同一失败 turn 和原用户输入，不新增 user 消息或 turn。
...
POST /api/chat/sessions/{session_id}/input
  ...
  -> CLI Runtime 执行并持续 publish turn.event.appended / turn.event.updated
  -> finishTurn publish turn.completed | turn.failed | turn.interrupted
  -> 客户端按 update_id / updated_at patch 本地 session
```

- `internal/chatruntime/application.Service` 通过 `SetSessionEventHook` 在输入接受、runtime step 追加/更新、turn 收口三个语义点发布 `SessionEvent`；Web 不再依赖持久化后的整会话 snapshot 推断运行态差异。Codex JSON 解析必须读取 `item.channel`：`agent_message` 的 `commentary` 分支创建 message 型 runtime event 并发布 `turn.event.appended`，`final` 或空频道只参与 `promoteFinalOutput`，其他非最终频道被忽略。Web `sessionUpdateBroker` 作为 owner 级派生 update 日志，和 session JSON 使用同一 owner/session 事实源。单条 update 包含 `update_id`、`type`、`session_id`、可选 `turn_id`、毫秒 `created_at` 和最小 payload；`update_id` 在当前 owner 的可续接窗口内单调递增，session 新鲜度只使用 `updated_at`。
- `finishTurn` 在通用 Codex 错误压缩前识别 `no rollout found for thread id`。命中后保留原 thread id 并把会话收敛为 `failed`，通过 `state/chat/history/<session-id>.md.tmp -> rename` 原子导出逐轮 `Prompt / Attachments / FinalOutput`，再把可直接复制到新会话的读取指令写入 `error_message`、system entry 与失败 runtime event；该分支不自动创建新 thread，也不把 command、reasoning 或 commentary 过程记录写入历史正文。
- 查询路径为 `/api/chat/sessions/updates`，使用 Chat owner；客户端必须以 `POST` 携带 `after_update_id`、`limit`、默认/最大 `1MiB` 的 `byte_limit` 与 `visible_event_kinds`。响应为 JSON envelope：`latest_update_id / resync_required / has_more / updates[]`。无变化时返回空 `updates` 与最新 `latest_update_id`，不返回会话列表或完整历史。`after_update_id` 为空时从当前 owner 可续接窗口的最新位置读取；非空时只返回该 update 之后的可见变更。当不可见事件被过滤或其他 owner 事件穿插时，当前响应中的可见 `update_id` 可以不连续。
- 事件 payload 严格控制体积：`session.created / session.updated / session.deleted` 只传会话摘要字段，包含标题、状态、置顶、更新时间和错误摘要，不携带 turns。`turn.started` 传 session 摘要与目标 turn patch；`turn.event.appended / updated` 传单个轻量 `runtime_event`；`turn.completed / failed / interrupted` 传当前 session 摘要和目标 turn 收口状态、毫秒时间戳、可用 `final_output`，不携带完整 runtime step 历史。Web 返回前根据 `visible_event_kinds` 裁剪 `runtime_event`，只保留当前可见的 runtime step；若某条 turn event 只包含被隐藏的 command/tool/system 等纯过程事件，服务端可跳过该事件但仍推进 `latest_update_id`。所有 updates payload 返回前移除 `owner_id / shell / working_dir / runtime_session_id` 等前端不消费运行元数据；`finished_at` 无值时在详情与 turn payload 中固定为 `null`。前端收到 `runtime_event` 后在本地映射为 message 级 partial step patch，按 `id` 合并 `processEvents`；同 turn 的详情快照或 turn 收口事件若未携带 runtime step，只补正文和状态，不清空本地已有 `processEvents`。附件原图、完整历史页、大段 thinking、结构化 `blocks` 和完整 event detail 不进入 updates。
- 当 `after_update_id` 早于服务端保留窗口、服务重启后无法证明事件连续、或 session `updated_at` 低于服务端摘要时，updates 返回 `resync_required:true`。客户端按当前 owner 的本地会话列表筛出 `local_running / recovering` runtime-backed 会话并逐个补拉详情，再用后续 updates 继续增量更新。新进程恢复持久化 session 时，孤儿 `running` turn 会被收敛为 `interrupted`；正在服务进程内存中但已经没有 live worker 的残留 `busy / running` 会话，也会在列表、详情、turns、entries 和继续输入前通过 `reconcileOrphanedRuntimeSession` 懒校准为 `interrupted` 并写回 store。补拉详情必须覆盖本地旧 `busy` 快照。
- `POST /input` 成功进入服务端执行链后，HTTP 请求断开只影响本次响应写回，不取消 runtime goroutine。input handler 在启动 runtime 前必须先持久化 busy session 与 user turn；应用层随后通过 `SessionEventHook` 发布 `turn.started`，该 typed event 同时携带 busy session 摘要与目标 turn patch。Web handler 不为 input 成功额外发布 `session.updated`，避免同一运行态变化在增量流中出现摘要事件与 typed turn event 两套表达。
- 前端在 `ConversationRuntimeProvider` 内维护 owner 级增量续接状态：按 route 保存 `latestUpdateIdByRoute`，页面可见且存在 `local_running / recovering` 会话时按短间隔请求 updates。请求把上次 `latest_update_id` 作为 `after_update_id`，并把当前过程披露过滤作为 `visible_event_kinds` 发送。收到事件后走同一 `mergeRuntimeSessions / mergePagedMessages` reducer，只 patch 命中的 session，不重建完整 session 列表或已稳定消息；`turn.started` 创建或更新当前 running turn，`turn.event.appended / updated` 以 `runtime_event` 单 step patch 合并 Thinking/process，`turn.completed / failed / interrupted` 只收口当前 turn 的 assistant 结果与状态。runtime event patch 必须以追加/覆盖单个 step 的方式进入本地缓存，保留已展开/折叠状态、已加载 step detail 和本地已有 step 顺序；事件级合并以 `id` 为稳定 key，若本地事件已经通过 detail 接口写入 `blocks`，后续摘要 patch 只更新 `status / text / completed_at / duration_ms` 等轻量字段，不覆盖 detail blocks，也不把该事件重新标记为待加载。可见 `update_id` 不连续时只按响应 `latest_update_id` 续接，不触发丢包判断。同一 owner 下非当前活动会话只更新摘要和必要最近 turn 状态，不主动拉取完整历史。updates 返回空 `updates`、只返回未命中本轮可恢复 session 的旧 backlog，或命中 session 但 `updated_at / messages / process steps` 均未推进时，前端累加连续无进展次数，updates 间隔从约 `2s` 起步并退避到约 `3s / 5s / 8s`；第 6 次连续无进展以及之后每 8 次，对仍处于 `local_running / recovering` 的 session 触发一次 bounded detail 兜底；命中会话出现新的 `updated_at`、消息或过程步骤时重置退避计数，详情成功但会话仍需继续轮询时保留退避计数，只有命中会话收敛到终态时才重置并停止自动 updates/detail。
- `mergeRuntimeSessions` 需要把本地 `local_running / recovering` 视为高优先级瞬时事实：当远端列表摘要缺少完整 turns、`updated_at` 未推进或仅返回旧 `ready` 状态时，不覆盖本地运行态；当远端详情明确带有稳定 assistant、失败、中断或删除结果时，再收口业务状态。该规则同时作用于 workspace header、移动端标题按钮和左侧会话列表，避免状态在列表刷新后回跳。
- 前端事件合并按 `update_id` 去重、按 `updated_at` 保序、按 `session_id + turn_id + runtime_event.id` 幂等写入。`turn.event.appended` 可直接追加结构化过程摘要；`turn.event.updated` 只能 patch 已存在事件或触发单 turn/detail 补拉；`turn.completed` 如果 payload 不含完整 final output，只更新状态并补拉当前会话详情，不把 preview 当最终正文。
...
- 前端首次进入与刷新恢复顺序固定为：读取本地快照渲染首屏；拉取 owner 轻量全量列表；成功列表对 server-backed membership 权威，失败或畸形响应不删除现有状态；再补拉当前 active 的最新 bounded detail。普通 page activation 按 5 分钟阈值执行，手动刷新固定同时刷新列表与当前详情。历史分页是 add-only，`before_turn_found=false` 视为无进展；updates 先丢弃不大于本地 cursor 的记录，再按响应 `latest_update_id` 续接。
- 轮询频率按可恢复状态驱动：页面可见且存在 `local_running / recovering` 会话时，按约 `2s` 起步只拉 owner updates，连续无进展后退避到约 `3s / 5s / 8s`；updates 未对本轮可恢复 session 产生相关进展时不立即回源详情，而是按连续无进展第 6 次以及之后每 8 次触发最新 bounded detail 兜底；新的 `updated_at`、消息或过程步骤会重置退避计数，仍未收敛且也没有新进展时继续保持退避计数；稳定完成且 `has_more_before=false` 的完整缓存会话不参与详情补偿，终态会话停止自动 updates/detail。
- 观测字段统一带上 `owner_id`、`session_id`、`turn_id`、`update_id`、`latest_update_id`、`updated_at`、`resync_required`、`poll_interval_ms` 与 `disconnect_reason`。事件窗口过期和补快照都以 info/warn 结构化日志记录，业务失败仍只由 turn/session 终态记录。
...
- `ConversationWorkspace.tsx` 必须把 Conversation 时间线装配结果按可见消息数组稳定 memoize，长历史初始只传入最新 32 条消息，顶部 `topContent` 渲染 `Load earlier messages / 加载更早消息` 控件，点击或滚到顶部后每次再扩展 32 条并按滚动高度差恢复阅读位置；本地隐藏历史已经展开且服务端仍有更早 turns 时，workspace 才调用 `ConversationRuntimeProvider.loadEarlierHistory()` 显式请求 `turn_before` 下一页。Workspace 记录上一帧完整消息 id 与可见消息 id；当历史分页只是在当前会话前方 prepend 旧消息时，当前渲染窗口继续使用上一帧可见消息，不因为全量缓存变长而刷新短会话正文区。只有用户触发加载更早消息时，`visibleCount` 才扩展到下一批历史。若新消息追加前当前可见窗口已经覆盖全部已加载消息，追加后 `visibleCount` 扩展到新的消息总数，避免发送后的本轮消息把原先可见历史挤出窗口。`ChatMessageRegion.tsx` / `buildChatTimelineItems` 进一步以 `session + language + callback + message id + message signature` 缓存单条 `RuntimeTimelineItem`，并用消息对象引用缓存签名计算，使结果 patch 只重建当前变化的 assistant 消息，不重复生成稳定历史消息的 Markdown HTML 与 Process 树；同一 assistant 消息含最终正文和过程事件时，timeline block 顺序固定为 prompt、`Thinking / 已思考` disclosure、最终 Markdown，保证过程入口位于最终回复上方且复制动作跟随最终回复。`RuntimeTimeline.tsx` 继续通过 `memo` 只在 `items / topContent / emptyState / overlay` 真正变化时重渲染；`RuntimeWorkspacePage.tsx` 也要把 session pane、workspace header 与 workspace content 这些重节点 memo 成稳定 ReactElement。仅有 Composer 草稿变化时，不重复解析 Markdown、不重建 `Process` 树，也不重跑整条消息时间线。
...
- `internal/interfaces/web/frontend/src/app/WorkbenchContext.tsx` 与 `WorkbenchApp.tsx` 统一维护移动端运行页面板状态：Chat 会话列表直接挂在主导航抽屉内，`Chat` 移动 workbar 都只暴露 `Menu` 抽屉入口；普通 `page-mode` 路由页新增的 `Menu` 入口也复用同一套状态切换与关闭路径，切路由、点遮罩或切会话时都通过同一条关闭链路收口，不再由各页面各自维护独立开关。`/settings` 直接进入或当前没有已注册会话 rail 时，`WorkbenchApp` 才额外挂载轻量 `ConversationRuntimeProvider + ConversationSessionRailBridge`，只注册主导航里的 Chat session rail，不渲染 Chat workspace；从 Chat 切入 Settings 且已有有效 rail 时不挂载该 bridge，不因页面切换触发 sessions 或 catalog 请求。该 bridge 复用 `/api/chat/sessions`、Chat session 分组和置顶/删除动作，并通过 rail version key 避免语义未变化时重复写入公共 rail 状态。
...
- React 托管入口集中在 `chat / settings`；Settings 切分 Runtime、Skills 与 Schedules，Memories 合并进 Runtime。
...
- `internal/execution/application.Service` 的活动主链只注入 MCP 与运行时事实；Codex Skills 和 Memories 由共享活动 Home 原生加载。
...
- 运行时执行器在运行期把工具动作与观察收敛为结构化 `RuntimeTraceEvent`；Chat 应用层在 turn summary 中输出轻量 `turns[].runtime_trace_events` 后透传给前端，事件详情通过 `session_id / turn_id / event_id` 单独读取，结构化 `blocks` 只在该详情接口返回。
...
- `chat.js` 内所有前端时间展示统一走同一北京时间格式化器，固定 `timeZone=Asia/Shanghai`、`hourCycle=h23`；时间标签输出 `HH:mm`，绝对时间输出 `YYYY-MM-DD HH:mm:ss`；控制台与账户管理视图的分钟精度时间戳输出 `YYYY-MM-DD HH:mm`，同样由共享时间格式器负责。Web API 的业务时间字段统一使用毫秒时间戳，`finished_at / completed_at` 等可空结束时间无值时返回 `null`。
...
- `ScrollJumpStrip.tsx` 负责 Chat 的四键阅读定位条。目标计算以滚动容器内可见消息块或 Chat turn 为单位缓存测量结果，并在滚动、窗口 resize、DOM 变更、展开折叠和 watch key 变化时重算。`上一条` 在当前最上方可见块尚未对齐时先把该块对齐到顶部偏移；若该块已处在目标偏移位置，下一次点击直接指向它前一块，保证连续上跳不会卡在同一内容块。若视口落在两个测量锚点之间而没有锚点相交，则以 `entry.bottom <= scrollTop` 反向查找上方最近目标，并以 `entry.top >= viewportBottom` 正向查找下方最近目标，确保长 assistant 回复中间仍保留有效的相邻跳转。
...
- 传输预算测试覆盖 updates payload 不包含完整历史 turns、大段 event detail、附件原图、结构化 `blocks` 或超大 Markdown 正文；这些内容继续通过详情或懒加载接口读取。
...
- 前端路由、Runtime、Skills、Schedules、Memories 控件与响应式布局由 React/Vitest 契约覆盖；Go Web 测试继续校验 API、嵌入资产与静态分发策略。
...
```text
Settings > Runtime
  -> PUT /api/control/codex/runtime
  -> Codex app-server config/batchWrite
  -> active CODEX_HOME/config.toml
  -> next Codex exec/resume reads native settings
...
- Runtime PUT 使用结构化 `memories` 对象写回三个原生键；总开关关闭不删除 `CODEX_HOME/memories` 已有文件。
- Codex 新建与 resume 命令不携带 `--enable memories` 或 `-c memories.*`，保证全局配置是唯一控制来源且不存在会话级覆盖。
...
- 工作区按 Chat、Task、Chat 分层隔离，删除会话或 Chat 时同步清理对应目录；所有服务级工作区都从 `ALTER0_RUNTIME_ROOT` 派生，不写入源码仓库下的 `.alter0`。
- 所有直连 Codex 会话共享 `codexapp.ResolveActiveHome()` 返回的活动 Home；会话隔离由独立工作区、仓库 checkout、附件和 thread id 保证。删除会话只清理会话工作区，不触碰共享认证、配置、Skills、Memories 或其他线程。
...
- Chat 应用层在 `ListTurns` / 单会话详情中直接输出轻量 `RuntimeTraceEvent`，并随 turn summary 输出 `runtime_trace_events`；Chat 前端装配 turn timeline 时直接消费该结构，把 `kind / status / detail_available` 写入稳定 DOM metadata。详情按展开时通过事件 id 对应的 `event_id` 懒加载，完整 `blocks` 只从 detail 接口进入消息缓存。
...
- `internal/chatruntime/application/service.go` 在 `InputWithAttachments` 中把 Chat 附件规范化为 turn 附件并写入会话工作区 `input-attachments/`。图片继续通过 `-i` 进入 Codex；普通文件提示路径相对实际 `cmd.Dir` 计算，因此绑定仓库时为 `../input-attachments/...`。输入请求不再解析 Skill 选择；历史 `skill_context` 使用 `json.RawMessage` 兼容解码，新快照不写回。
...
- `internal/interfaces/web/server_chat.go` 的单会话详情默认返回最新 `20` 个 turns，`turn_limit` 上限为 `160`，并按约 `1MiB` 的前端 API turn DTO 页预算从最新 turn 向前装载；`turn_before=<turn_id>` 以目标 turn 为右开边界返回更早一页，并在 `turns_paging` 中输出 `total / byte_limit / approx_bytes / has_more_before / has_more_after / oldest_turn_id / newest_turn_id / next_before_turn_id`。集合接口继续只返回会话摘要，避免左侧会话列表拉取大体积 turns。
...
- `internal/codex/domain` 负责 `auth.json` 快照、身份识别与可选额度窗口模型；`internal/codex/application` 负责读取当前活动 `auth.json`、刷新 quota，并按 `limit_window_seconds` 而非 primary/secondary 位置把 5 小时与周窗口归入兼容的 `hourly / weekly` 字段，通过 Codex app-server 的 `model/list`、`config/read`、`config/batchWrite` 读取真实运行时能力并更新 `model` / `model_reasoning_effort`，以及编排独立 Codex 登录会话。`LoginSessionStartRequest.AuthMethod=device_auth` 会把命令参数收敛为 `codex login --device-auth`；登录进程使用 Store 分配的独立登录目录作为 `CODEX_HOME`，stdout/stderr 通过流式 capture 写回会话日志，并从输出中宽松解析 `verification_uri / verification_uri_complete / user_code / expires_in / interval`。
...
- `scripts` 承载运行账户凭据、Node/Playwright 工具链和部署初始化脚本；Node 初始化同时覆盖 `internal/interfaces/web` 与 `internal/interfaces/web/frontend`。`playwright.config.shared.js` 忽略父进程继承的正式 `ALTER0_RUNTIME_ROOT`，每次运行生成系统临时目录下的独立 runtime root，并在 WebServer 环境中把 `ALTER0_STORAGE_DIR / ALTER0_CODEX_WORKSPACE_ROOT` 显式置空，避免 Playwright 的环境合并重新带回父进程值；仅允许通过 `ALTER0_PLAYWRIGHT_RUNTIME_ROOT` 指定系统临时目录的直接 `alter0-playwright-*` 子路径，其他覆盖直接拒绝。
...
- `cmd/alter0/codex_command.go` 负责 Codex CLI 版本解析：默认自动收集运行账户 `.local/bin`、NVM `current`、全部 NVM Node 版本、启动参数/环境候选、公共路径与 `PATH` 候选，通过各命令的 `--version` 输出解析语义版本并选择最高版本；同版本按稳定入口优先级保留首项。Runtime supervisor child 每次启动都会重新解析，历史 `-codex-command=/.../.nvm/versions/node/<old>/bin/codex` 只作为候选，不再形成永久锁定。`ALTER0_CODEX_COMMAND_MODE=pinned` 是唯一跳过自动比较的锁版入口。
- Codex 运行时状态接口同时返回活动 `auth.json`、`config.toml`、当前 profile、活动 model、思考深度、配置来源、`model/list` 返回的可选 model 能力集，以及当前 `auth.json` 解析出的身份快照与实时刷新后的 quota 信息；模型与思考强度保留 Codex 返回的原始值和顺序，前端不做本地重命名或排序。quota 解析读取每个窗口的 `limit_window_seconds`，识别 18000 秒的 5 小时窗口与 604800 秒的周窗口，缺失或未知窗口不按返回位置兜底，并通过可选 `hourly / weekly` 字段避免零值占位。更新运行时设置时，后端先通过 `config/read` 解析当前生效 key path，再调用 `config/batchWrite` 更新 `model` 与 `model_reasoning_effort`，并触发 `reloadUserConfig` 让当前运行时立即生效。登录会话接口复用 Codex account service，不把 device-code 状态写入长期账号存储；只有 `codex login` 成功产出登录目录下的 `auth.json` 后，才通过既有 `AddFromRaw` 保存账号快照并允许覆盖 `runtime-device` 记录。
- CLI Runtime 运行参数通过源码内置默认值、启动参数和运行账户配置解析，仍受 Provider、Claude Code 与 Codex CLI 实际能力约束。
- Runtime 重启必须由 supervisor 托管，候选实例通过 readyz 后才切换；Runtime 页打开重启弹窗时默认选择 `sync_remote_master=true`。候选接口由 `cmd/alter0` 执行 `git fetch --prune origin master`，基于运行中二进制的 `vcs.revision` 或仓库 `HEAD` 解析当前 commit，再按 `current..origin/master` 返回全部后续提交，并通过 `git log -n 11 <current>` 追加当前 commit 及其向前 10 个历史提交；Web API 与前端渲染层都按 `committed_at` 从新到旧稳定排序候选项。同步请求未传入 `target_commit` 时执行既有快进；传入 `target_commit` 时先解析短 hash，确认目标提交可从 `origin/master` 到达，再对本地 `master` 执行 `git reset --hard <target>` 并通过统一构建入口重建前端产物和候选二进制。当存在 Git 已跟踪改动时，`cmd/alter0` 返回 `RuntimeRestartError`，错误码固定为 `runtime_restart_discard_confirmation_required`，Web API 与 supervisor client 必须透传同一 JSON `code`，前端只在收到该错误码后进入二次确认。未确认时不得清理或回滚本地工作区内容。
...
- Web 接口测试覆盖会话置顶、7 天不活跃清理、置顶跳过、queued/running 任务保护、workspace 删除、维护执行器不可用、资源清理失败、Runtime 重启候选列表时间排序、目标 commit 透传和确认错误码；前端组件测试覆盖 Schedules 内置任务、Runtime 重启默认远端同步、master 候选 commit 按时间展示与选择、按需二次确认、Codex device-code 登录和 Runtime 页 Claude Code Provider Console 的多 Provider 连续注册、查看与编辑。

...
- `ConversationRuntimeProvider.tsx` 维护模块级 `conversationRuntimeCache`，专门覆盖同一 SPA 工作台内从 Chat 切到其他路由再返回的个人单设备恢复场景。缓存保存 Chat 会话列表、活动会话 id 和每个会话完整已加载消息，写入时复制消息、附件和 process events，读取时按 8 小时 TTL 校验；命中时作为 Provider 初始状态，随后原有 `/api/terminal/sessions?scope=chat` 与单会话详情请求继续回源并沿现有合并规则覆盖。Provider 同步写入 30 天 `localStorage` 完整消息长期快照，用于刷新、重开或 sessionStorage 丢失后的首屏恢复；同时写入只含会话元数据的轻量快照，用于完整消息快照超出浏览器配额或被清理后的会话列表兜底。长期快照不作为跨设备事实来源，删除、置顶、输入返回、列表刷新和详情刷新仍通过 React state 统一回写缓存并等待服务端回源确认。

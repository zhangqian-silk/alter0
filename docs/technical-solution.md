...
- `ConversationWorkspace.tsx` 在 Chat 时间线滚动容器上只监听 `scroll`：普通滚动到顶且本地仍有隐藏消息时展开下一批本地消息，已完全展开时不再调用 `refreshActiveSession()`，也不注册额外触摸历史加载监听。服务端更早 turns 由 `ConversationRuntimeProvider` 的 `turns_paging` 后台补页 effect 负责。
...
- `ReactManagedTerminalRouteBody.tsx` 维护模块级 `terminalRuntimeCache`，专门覆盖同一 SPA 工作台内从 Terminal 切到其他路由再返回的个人单设备恢复场景。缓存保存排序后的会话列表、活动会话 id 和每个会话完整已加载 turns，写入时复制 turn/runtime event/attachment 数据，不裁剪已加载历史，读取时按 24 小时 TTL 校验；命中时直接作为 `useTerminalRuntimeController` 初始状态并跳过空白 loading 首屏，随后原有 `/api/terminal/sessions` 与单会话详情请求继续回源并通过 `mergeSessionSnapshot` 合并覆盖。当前活动会话详情若返回 `turns_paging.has_more_before`，组件会用 `turns_paging.next_before_turn_id / oldest_turn_id` 自动带 `turn_before` 和 `turn_limit=20` 后台补拉更早页，分页、输入返回、轮询和 page-activation 结果都按 turn id 合并并刷新缓存时间。缓存不写入 `localStorage`，不作为跨刷新或跨设备事实来源；删除、置顶、输入返回、列表刷新和详情刷新仍通过 React state 统一回写缓存。

...
- `ConversationRuntimeProvider.tsx` 对 `chat` 路由固定归一到 `alter0-chat`：读取本地活动会话、最近快照、服务端集合、单会话详情和用户手动 focus 时都会把 Chat 会话规整成这一条长期会话，并停止写回 `chat_session_id` URL 参数；`agent-runtime` 仍保留原来的多 Agent、多 session 列表、query 恢复和删除能力。
...
- `src/app/routeState.ts` 负责运行页路由与会话 query 协调：保留现有 hash 路由，并继续维护 `agent_session_id`、`terminal_session_id` 两个多会话恢复参数。`ConversationRuntimeProvider.tsx` 对 `chat` 始终使用 `alter0-chat`，并清理旧 `chat_session_id`；对 `agent-runtime` 则在初始化时优先读取 `agent_session_id` 恢复活动会话，并在会话 focus、创建、删除和服务端恢复后回写 URL。`ReactManagedTerminalRouteBody.tsx` 对 Terminal 采用同一策略。query 缺失或目标会话不存在时，运行页才回退到 `sessionStorage` 快照与服务端列表默认项。

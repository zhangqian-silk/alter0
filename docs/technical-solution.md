...
- `internal/interfaces/web/frontend/index.html`、`static/dist/index.html` 与登录页模板统一以 `html[lang="en"]` 启动；`LegacyWebShell` 与 `ReactRuntimeFacade` 通过监听和回写 `document.documentElement.lang` 统一驱动中英文壳层文案切换。`renderLoginPage` 继续直接输出服务端 HTML，但视觉与文案已对齐工作台基线：复用 `IBM Plex Sans + Sora` 字体组合、近白卡片表面与安全入口 copy。
- `internal/interfaces/web/frontend/src/features/shell` 维护稳定 Web Shell 的前端信息架构：当前路由高亮、导航折叠态、导航 tooltip、语言感知文案，以及与主仓库旧壳层一致的品牌头部、Session Pane 头部与历史列表、ChatWorkspace 头部动作区、欢迎区文案、欢迎区 target picker、prompt pills、欢迎区/消息区显隐、消息列表 DOM、运行时 controls/note/sheet 原生 DOM、Session 历史空态提示/可访问标签、Composer 面板、路由页 `route-head` 标题/副标题和主工作区的 `page-mode / data-route / chatView / routeView` 显隐状态。`bootstrap/ReactRuntimeFacade.tsx` 负责 Chat / Agent 的会话创建、切换、删除、消息流、结构化 runtime、移动端 runtime sheet、草稿恢复与 `alter0:legacy-shell:*` bridge 事件处理，并通过 `legacyRuntimeSnapshotStore` 直接发布 chat workspace、session pane、message region 与 chat runtime 四类结构化快照；React 再从共享 store 订阅当前会话标题、副标题、欢迎区描述、结构化 target picker、欢迎区/消息区显隐、结构化消息快照，以及运行时 target/provider/model/capabilities/skills 状态、错误提示与移动端 runtime sheet。chat runtime 的 target/model/tool/skill 交互通过共享 runtime API 直接回写 React runtime 状态。`components/ScrollJumpStrip.tsx` 仅在 `agent-runtime` 消息区输出与主仓库 Terminal 一致的右侧箭头四键，并继续通过容器滚动、resize 和 DOM 变更实时采样当前可见消息块、输出 `data-scroll-jump-*` 契约；普通 `Chat` 消息区与非 Terminal 托管 route 页不再挂载这组按钮。`components/ReactManagedAgentRouteBody.tsx`、`components/ReactManagedTerminalRouteBody.tsx`、`components/ReactManagedMemoryRouteBody.tsx`、`components/ReactManagedControlRouteBody.tsx`、`components/ReactManagedSessionsRouteBody.tsx`、`components/ReactManagedTasksRouteBody.tsx` 与 `components/ReactManagedProductsRouteBody.tsx` 统一通过 `components/ReactManagedRouteBody.tsx` 接管 route body，其中 Agent 配置页复用 `shared/api/client.ts` 请求 `/api/control/agents`、`/api/control/skills` 与 `/api/control/mcps`，在 React 内维护选中 Agent、表单草稿、紧凑表单栅格、保存/删除与运行页跳转入口；Terminal 页直接请求 `/api/terminal/sessions*` 与 step 明细接口，在 React 内维护会话列表、详情轮询、step 展开、滚动定位、关闭/删除、本地草稿恢复与 Markdown 输出渲染，Terminal 组件本身通过滚动快照、交互期降频轮询与逐帧 jump 状态同步保持阅读连续性，并继续输出旧版 `terminal-*` DOM class 契约作为布局皮肤基线，但不再依赖 legacy controller 驱动任何状态；其轮询计划由会话状态驱动：`busy` 会话在非滚动态下继续刷新列表与详情，`ready` 会话仅保留低频列表刷新，滚动中的输出阅读优先于明细轮询；窄屏下会话抽屉入口统一复用壳层头部 `Sessions`，Terminal 工作区头部则隐藏自身重复的 `data-terminal-session-pane-toggle`；Memory 页复用同一客户端请求 `/api/agent/memory` 与 `/api/memory/tasks*`，在 React 内维护任务筛选、任务表格、详情侧栏、日志、产物与只读记忆文档卡片；控制台卡片页复用同一客户端请求 `/api/control/channels`、`/api/control/skills`、`/api/control/mcps`、`/api/control/llm/providers`、`/api/control/environments` 与 `/api/control/cron/jobs`，其中 environments route 由专用环境页分支维护运行态工具栏、敏感值显隐、模块卡片表单、运行时重启轮询与审计渲染，其余控制页继续复用通用卡片列表；`ReactManagedControlRouteBody.tsx` 在 `channels / skills / mcp / models / cron-jobs` 五类共享卡片页外层输出稳定的 `control-route-grid` 容器，并由 `shell.css` 统一收敛桌面网格与窄屏单列重排，避免字段标签、状态徽标和复制按钮在真窄屏上错位；Sessions 页复用同一客户端请求 `/api/sessions` 并在 React 内维护筛选表单、查询参数与会话详情卡片，Tasks 页复用同一客户端请求 `/api/control/tasks*` 并在 React 内维护筛选表单、分页、紧凑任务列表、详情面板、日志回放与 follow-up terminal 输入；`components/SessionPane.tsx` 继续在单卡内渲染标题、摘要、短 hash 与辅助操作，并通过 React 会在 `routeBody` 上输出当前页 `data-react-managed-route` ownership 标记、在 `appShell` 上输出稳定的 `data-react-managed-routes` 路由清单，保证壳层状态更新不会清空页面主体。
- Web 壳层的品牌展示统一由前端源与服务端模板共同维护：`frontend/index.html` / `static/dist/index.html` 负责 `Alter0 Chat` 页签标题，`renderLoginPage` 负责 `Alter0 Login` 与 `Alter0 Console Login`，`legacyShellCopy.ts`、`ChatWorkspace.tsx` 与兼容 `static/chat.html` 负责导航品牌位、会话栏标题和欢迎区 `Alter0 workspace` 文案；默认 welcome 标题与说明由 `ChatWorkspace.tsx` 直接提供 repo / task / runtime 导向的空态文案，而不是通用助手问候语；这些展示文案调整不影响 `alter0.*` 事件名、存储 key、cookie 或元数据字段等运行契约。
- `internal/interfaces/web/frontend/src/styles/shell.css` 维护 React 壳层的 source-owned 布局覆盖，但视觉与结构口径回到主仓库当前 shell：桌面端使用全宽三栏工作台，不再额外包裹居中的三块胶囊 canvas；品牌区、Session Pane、Chat header、欢迎区、`route-head` 与 Composer 直接沿用旧版 `chat-core.css / chat-routes.css / chat-terminal.css` 的布局节奏，React 只补足需要的 DOM 层级、移动端抽屉、阅读宽度约束和滚动定位条样式。壳层默认使用更克制的工作台视觉：字体收敛为 `IBM Plex Sans + Sora + Noto Sans SC` 组合，常规表面使用近白底、低对比边框和有限强调色，不再依赖大面积玻璃感、强渐变和营销式装饰层。共享 route surface 继续作为 `Control / Sessions / Tasks / Memory / Terminal / Codex Accounts` 等页面的统一视觉基线：`.route-card / .route-surface / .route-detail-panel / .route-data-table / .environment-* / .task-summary-row` 统一收敛为近白主表面、浅灰说明层、低对比边框和浅蓝选中态，避免各页再派生独立的高装饰卡片系统。头部 `nav-toggle / panel-toggle / mobile-new-chat / pane-action` 在桌面三栏默认隐藏，仅在 `max-width: 1100px` 时重新作为抽屉入口显示，并在 `info-mode` 默认隐藏 `panel-toggle`、在 terminal `page-mode` 保留 `panel-toggle / mobile-new-chat`；空态 `Chat` 通过 `chat-pane.empty-state` 覆盖显式压缩欢迎区上边距、welcome tag 节奏与 Composer 顶距，并把欢迎区切成 centered stack：桌面与中宽度下，`chat-view` 使用“可用内容区 + 贴底 composer”两行栅格，让欢迎区在 header 与 composer 之间沿主工作区中轴竖向居中；中窄屏隐藏重复的 `chat-header-copy`，只保留操作行让欢迎区承接标题与说明，真窄屏再把欢迎区切回贴顶起排，避免桌面和窄屏继续用自动顶距或双标题结构把首屏内容拉散；窄屏最终生效的 legacy override 继续统一声明普通 `page-mode` / `Terminal` 的 `chat-header` 两行 grid 头部、移动按钮显隐、欢迎区顶部节奏以及 `route-view` 的边框盒/高度约束，确保不同页面在 `1100px` 及以下共享同一贴顶布局基线。该 override 下主导航抽屉中的 `.menu` 需显式声明 `min-height: 0`、`overflow-y: auto`、`-webkit-overflow-scrolling: touch` 与 `overscroll-behavior-y: contain`，让品牌区与语言切换入口之外的菜单内容在小高度视口中独立滚动，不再被抽屉容器裁切；`.scroll-jump-strip` 与 `.terminal-jump-cluster` 分别负责把 Agent 运行态消息区和 Terminal 对话区的箭头四键固定在阅读容器右侧，普通 `Chat` 消息区与其他 route 页不再覆写桌面态隐藏的 `pane-action`；`max-width: 760px` 再进一步压缩抽屉宽度、头部按钮尺寸与间距，保持真手机宽度下的可触达性；这些断点与 `legacyShellState.ts`、`shared/viewport/mobileViewport.ts` 的视口阈值保持一致；`public/legacy/*.css` 继续承载兼容内容区样式、Terminal 细节和 route body 内容皮肤。
- `LegacyWebShell.tsx` 在窄屏抽屉态下接管主导航路由点击后的收口：`requestLegacyShellRouteNavigation` 发出后，若当前视口命中移动断点，则立即调用 transient panel dismiss 逻辑移除 `nav-open / overlay-open`，避免切页后旧菜单层继续停留在新页面上。
...
- `ChatWorkspace` 在窄屏下按页面类型收口 header：普通 `page-mode` 与 Terminal React 工作区保持“操作行 + 标题行”，空态 `Chat` 则隐藏重复的标题行，仅保留操作行并由 `welcome-screen` 承接首屏标题；E2E 以 `chat.spec.ts` 的窄屏/中宽度 chat、page-mode 场景与 `terminal.spec.ts` 的移动端 terminal 场景共同约束顶部间距、标题显隐与抽屉入口显隐。
- `ChatWorkspace` 的空态 `Chat` 额外通过 E2E 约束桌面与窄屏首屏距离：`chat.spec.ts` 同时验证欢迎标题与 prompt/composer 间距，保证首屏内容始终从头部下沿开始连续排布，而不是被自动留白拆散。

...
- `shared/viewport/mobileViewport.ts` 对 `VisualViewport.height` 与 `offsetTop` 分开判定：当输入框聚焦或上一轮键盘占位仍在释放中，且回报的原始 `height` 相对 `baselineHeight` 仍达到键盘阈值时，状态机优先使用原始 `height` 计算 `--keyboard-offset`；`offsetTop` 只参与非键盘场景的可视底边同步，避免部分移动浏览器在键盘动画中用正 `offsetTop` 抵消收缩高度并造成 Composer 立即回弹。

...
| Skill & Memory | `internal/runtime`、`internal/execution`、`internal/orchestration`、`docs/skills`、`<runtime_root>/storage/memory` | CLI Runtime 上下文注入、Skill 仓库、Memory Context、会话摘要、长期记忆、记忆维护任务 |
...
- 直连 Codex 的 Chat 会话在自身工作区下维护 `.alter0/codex-runtime/` 与 `.alter0/codex-runtime/codex-home/`；Chat 的 Codex thread id 写入 `.alter0/codex-runtime/thread.json`，Chat 的 Codex thread id 写入 `.alter0/codex-runtime/threads/<YYYY-MM-DD>.json`；Chat 会话在 `<runtime_root>/workspaces/chat/sessions/<chat_session_id>/codex-home/` 下维护独立 `CODEX_HOME`。
...
- `internal/chatruntime/application/service.go` 在 `InputWithAttachments` 中把 Chat 附件规范化为 turn 附件、在工作区 `input-attachments/` 下写入本轮输入文件，并按类型拆分消费：图片继续通过 `codex exec -i <file>` 或 `codex resume -i <file>` 进入 Codex 视觉输入；首轮 `codex exec` 在所有图片参数后追加 `--` 再传入 prompt，隔离 Codex CLI 可变长 `-i` 参数解析；普通文件则在同轮 prompt 中附上 `input-attachments/<turn_id>/<filename>` 形式的 workspace 相对路径，要求 Codex 按需直接读盘；turn 摘要与持久化快照同步保留附件元数据，供 Chat 输入草稿与图片历史回显复用。Chat 输入请求携带的 `SkillContext` 会在每轮执行前渲染到工作区 `.alter0/codex-runtime/skills.md`，并通过托管 `AGENTS.md` 指令要求 Codex 只应用本轮选择的 Skill；空选择也会写入“未选择”标记，避免旧轮次 Skill 指令残留。

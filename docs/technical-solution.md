...
- 轮询频率按可恢复状态驱动：页面可见且存在 `local_running / recovering` 会话时，按约 `2s` 起步只拉 owner updates，连续无进展后退避到约 `3s / 5s / 8s`；updates 未对本轮可恢复 session 产生相关进展时不立即回源详情，而是按连续无进展第 6 次以及之后每 8 次触发最新 bounded detail 兜底；新的 `updated_at`、activity、消息或过程步骤会重置退避计数，仍未收敛且也没有新进展时继续保持退避计数；稳定完成且 `has_more_before=false` 的完整缓存会话不参与详情补偿，终态会话停止自动 updates/detail。

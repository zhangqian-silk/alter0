...
2026-07-10 更新：Chat 同步按移动端个人单设备场景收敛。共享 runtime controller 是唯一会话状态提交入口，请求以代次拒绝迟到结果，消息合并前先用内容 `updated_at` 判旧；input 通过 `client_request_id` 对账后端 turn，不创建 optimistic user turn，turn/event/update id 均使用 canonical 字符串。列表只返回六个展示/判鲜字段，详情由应用层原子快照生成，所有会话 API 使用 `Cache-Control: no-store`。页面隐藏时停止轮询；不足 5 分钟回前台只恢复本机未完成 updates，达到 5 分钟、首次进入或手动刷新才拉轻量列表与当前详情。长期缓存保留全部 summary，但只保留当前加最近四个内容快照，并排除 data URL 与 event detail blocks。本段规则替代下文早期“effect 重新合并 ref/state”“长期保存 event detail blocks”以及“成功空列表仍保留服务端会话”的旧描述。

2026-07-12 更新：Composer submission 与 Chat session state 完全分层。`sendPrompt` 在 input 响应前只设置内存态 `submitting`，不追加消息、不写 `local_running / failed`、不持久化请求错误；input、summary、detail 与 update 是会话状态的唯一事实源。结构化 HTTP 后端错误与 transport failure 都只进入 Composer 工具栏弱提示，transport failure 额外静默强制回读一次详情且不自动重发 POST。提交前已发出的详情请求通过请求代次失效，避免迟到快照清除新一轮同步意图。
...
- `internal/interfaces/web/frontend/src/shared/visibility/usePageActivation.ts` 是 Chat 的共享页面激活入口，监听 `focus`、`visibilitychange`、`pageshow` 与 `online`。Provider 记录隐藏起点并通过 `conversationSyncPolicy.ts` 计算恢复动作：不足 5 分钟时稳定会话零请求，本机已获后端接受且仍待收敛的同步意图恢复 updates；达到 5 分钟时刷新轻量全量列表和当前详情。入口使用短 debounce 合并同一恢复阶段的重复事件。
...
- `ConversationRuntimeProvider.tsx` 的会话状态提交入口统一通过 `setSessionsByRoute` 与 `setActiveSessionByRoute` 先写 `conversationRuntimeCache`、route 长期会话快照和轻量信息快照，再调用 React state 更新；`upsertRuntimeSession` 也必须走 `mergeRuntimeSessions`，不得以迟到 summary 直接替换更新的后端会话。`sendPrompt` 在 input 请求未完成前只设置 Composer `submitting`；若发送前没有真实 active session，先创建不带 `title` 的 ChatRuntime session，再把 input 提交给该 session。前端不从 prompt 派生标题、消息或状态，当前标题与时间线只来自底层 ChatRuntime input/summary/detail/update 的合并结果。收到权威 session 后，Provider 先建立非持久化同步意图再 upsert，保证 effect 能启动 owner updates；完成确认后才清空文本、附件与仓库草稿。transport failure 解除 submitting、保留草稿、显示定时消失的 Composer notice，并静默 `force` hydrate 当前 session；结构化 HTTP 后端错误显示安全后端消息但不做歧义重试或状态写入。`mergeRuntimeSessionTurnPagingPayload` 使用合并后的本地消息集校准 `turnsPaging`，消息排序使用稳定比较器保证同一 turn 的 user 在 assistant/Thinking 前。Provider 关闭自动 progressive history，缓存写入仍是权威 controller state 的单向输出。
...
- `ConversationRuntimeProvider.tsx` 将自动轮询条件、Composer 提交锁与会话业务状态拆分：后端 input 已确认且 turn 尚未收敛时记录非持久化同步意图，用它驱动 owner updates；`shouldBlockRuntimeInput` 只检查后端会话/turn 是否真实运行，Composer 另用 `submitting` 防止同一次 HTTP 提交重复触发。失败、中断或退出终态允许同一会话继续提交下一条输入。ChatRuntime `finishTurn` 只在服务端 runtime 执行失败时把 session 与 turn 同步标为 `failed`；浏览器 transport failure 与 input HTTP 拒绝不经过该前端状态链路。
...
- `mergeRuntimeSessions` 只比较后端会话事实的来源与新鲜度：旧 summary 不覆盖更新的 input/detail/event，详情或事件明确带有稳定 assistant、失败、中断或删除结果时收口业务状态。本机同步意图仅决定是否继续轮询，不参与 session status 合并，也不作用于 workspace header、移动端标题按钮或左侧会话列表。
...
- 前端请求状态独立于业务状态：input transport failure、结构化 HTTP 后端错误、轮询失败、退避或页面隐藏都不会把 assistant 消息或 session 标记为 failed。input 错误只进入 Composer 弱提示；只有快照或事件明确返回 `turn.failed`、`turn.interrupted`、`session.deleted` 或会话不存在，才改变会话业务状态。

...
- `internal/interfaces/web/frontend/src/features/conversation-runtime` 承载 `chat` 运行态：`ConversationRuntimeProvider.tsx` 负责复用 Terminal session API 的数据模型和状态机，通过 Chat 专属 `/api/chat/sessions` 完成会话创建/切换/删除/置顶、`POST /api/chat/sessions/{session_id}/input` 输入提交、结果收口、文本草稿与附件草稿恢复、模型与能力项选择，并在加载本地 snapshot 与 Terminal session `turns` 时把历史 `chat` 会话迁移为当前 Chat 会话结构。`ConversationWorkspace.tsx` 不再渲染 Skill 选择器、Deliverables、Session Profile 或独立 Skill 面板；历史会话的 `target_type=runtime` 仅作为会话摘要元数据保留。Chat 仍在常规模型 Provider 列表外额外注入前端内置 `Codex` provider，Skill 选择在发送时通过 Terminal input 的 `skill_ids` 字段提交。运行页会话列表与消息详情通过 `/api/chat/sessions` 与单会话详情接口恢复，浏览器只继续保存未发送草稿、附件草稿和当前页局部 UI 状态；Terminal 默认 `/api/terminal/sessions` 会话列表不包含 Chat owner 会话。单会话详情若携带 `turns_paging.has_more_before`，Provider 会先按消息 id 合并当前分页片段，再使用 `turns_paging.next_before_turn_id / oldest_turn_id` 继续请求 `turn_before` 后台补齐更早 turns；无重叠片段按消息时间判断前置或追加，避免 page-activation、轮询或输入返回的短 turns 页覆盖已加载历史。Provider 的模块级运行态缓存保留完整已加载消息并按 24 小时 TTL 失效；`LONG_TERM_SESSION_SNAPSHOT_STORAGE_KEY` 以 `localStorage` 保存 24 小时完整会话消息快照，`SESSION_INFO_SNAPSHOT_STORAGE_KEY` 额外保存 24 小时轻量会话信息快照；`resolveInitialConversationRuntimeState` 在运行态缓存 miss 后先读取完整本地快照，缺失时回退轻量会话信息快照，再合并 sessionStorage 活动会话与最近快照。
...
- `internal/interfaces/web/maintenance.go` 负责系统维护任务：服务启动时创建每日记忆维护与会话清理调度循环；`RunMemoryMaintenance` 通过编排入口发送系统消息并注入 `memory-maintenance` Skill；`RunSessionCleanup` 调用 Session application 清理超过 7 天不活跃且未置顶的会话，并同步删除任务关联与 Session workspace。运行页会话清理同一轮扫描 `terminal` 与 `chat` 两个标准 owner：`terminal` 对应 Terminal 页面，`chat` 对应 Chat 页面；两者使用相同不活跃阈值、置顶保护、运行中保护和删除流程，统计计入同一个 cleanup 响应。历史 `shared` owner 仅作为旧 Terminal 数据别名在读取时归一为 `terminal`。
...
- `internal/interfaces/web/server_terminal.go` 对 `/api/chat/sessions` 输入使用服务端请求生命周期控制 Terminal turn；浏览器刷新或前端主动断开请求只结束当前 HTTP 回传，不取消已进入 Terminal runtime 的执行，前端恢复链路依赖 Chat owner 的 Terminal session 详情补拉最终结果。
...
- `ConversationRuntimeProvider.tsx` 的发送链路始终确保存在服务端 Chat owner 的 Terminal-compatible session，再调用 `POST /api/chat/sessions/{session_id}/input`，并用返回的 Terminal session 或后续详情恢复生成本轮 user / assistant 消息；前端不再创建本地 running process step，不再解析 SSE。收到 input 结果、错误或恢复详情后同步收口最终状态，迟到结果不得重新打开已完成消息或覆盖最终正文。
...
- `internal/interfaces/web/server_terminal.go` 的 session collection、item、pin、delete 与 input handlers 由 Chat / Terminal 命名路由复用；Chat 请求必须走 `/api/chat/sessions` 并落到 `chat` owner，Terminal 请求走 `/api/terminal/sessions` 并落到 `terminal` owner。Chat 前端将 `turn.prompt` 映射为用户消息，将 `turn.final_output` 映射为 assistant 正文，将 `turn.runtime_trace_events` 映射为运行过程。
...
- `features/shell/components/useRuntimeComposerViewportSync.ts` 在移动端只同步共享 Runtime Composer 的静态 footprint；主输入框首触完全交给浏览器原生链路，不取消默认行为，不主动 focus，不监听或恢复 `window` page scroll，也不采集或恢复输入框后方的 `workspaceBody / runtime-workspace-screen / runtime-workspace-panel` 滚动位置。`RuntimeWorkspacePage.tsx` 在移动端把共享 Composer portal 到 `document.body`，`shell.css` 只让 workspace body 内的 spacer 承担静态 footprint；`runtimeKeyboardIsolation.css` 在 `shell.css` 之后加载，把 App Shell 改为 `height: 100dvh` 的动态视口容器，并在 `760px` 及以下把真实移动 workbar 固定为 `position: fixed; top: 0`，通过 `transform: translate3d(0, var(--mobile-viewport-offset-top, 0px), 0)` 在独立合成层对齐 visual viewport 顶边，workspace body 第一行保留 `--runtime-mobile-header-fixed-height` 占位，正文 panel 固定在第二行内部滚动，真实 Composer 作为 `position: fixed; bottom: 0` 浮层贴住动态视口底边。运行页后方层不再消费 `--keyboard-offset` 或 transform 改写高度。
- `ConversationWorkspace.tsx` 与 `ReactManagedTerminalRouteBody.tsx` 不会把键盘偏移变量透传给其他运行区控件：移动端顶部操作行与紧凑 workspace header 继续留在静态工作区流里，键盘开合期间只有底部 Composer 自身通过 fixed bottom 贴住动态视口底边；阅读定位条在主输入框聚焦期间直接停止渲染，待输入框失焦后再恢复；Composer 配置面板在 `760px` 及以下改为独立 fixed 浮层并限制最大宽高，避免技能、模型或工具面板重新挤压正文与输入区。
...
- `public/legacy/chat-terminal.css` 中的移动端 `.terminal-composer-shell` 不再声明 `transition: bottom ...`；Composer 位置由 CSS 动态视口和 `bottom: 0` 驱动，避免键盘回弹动画和 CSS 补间叠加造成卡顿。
- `shared/viewport/mobileViewport.ts` 仍保留移动端视口状态推导，用于诊断、面板高度上限和历史兼容变量输出；Chat / Terminal 主运行页布局不再依赖这些变量驱动 App Shell 高度、Composer bottom 或 offsetTop 位移，键盘后的可见高度由 `100dvh` 和 fixed bottom CSS 承接。
...
- 读取本地缓存时先归一残留 `streaming` 消息，消息通过 Chat owner 的 Terminal-compatible session 详情恢复。
- Chat Web 前端不再使用 Web 流式网关；`ConversationRuntimeProvider.tsx` 只通过 `POST /api/chat/sessions/{session_id}/input` 获取更新后的 Terminal-compatible session，并通过 Chat owner 的 session 详情恢复请求断开后的最终消息。
...
- 移动端 root 在窄屏下不再使用 fixed 定位隔离 document 滚动，也不锁定 `html / body / #frontend-root` 的 `overflow:hidden`；前端入口 viewport meta 声明 `viewport-fit=cover, interactive-widget=resizes-content`，让支持该属性的浏览器把虚拟键盘纳入内容视口 resize。App Shell 由 `runtimeKeyboardIsolation.css` 固定为 `height: 100dvh`，运行页和 workspace body 继承该可见视口高度链，不再使用 `calc(var(--mobile-viewport-height) + var(--keyboard-offset))` 参与键盘布局；移动 workbar 通过 fixed top 加无 transition transform 对齐 visual viewport 顶边，Composer 通过 `position: fixed; bottom: 0` 贴住动态视口底边，避免浏览器工具栏或软键盘变化导致底部留白、内容裁切、页面级上移、顶部 header 位移或重复键盘补偿。
...
- `src/app/routeState.ts` 负责运行页路由与会话 query 协调：路由只解析 canonical path，不再使用 hash fragment；`/chat` 与 `/terminal` 统一识别 `session_id` 多会话恢复参数，写入时通过 `sessionRouteToken` 把完整会话 id 收敛为 8 位短 hash。主导航进入 `Chat` 时即使当前已经位于 `/chat`，也会删除旧 `session_id` 并派发路由同步事件；`ConversationRuntimeProvider.tsx` 监听该同步事件，在 query 缺失时切到当前会话列表第一项。`/chat` 兼容映射到 `/chat`，`ConversationRuntimeProvider.tsx` 在 Chat 初始化时只读取 `/api/chat/sessions` 集合，并把本地旧 `chat` snapshot 合并到 Chat 列表。`ReactManagedTerminalRouteBody.tsx` 对 Terminal 采用同一短 hash 策略。Terminal query 缺失或目标会话不存在时，运行页才回退到 `sessionStorage` 快照与服务端列表默认项。
...
- `ReactManagedTerminalRouteBody.tsx` 维护模块级 `terminalRuntimeCache` 与 24 小时 `localStorage` 完整快照，覆盖同一 SPA 工作台内从 Terminal 切到其他路由再返回、刷新或重开后的个人单设备恢复场景。缓存保存排序后的会话列表、活动会话 id 和每个会话完整已加载 turns，写入时复制 turn/runtime event/attachment 数据，不裁剪已加载历史，读取时按 24 小时 TTL 校验；命中时直接作为 `useTerminalRuntimeController` 初始状态并跳过空白 loading 首屏，随后原有 `/api/terminal/sessions` 与单会话详情请求继续回源并通过 `mergeSessionSnapshot` 合并覆盖。当前活动会话详情若返回 `turns_paging.has_more_before`，组件会用 `turns_paging.next_before_turn_id / oldest_turn_id` 自动带 `turn_before` 和 `turn_limit=20` 后台补拉更早页，分页、输入返回、轮询和 page-activation 结果都按 turn id 合并并刷新缓存时间；后台补页不得覆盖当前输入、详情面板、选择态或主动阅读滚动，同一 `session_id + turn_before` 边界已完成补拉后不会再次触发，只有分页边界推进后才继续下一页。缓存不作为跨设备事实来源；删除、置顶、输入返回、列表刷新和详情刷新仍通过 React state 统一回写缓存。
...
- `WorkbenchApp` 继续安装共享 `mobileViewportSync` controller 输出诊断与兼容变量，但 Chat / Terminal 主运行页布局不再依赖这些变量驱动键盘位移；移动端 `root.css` 在 `1100px` 及以下解除 `html / body / #frontend-root` 的 fixed 页面锁和根层 `overflow:hidden`，改用 `min-height: 100dvh`。`runtimeKeyboardIsolation.css` 在 `shell.css` 之后把 App Shell 改为动态视口容器，直接用 `height: 100dvh` 控制高度。真手机宽度下，真实移动 workbar 从 workspace grid 的普通流里脱离并固定在 visual viewport top，workspace body 只保留 header footprint；真实 Composer 从 workspace body 和 workspace grid 中脱离，经 `document.body` 顶层 portal 成为 fixed 浮层，通过 `bottom: 0` 贴住动态视口底边；正文 panel 是唯一内部滚动区。运行页后方层不让 document 或普通流布局承接 iOS 聚焦滚动，也不再通过 transform 处理 offsetTop；`useRuntimeComposerViewportSync` 不接管主输入框首触，不回放页面级滚动锚点，也不锁定输入框后方的 workspace 滚动容器。命令候选、配置面板和阅读定位控件保持静态位置。

...
- 前端刷新恢复顺序固定为：读取本地 route 快照渲染首屏；拉取 owner 会话列表；当当前活动会话缺少消息缓存、显式进入详情恢复、历史分页、手动刷新或 updates 返回 `resync_required` 时，才补拉对应单会话详情；随后按当前 owner cursor 请求 updates。该顺序保证刷新过程中即使网络短暂失败，也不会把本地 `Thinking...` 或 user-only turn 直接收敛为失败，也不会把常规增量轮询退化为全量详情轮询。
- 轮询频率按可恢复状态驱动：页面可见且存在运行中会话时，短时窗口内按约 `1s` 只拉 owner updates，超过短时窗口后退避到约 `5s`；updates 无事件只推进退避，不补拉当前活动会话或 owner 下 `busy / recoverable` 会话详情；稳定完成且 `has_more_before=false` 的完整缓存会话不参与详情补偿。
...
- `shared/visibility/usePageActivation.ts` 提供运行页共享的 page-activation hook：统一监听 `visibilitychange`、`focus`、`pageshow` 与 `online`，并用短时间去抖吸收同一恢复阶段的连续触发。`ConversationRuntimeProvider.tsx` 与 `ReactManagedTerminalRouteBody.tsx` 都复用这条链路；Conversation 在页面重新变为前台可见时刷新当前 owner 会话列表，并让下一轮 updates 按 cursor 补齐增量；只有本地缓存不完整、updates 要求 `resync_required`、用户显式刷新或进入详情/历史分页时才补拉单会话详情。页面隐藏时停止高频轮询，Terminal 兼容组件则继续按自身状态刷新会话列表与当前活动会话详情。
...
- 前端组件测试覆盖不创建浏览器长连接、重复 event id 幂等丢弃、`resync_required` 触发单会话详情补拉、updates 命中时不对每个 busy 会话固定周期拉完整详情、latest user-only 待水合状态继续短周期读取 owner updates。

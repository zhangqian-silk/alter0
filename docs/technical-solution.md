...
- `internal/interfaces/web/frontend/src/features/conversation-runtime` 承载 `chat` 运行态：`ConversationRuntimeProvider.tsx` 负责复用 Terminal session API 的数据模型和状态机，通过 Chat 专属 `/api/chat/sessions` 完成会话创建/切换/删除/置顶、`POST /api/chat/sessions/{session_id}/input` 输入提交、结果收口、文本草稿与附件草稿恢复、模型与能力项选择，并在加载本地 snapshot 与 Terminal session `turns` 时把历史 `chat` 会话迁移为当前 Chat 会话结构。`ConversationWorkspace.tsx` 不再渲染 Skill 选择器、Deliverables、Session Profile 或独立 Skill 面板；历史会话的 `target_type=runtime` 仅作为会话摘要元数据保留。Chat 仍在常规模型 Provider 列表外额外注入前端内置 `Codex` provider，Skill 选择在发送时通过 Terminal input 的 `skill_ids` 字段提交。运行页会话列表与消息详情通过 `/api/chat/sessions` 与单会话详情接口恢复，浏览器只继续保存未发送草稿、附件草稿和当前页局部 UI 状态；Terminal 默认 `/api/terminal/sessions` 会话列表不包含 Chat owner 会话。单会话详情若携带 `turns_paging.has_more_before`，Provider 会先按消息 id 合并当前分页片段，再使用 `turns_paging.next_before_turn_id / oldest_turn_id` 继续请求 `turn_before` 后台补齐更早 turns；无重叠片段按消息时间判断前置或追加，避免 page-activation、轮询或输入返回的短 turns 页覆盖已加载历史。Provider 将时间线/会话 workspace 上下文与 Composer 上下文分离，后台 `turn_before` 补页只更新消息集合；Composer 上下文只消费稳定草稿、附件和运行配置字段，避免未变化的输入区随历史合并刷新。Provider 的模块级运行态缓存保留完整已加载消息并按 24 小时 TTL 失效；`LONG_TERM_SESSION_SNAPSHOT_STORAGE_KEY` 以 `localStorage` 保存 24 小时完整会话消息快照，`SESSION_INFO_SNAPSHOT_STORAGE_KEY` 额外保存 24 小时轻量会话信息快照；`resolveInitialConversationRuntimeState` 在运行态缓存 miss 后先读取完整本地快照，缺失时回退轻量会话信息快照，再把旧 `ACTIVE_SESSION_SNAPSHOT_STORAGE_KEY / RECENT_SESSION_SNAPSHOT_STORAGE_KEY` sessionStorage 快照一次性归一化写入最新 runtime cache 与 localStorage cache，随后删除旧 snapshot key，后续状态变化不再写回旧结构。
...
- `internal/interfaces/web/server_terminal.go` 对 Chat owner 的输入使用服务端请求生命周期控制 runtime turn；浏览器刷新或前端主动断开请求只结束当前 HTTP 回传，不取消已进入 Terminal runtime 的执行，前端恢复链路依赖 Chat owner 的 session 详情补拉最终结果。
- `ConversationWorkspace.tsx` 的移动端发送手势链路继续复用共享 `RuntimeComposer` submit capture，但在 `chat / chat` 路由下会先检查当前聚焦的主 textarea，并在直达 `sendPrompt` 前主动 `blur()`；发送动作与软键盘回弹依赖浏览器动态视口恢复和 fixed bottom 贴底完成，不再由键盘偏移变量驱动主布局。
...
- `ConversationRuntimeProvider.tsx` 的发送链路始终确保存在服务端 Chat owner 的 runtime session，再调用 `POST /api/chat/sessions/{session_id}/input`，并用返回的 Terminal session 或后续详情恢复生成本轮 user / assistant 消息；前端不再创建本地 running process step，不再解析 SSE。收到 input 结果、错误或恢复详情后同步收口最终状态，迟到结果不得重新打开已完成消息或覆盖最终正文。
- `ConversationRuntimeProvider.tsx` 维护模块级 `conversationRuntimeCache`，专门覆盖同一 SPA 工作台内从 Chat 切到其他路由再返回的个人单设备恢复场景。缓存保存 Chat 会话列表、活动会话 id 和每个会话完整已加载消息，写入时复制消息、附件和 process events，读取时按 24 小时 TTL 校验；命中时作为 Provider 初始状态，随后原有 `/api/chat/sessions` 与单会话详情请求继续回源并沿现有合并规则覆盖。Provider 同步写入 24 小时 `localStorage` 完整消息快照，用于刷新、重开或 sessionStorage 丢失后的首屏恢复；同时写入只含会话元数据的轻量快照，用于完整消息快照超出浏览器配额或被清理后的会话列表兜底。本地快照不作为跨设备事实来源，删除、置顶、输入返回、列表刷新和详情刷新仍通过 React state 统一回写缓存并等待服务端回源确认。
- `internal/interfaces/web/server_terminal.go` 的 session collection、item、pin、delete 与 input handlers 由 Chat / Terminal 命名路由复用；Chat 请求必须走 `/api/chat/sessions` 并落到 `chat` owner，Terminal 请求走 `/api/terminal/sessions` 并落到 `terminal` owner。Chat 前端将 Terminal `turn.prompt` 映射为用户消息，将 `turn.final_output` 映射为 assistant 正文，将 `turn.runtime_trace_events` 映射为运行过程。
- `internal/interfaces/web/frontend/src/features/shell/components/RuntimeTimelinePrimitives.tsx` 只维护运行页时间线里的共享展示原语：`RuntimeAttachmentGallery` 统一渲染用户图片附件与 Terminal turn 附件缩略图。`RuntimeTimeline.tsx` 统一输出 `runtime-message / runtime-message-user / runtime-message-assistant / runtime-message-bubble` 契约，Chat 与 Terminal 的 `Process` 都消费 `RuntimeTraceEvent`，默认只渲染 `Thinking / 已思考` 披露行。`RuntimeProcessStepMeta.tsx` 是 Chat 与 Terminal 的共享事件行 meta 渲染入口，统一输出类型标签、耗时与 `Ready / Busy / Failed / Interrupted` 状态样式；`RuntimeProcessDetailBlocks.tsx` 是共享过程详情渲染入口，直接消费 `RuntimeTraceEvent.blocks` 或 event detail API 返回的 `blocks`。Chat 通过 `ConversationRuntimeProvider.loadProcessEventDetail` 在步骤展开时按 `raw.ref` 或事件 id 拉取 detail 并写回当前消息的 `processEvents`，Terminal 继续在 `ReactManagedTerminalRouteBody.tsx` 内维护同类 detail cache。组件统一输出 `.terminal-rich-block`，其中 `terminal / code / diff / tool_input` 与 JSON 类 `tool_output` 输出 `.terminal-rich-pre.terminal-step-content`，`text / markdown / thinking / tool_output(text) / error / log` 输出 `MessageMarkdownHTML` + `.terminal-step-content.terminal-step-richtext`，并统一保留标题、文件名与起始行号，避免任何过程详情首帧先按临时普通 Markdown 显示。`runtime-process-answer`、`terminal-final-rendered` 等差异样式只作为变体 class 挂载在 block 外层，不再把页面私有 wrapper 重新塞回 markdown HTML 字符串。
...
- `features/shell/components/RuntimeWorkspacePage.tsx` 与 `RuntimeWorkspaceShell.tsx` 为运行页共享 `workspaceBodyRef`、会话列表注册、移动端 workbar、layout state 与 Composer suspended 契约；`runtimeMobileLayout.ts` 负责把 `isMobileViewport / inputFocused / sessionPaneOpen / composerPanelOpen` 归一为 `desktop / mobile-rest / mobile-keyboard / mobile-session-drawer / mobile-composer-panel`。`RuntimeWorkspaceHeader.tsx` 为 `Chat / Terminal` 输出同一组标题、状态按钮与 `Details` 按钮元素，Terminal 只传入状态值和详情面板内容，不再传入 Terminal 专属 header kind、状态按钮属性或 details toggle 外观；`RuntimeComposer.tsx` 为三条运行页输出同一套 composer shell 与 form surface，`shell.css` 只允许 Terminal 在 meta、Codex 候选和工具按钮上追加领域样式，不再通过 `[data-runtime-view="terminal"] .runtime-composer-shell` 覆盖更深背景或更低 padding，也不再为 Terminal 额外渲染 form 外层的 status note；`ReactManagedTerminalRouteBody.tsx` 将失败、退出和附件错误提示写入 `RuntimeComposer.metaContent`，与 Skill/Chat 工具栏 meta 保持同一结构。`ConversationWorkspace.tsx` 与 `ReactManagedTerminalRouteBody.tsx` 在移动端通过 `ResizeObserver` 持续测量 Composer 的静态 footprint：`--runtime-composer-rest-inset` 与 `--runtime-bottom-reserved` 表示输入区自身高度，`--runtime-composer-inset` 固定为 `0px`，键盘抬起量不再写入 workspace inset；`shell.css` 与 `public/legacy/chat-terminal.css` 再用这些变量收口 `.conversation-chat-screen` / `.terminal-chat-screen` 与 jump controls 的可见高度，保证 Chat 与 Terminal 的最后一屏内容稳定停在输入区上沿，同时不会在输入框上方重复留下键盘高度对应的空白带。
- 上述 Composer footprint 同步不能只依赖键盘事件起点；当前实现会在 Composer 自身 `ResizeObserver`、窗口 resize 与 `transitionend` 上继续补测，并在下一帧和动画尾帧兜底重算，确保键盘收起或 Composer 回弹到底边后及时释放旧的底部占位，不在页面上遗留空白带。
- `features/shell/components/useRuntimeComposerViewportSync.ts` 在移动端只同步共享 Runtime Composer 的静态 footprint，并把 `data-runtime-mobile-layout` 与 `--runtime-bottom-reserved` 写入 workspace body；主输入框首触完全交给浏览器原生链路，不取消默认行为，不主动 focus，不监听或恢复 `window` page scroll。输入框首触后的短暂键盘动画稳定窗口只采集并恢复 `workspaceBody / runtime-workspace-screen` 的输入前滚动快照，用于抵消 iOS Safari 原生 focus 自动推移；该窗口在消息区产生新滚动后立即取消，且不采集或恢复 `runtime-workspace-panel` 或页面级滚动位置。`RuntimeWorkspacePage.tsx` 在移动端把共享 Composer portal 到 `document.body`，portal host 同步 `data-runtime-composer-suspended` 与 `aria-hidden`；`shell.css` 只让 workspace body 内的 spacer 承担静态 footprint；`runtimeKeyboardIsolation.css` 在 `shell.css` 之后加载，把 App Shell 改为 `height: 100dvh` 的动态视口容器，并在 `760px` 及以下把真实移动 workbar 固定为 `position: fixed; top: 0`，通过 `transform: translate3d(0, var(--mobile-viewport-offset-top, 0px), 0)` 在独立合成层对齐 visual viewport 顶边，workspace body 第一行保留 `--runtime-mobile-header-fixed-height` 占位，正文 panel 固定在第二行内部滚动，真实 Composer 作为 `position: fixed; bottom: var(--keyboard-composer-offset, var(--keyboard-offset, 0px))` 浮层贴住键盘或动态视口底边。运行页后方层不再消费键盘高度或 transform 改写高度。
- `ConversationWorkspace.tsx` 与 `ReactManagedTerminalRouteBody.tsx` 不会把键盘偏移变量透传给其他运行区控件：移动端顶部操作行与紧凑 workspace header 继续留在静态工作区流里，键盘开合期间只有底部 Composer 自身通过 fixed bottom 与 `--keyboard-composer-offset` 贴住键盘或动态视口底边；阅读定位条在主输入框聚焦期间直接停止渲染，待输入框失焦后再恢复；Composer 配置面板在 `760px` 及以下改为独立 fixed 浮层并限制最大宽高，避免技能、模型或工具面板重新挤压正文与输入区。
- `runtimeKeyboardIsolation.css` 为移动端运行页建立固定层级：Composer 高于消息阅读定位按钮和 Terminal 四键定位条，左侧 session drawer 高于 Composer。`RuntimeWorkspaceShell.tsx` 在 `mobile-session-drawer` 状态下 blur 当前活动输入元素，并把 root 与 workspace body 标记为 `data-runtime-composer-suspended="true"`；`RuntimeWorkspacePage.tsx` 同步暂停 portal Composer，避免抽屉打开时输入框覆盖抽屉或键盘残留。`ConversationWorkspace.tsx` 与 `ReactManagedTerminalRouteBody.tsx` 会在移动端输入框聚焦时直接移除 overlay，避免右下角残留半截 jump control 漏到键盘上沿。
...
- `public/legacy/chat-terminal.css` 中的移动端 `.terminal-composer-shell` 不再声明 `transition: bottom ...`；Composer 位置由 CSS 动态视口和 `--keyboard-composer-offset` 驱动，避免键盘回弹动画和 CSS 补间叠加造成卡顿。
- `shared/viewport/mobileViewport.ts` 仍保留移动端视口状态推导，用于诊断、面板高度上限、`--keyboard-offset` 与 `--keyboard-composer-offset` 兼容变量输出；Chat / Terminal 主运行页布局不再依赖这些变量驱动 App Shell 高度、workspace header 或正文 panel 位移，键盘后的可见高度由 `100dvh` 承接，Composer 只消费 composer offset。
- `useRuntimeComposerViewportSync.ts` 只把 Composer 自身高度写入 `--runtime-composer-rest-inset` 与 `--runtime-bottom-reserved`，并保持 `--runtime-composer-inset=0px`；软键盘期间不再用键盘抬升量压缩 `runtime-workspace-screen`，确保 workspace header、正文滚动区、空态和 Process 展示保持原高度和原位置。Terminal 的 `.terminal-jump-cluster` 在 `max-width: 760px` 下只消费独立的 Composer footprint；输入框聚焦且软键盘弹起时，`ReactManagedTerminalRouteBody.tsx` 直接隐藏该 overlay，键盘收起和 Composer 回弹完成后再恢复到输入区上沿之上。
...
- 读取本地缓存时先归一残留 `streaming` 消息，消息通过 Chat owner 的 session 详情恢复。
- Chat Web 前端不再使用 Web 流式网关；`ConversationRuntimeProvider.tsx` 只通过 `POST /api/chat/sessions/{session_id}/input` 获取更新后的 runtime session，并通过 Chat owner 的 session 详情恢复请求断开后的最终消息。
...
- 移动端输入区以 CSS 动态视口为有效高度来源，并按 Composer 自身尺寸变化刷新静态 footprint。
- 移动端 root 在窄屏下不再使用 fixed 定位隔离 document 滚动，也不锁定 `html / body / #frontend-root` 的 `overflow:hidden`；前端入口 viewport meta 声明 `viewport-fit=cover, interactive-widget=resizes-content`，让支持该属性的浏览器把虚拟键盘纳入内容视口 resize。App Shell 由 `runtimeKeyboardIsolation.css` 固定为 `height: 100dvh`，运行页和 workspace body 继承该可见视口高度链，不再使用 `calc(var(--mobile-viewport-height) + var(--keyboard-offset))` 参与键盘布局；移动 workbar 通过 fixed top 加无 transition transform 对齐 visual viewport 顶边，Composer 通过 `position: fixed; bottom: var(--keyboard-composer-offset, var(--keyboard-offset, 0px))` 贴住键盘或动态视口底边，避免浏览器工具栏或软键盘变化导致底部留白、内容裁切、页面级上移、顶部 header 位移或重复键盘补偿。
- `shared/viewport/mobileViewportSync.ts` 作为根壳层共享 controller，除常规 `resize / visualViewport.resize / visualViewport.scroll / focusin / focusout` 外，还在 `visibilitychange(visible) / window focus / pageshow` 上强制补做一次同步；浏览器从后台回前台、标签页重新激活或 iOS WebView 恢复可见时，重建视口诊断变量并触发 Composer footprint 复测，主运行页键盘布局仍以 `100dvh` 和 fixed bottom 为准。
- 移动端 Chat 在输入框与会话设置底部面板之间切换时，打开移动端设置面板前先 blur 当前输入，主输入框重新聚焦时则先关闭设置面板，再交给浏览器原生键盘和动态视口完成贴底。
- `runtimeMobileLayout.ts` 是移动端 overlay owner 的唯一判定入口：`mobile-primary-nav-drawer / mobile-session-drawer / mobile-details-dialog / mobile-attachment-preview` 会暂停 portal Composer，`mobile-composer-panel` 由 Composer 自身承载，`mobile-keyboard / mobile-rest` 保持 Composer 可交互。`ConversationWorkspace.tsx` 与 `ReactManagedTerminalRouteBody.tsx` 只传入 `mobileNavOpen / mobileSessionPaneOpen / inputFocused / composerPanelOpen` 等事实状态，不再传递局部 `composerSuspended` 开关。`RuntimeWorkspaceShell.tsx`、`RuntimeWorkspacePage.tsx` 和 `useRuntimeComposerViewportSync.ts` 都消费同一组 helper，避免 route 层、portal 层和 viewport sync 互相覆盖布局状态。
- `useRuntimeComposerViewportSync.ts` 在 Composer 输入框的 `pointerdown / touchstart` 捕获阶段记录 `workspaceBody.scrollTop` 与 `.runtime-workspace-screen.scrollTop`，并在短暂键盘动画稳定窗口内恢复该快照，抵消 iOS Safari 原生 focus 对后方滚动容器的自动推移。该逻辑不调用 `window.scrollTo`，也不在稳定窗口结束后继续恢复滚动；用户在消息区产生新的 `touchstart / pointerdown / touchmove / pointermove / wheel / scroll` 时会取消待执行恢复，避免 iOS 真机 scroll 事件晚到时被后续恢复拉回。
- `useRuntimeComposerViewportSync.ts` 同时在 `inputFocused && isMobileViewport` 时为 `.runtime-workspace-screen` 安装 touch-scroll bridge：`touchstart` 记录单指 identifier 与起始 Y，`touchmove` 按 Y 轴差值直接更新 `scrollTop` 并阻止默认行为，`touchend / touchcancel` 清理状态；未聚焦时不接管，保留浏览器原生滚动。该 bridge 专门覆盖 iOS Safari 在软键盘、body-level fixed Composer、嵌套 overflow 滚动容器同时存在时可能不派发原生滚动的问题。
- `runtimeKeyboardIsolation.css` 在移动端把 `.runtime-composer-shell`、portal shell 和 `.runtime-composer-form` 标记为 `pointer-events: none`，再让 `.runtime-composer-input / input-assist / utility / upload / submit / attachment / panel` 等真实控件恢复 `pointer-events: auto`；这样底部 Composer 的透明渐变、外层 padding、body-level fixed shell 和输入胶囊空白 surface 不会截获后方消息区的 touch scroll，真正的输入、工具按钮、附件预览、发送按钮和配置面板仍保持可交互。
...
- `src/app/routeState.ts` 负责运行页路由与会话 query 协调：路由只解析 canonical path，不再使用 hash fragment；`/chat` 与 `/terminal` 统一识别 `session_id` 多会话恢复参数，写入时通过 `sessionRouteToken` 把完整会话 id 收敛为 8 位短 hash。主导航进入 `Chat` 时即使当前已经位于 `/chat`，也会删除旧 `session_id` 并派发路由同步事件；`ConversationRuntimeProvider.tsx` 监听该同步事件，在 query 缺失时切到当前会话列表第一项。`/chat` 兼容映射到 `/chat`，`ConversationRuntimeProvider.tsx` 在 Chat 初始化时只读取 Chat owner 的 session 集合，并把本地旧 `chat` snapshot 合并到 Chat 列表。`ReactManagedTerminalRouteBody.tsx` 对 Terminal 采用同一短 hash 策略。Terminal query 缺失或目标会话不存在时，运行页才回退到 `sessionStorage` 快照与服务端列表默认项。
...
- `internal/interfaces/web/frontend/src/features/shell/components/runtimeSessionApi.ts` 是 Chat / Terminal 前端会话 API 地址与分页参数的单一来源，集中生成 collection、recover、detail、input、pin、attachments 与 event detail endpoint，并导出共享历史补页 turn limit。`runtimeSessionController.ts` 在其上承载共享会话状态机和网络动作，统一处理列表、详情刷新、创建、删除、置顶、输入、附件上传、事件详情、轮询判定与渐进补历史；`ConversationRuntimeProvider.tsx` 与 `ReactManagedTerminalRouteBody.tsx` 都直接消费同一 controller。`runtimeSessionViewModel.ts` 是 Chat / Terminal turns、`turns_paging` 与时间线 message 的统一视图模型，集中提供 turn 排序、turn 合并、paging 合并、runtime event 归一化与 `runtimeSessionTurnsToTimelineMessages` 转换；prompt-only、running、final output 与 process events 在这里统一落成同一种 `RuntimeSessionTimelineMessage`。`buildRuntimeSessionTimelineItems` 是 Chat / Terminal 时间线 item 构造的唯一入口，两个页面都只把共享 timeline message 传入该 builder，不再保留私有 timeline item builder 或页面私有 turn-to-message 转换器。`runtimeSessionCatalogs.ts` 统一加载 model provider、Skill 与 MCP catalog，Chat / Terminal 不再各自维护 catalog 请求链路。
...
- `ReactManagedTerminalRouteBody.tsx` 维护模块级 `terminalRuntimeCache`，专门覆盖同一 SPA 工作台内从 Terminal 切到其他路由再返回的个人单设备恢复场景。缓存保存排序后的会话列表、活动会话 id 和每个会话完整已加载 turns，写入时通过共享 `cloneRuntimeSessionViewSession` 复制 turn/runtime event/attachment 数据，不裁剪已加载历史，读取时按 24 小时 TTL 校验；命中时直接作为 `useTerminalRuntimeController` 初始状态并跳过空白 loading 首屏，随后原有 `/api/terminal/sessions` 与单会话详情请求继续回源并通过共享 `mergeRuntimeSessionViewSession` 合并覆盖。当前活动会话详情若返回 `turns_paging.has_more_before`，共享 controller 会用 `turns_paging.next_before_turn_id / oldest_turn_id` 自动带 `turn_before` 和 `turn_limit=20` 后台补拉更早页，分页、输入返回、轮询和 page-activation 结果都按 turn id 合并并刷新缓存时间；后台补页不得覆盖当前输入、详情面板、选择态或主动阅读滚动，同一 `session_id + turn_before` 边界已完成补拉后不会再次触发，只有分页边界推进后才继续下一页。缓存不写入 `localStorage`，不作为跨刷新或跨设备事实来源；删除、置顶、输入返回、列表刷新和详情刷新仍通过 React state 统一回写缓存。
...
- `WorkbenchApp` 继续安装共享 `mobileViewportSync` controller 输出诊断与兼容变量，但 Chat / Terminal 主运行页布局不再依赖这些变量驱动后方布局位移；移动端 `root.css` 在 `1100px` 及以下解除 `html / body / #frontend-root` 的 fixed 页面锁和根层 `overflow:hidden`，改用 `min-height: 100dvh`。`runtimeKeyboardIsolation.css` 在 `shell.css` 之后把 App Shell 改为动态视口容器，直接用 `height: 100dvh` 控制高度。真手机宽度下，真实移动 workbar 从 workspace grid 的普通流里脱离并固定在 visual viewport top，workspace body 只保留 header footprint；真实 Composer 从 workspace body 和 workspace grid 中脱离，经 `document.body` 顶层 portal 成为 fixed 浮层，通过 `--keyboard-composer-offset` 贴住键盘或动态视口底边；正文 panel 是唯一内部滚动区。运行页后方层不让 document 或普通流布局承接 iOS 聚焦滚动，也不再通过 transform 处理 offsetTop；`useRuntimeComposerViewportSync` 不接管主输入框首触，不回放页面级滚动锚点，也不锁定输入框后方的 workspace 滚动容器。命令候选、配置面板和阅读定位控件保持静态位置，左侧抽屉打开时 portal Composer 暂停并释放输入焦点。
- Conversation runtime 的窄屏工作区网格仅承载移动 workbar、workspace header、正文 panel 与静态 Composer spacer；真实 Composer 作为 body-level fixed portal 独立覆盖在工作区上方，避免 Chat 或 Terminal 复用共享 surface class 时被错误套用普通 grid 子项布局，导致长历史输出把 Composer 挤出屏幕或让背景随键盘聚焦平移。

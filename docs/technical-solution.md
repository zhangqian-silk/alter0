...
- `internal/interfaces/web/frontend/src/shared/visibility/usePageActivation.ts` 是 Chat 的共享页面激活补偿入口，监听 `focus`、`visibilitychange`、`pageshow` 与 `online`；Provider 会先检查本地是否存在 `local_running / recovering`、缓存不完整或可恢复占位，稳定 `ready` 会话直接跳过该次激活，不刷新 owner 列表、当前详情或 updates。需要恢复时再按 `latest_update_id` 触发 owner 增量轮询，并在 `resync_required` 或缓存不完整时触发必要快照回源，通过短 debounce 合并同一恢复阶段的重复事件。
...
- `ConversationRuntimeProvider.tsx` 维护模块级 `conversationRuntimeCache`，专门覆盖同一 SPA 工作台内从 Chat 切到其他路由再返回的个人单设备恢复场景。缓存按 route 保存会话列表、活动会话 id 和每个会话完整已加载消息或 turns，写入时复制消息、附件和 process events，读取时按 24 小时 TTL 校验，且只接受 canonical Chat session id；命中时作为 Provider 初始状态，随后当前 owner 的集合接口继续回源并沿现有合并规则覆盖。当前活动会话若已经带有完整稳定消息缓存，仍只作为首屏加速来源；首次进入和刷新会回源当前 active 会话的最新 bounded detail，普通 page activation 只有在本地存在 `local_running / recovering`、缓存不完整或可恢复占位时才回源，稳定 `ready` 缓存直接短路。Provider 同步写入当前 route 的 24 小时 `localStorage` 完整消息快照，用于刷新、重开或 `sessionStorage` 丢失后的首屏恢复；同时写入当前 route 只含会话元数据的轻量快照，用于完整消息快照超出浏览器配额或被清理后的会话列表兜底。旧 `alter0.web.session.snapshot.v1` 与 `alter0.web.session.recent.v1` 启动时直接清理，不迁移到当前缓存。完整快照写入时统一序列化为稳定存储字段，消息过程使用轻量 `runtime_trace_events`；已通过 event detail 接口加载的 `blocks` 必须写入长期快照，但后续轻量摘要不得覆盖这些 blocks。持久快照读取时先按 route 校验 TTL，再按 session 合并完整快照与轻量快照：完整快照保留消息、过程事件、附件、分页状态、`updated_at` 与本地详情新鲜度，轻量快照只补齐完整快照缺失的会话摘要，不得把已加载消息降级为空。本地快照不作为跨设备事实来源，删除、置顶、输入返回、列表刷新和详情刷新仍通过 React state 统一回写当前 route 缓存并等待服务端回源确认。
...
- 前端首次进入与刷新恢复顺序固定为：读取本地 route 快照渲染首屏；拉取 owner 会话列表；会话列表刷新只把远端返回项作为 summary patch，远端空 `items` 或短列表不会删除本地仍未确认删除的 session，也不会清空 active session；对当前 active server session 补拉最新 bounded detail 校准正文、状态与 `runtime_trace_events`。普通 page activation 先检查本地同步意图和缓存完整性，稳定 `ready` 直接跳过；历史分页、手动刷新、updates 返回 `resync_required` 或本地 `local_running / recovering` 仍未收敛时继续按对应会话补拉详情；随后按当前 owner `latest_update_id` 请求 updates。该顺序保证刷新过程中即使网络短暂失败，也不会把本地 `Thinking...` 或 user-only turn 直接收敛为失败，也不会把常规增量轮询退化为全量详情轮询。

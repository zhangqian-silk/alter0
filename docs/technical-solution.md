...
- `ReactManagedTerminalRouteBody.tsx` 通过 `usePageActivation` 维护 Terminal 的前后台恢复：页面隐藏时仅更新 `pageHidden` 供轮询降频使用；页面重新可见或重新获得焦点后，会先刷新会话列表，再在存在活动会话时捕获当前滚动锚点并回源活动会话详情，避免后台期间新增输出、标题变化或状态切换在恢复前台后继续停留在旧视图。

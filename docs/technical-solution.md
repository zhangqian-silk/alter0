...
- 前端组件测试需覆盖 React 工作台的稳定契约，至少校验 `WorkbenchApp` 的 hash 路由、语言切换、移动端导航收口，以及 Conversation / Terminal workspace 的固定 header、会话列、消息区、Composer 和 `Details` 面板未被回归破坏；Conversation 消息区还需覆盖轻量 IM 气泡 DOM 与样式契约、长历史最新优先渲染与加载更早批次。

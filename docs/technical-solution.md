...
- `internal/interfaces/web/frontend/src/features/conversation-runtime` 承载共享对话运行态 UI：`ConversationRuntimeProvider.tsx` 负责复用 Terminal session API 的数据模型和状态机，按 route 通过 `/api/chat/sessions` 或 `/api/terminal/sessions` 完成会话创建/切换/删除/置顶、input 输入提交、结果收口、文本草稿与附件草稿恢复、模型与能力项选择。`RuntimeRouteHost.tsx` 将 `/chat` 与 `/terminal` 都挂载到 `ConversationRuntimeProvider + ConversationWorkspace`，但通过 `key={route}` 和真实 `route` 保证 controller、API owner、active session、缓存和草稿命名空间彼此独立；`/terminal` 不再挂载 `ReactManagedTerminalRouteBody`。`ConversationWorkspace.tsx` 不再渲染 Skill 选择器、Deliverables、Session Profile 或独立 Skill 面板；历史会话的 `target_type=runtime` 仅作为会话摘要元数据保留。Chat 仍在常规模型 Provider 列表外额外注入前端内置 `Codex` provider，Skill 选择在发送时通过当前 owner input 的 `skill_ids` 字段提交。运行页会话列表与消息详情通过当前 route 的 session 接口恢复，浏览器只继续保存当前 route 的未发送草稿、附件草稿和当前页局部 UI 状态；Chat owner 与 Terminal owner 的会话列表互不包含。单会话详情若携带 `turns_paging.has_more_before`，Provider 会先按消息 id 合并当前分页片段，再使用 `turns_paging.next_before_turn_id / oldest_turn_id` 继续请求 `turn_before` 后台补齐更早 turns；无重叠片段按消息时间判断前置或追加，避免 page-activation、轮询或输入返回的短 turns 页覆盖已加载历史。Provider 将时间线/会话 workspace 上下文与 Composer 上下文分离，后台 `turn_before` 补页只更新消息集合；Composer 上下文只消费稳定草稿、附件和运行配置字段，避免未变化的输入区随历史合并刷新。Provider 的模块级运行态缓存保留完整已加载消息并按 24 小时 TTL 失效；完整会话消息快照、轻量会话信息快照、active session、文本草稿、附件草稿与过程披露过滤均使用 route 独立 `sessionStorage / localStorage` key，旧合并快照只作为读取迁移 fallback。

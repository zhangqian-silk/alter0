...
- `internal/interfaces/web/server.go` 的 `renderLoginPage` 继续由服务端直出统一登录页，但页面高度契约切到 `100dvh + viewport-fit=cover`：`body` 禁止整页额外滚动，外层容器按 `env(safe-area-inset-top/bottom)` 收口，登录卡片在正常真机高度下保持单屏居中；仅在极小可视高度下允许卡片内部滚动，避免地址栏或安全区导致的顶部/底部空白与整页拖动。
...
- `cmd/alter0/builtin_skills.go` 负责注册内置 Skill，并在启动阶段校验所有 file-backed 内置 Skill 文件存在；当前 `deploy-test-service` 使用 `.alter0/skills/deploy-test-service/SKILL.md`，`frontend-design` 使用源码目录下的 `docs/skills/frontend-design/SKILL.md`，`artifact-preview` 使用 `.alter0/skills/artifact-preview/SKILL.md` 并通过同目录下的 `scripts/publish_preview_artifact.sh` 复用 workspace service 网关发布静态预览页。

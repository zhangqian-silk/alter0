...
- `shared/visibility/usePageActivation.ts` 提供运行页共享的 page-activation hook：统一监听 `visibilitychange`、`focus`、`pageshow` 与 `online`，并用短时间去抖吸收同一恢复阶段的连续触发。`ConversationRuntimeProvider.tsx` 与 `ReactManagedTerminalRouteBody.tsx` 都复用这条链路；Conversation 在页面重新变为前台可见时补偿刷新 Chat 会话列表、当前活动会话详情与 pending task 状态，页面隐藏时通过 `resolveChatTaskPollPlan` 停止 pending task 定时轮询；Terminal 则立即刷新会话列表与当前活动会话详情。

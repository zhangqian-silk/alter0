...
- `ConversationRuntimeProvider.tsx` 的发送链路始终确保存在当前 route owner 的 runtime session，再调用对应 owner 的 input 接口，并用返回的 session 或后续详情恢复生成本轮 user / assistant 消息；前端不再创建本地 running process step，不再解析 SSE。发送前只拦截真实运行中的同会话输入，已失败或中断的历史 turn 即使没有 assistant 正文也不会阻断新的 input。收到 input 结果、错误或恢复详情后同步收口最终状态，迟到结果不得重新打开已完成消息或覆盖最终正文。

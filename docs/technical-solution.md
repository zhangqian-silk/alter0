...
- `internal/interfaces/web/frontend/src/features/conversation-runtime` 承载 `chat / agent-runtime` 的运行态：`ConversationRuntimeProvider.tsx` 负责会话创建/切换/删除、消息流、SSE 收口、任务轮询、文本草稿与图片附件草稿恢复、模型与能力项选择；`chat` 路由会在常规模型 Provider 列表外额外注入前端内置 `Codex` provider，选中后通过统一的 message metadata builder 把请求改写为 `alter0.execution.engine=codex`，不再附带普通 `alter0.llm.provider_id / alter0.llm.model` 组合。`composerImageAttachments.ts` 负责图片文件读取、类型/体积约束、上传前原图/预览图生成；`ConversationWorkspace.tsx` 已退化为 route wrapper，`useConversationRuntimeController()` 负责把 provider 状态映射成共享运行页 controller 并注入 `RuntimeWorkspacePage.tsx`，不再直接拼装 session pane、workspace header、timeline 与 composer。用户在 Composer 里新增图片时，前端会先调用 `POST /api/sessions/{session_id}/attachments` 把原图与缩略预览写入当前 Session 工作区，再在消息请求、草稿恢复和最近会话里只保存 `asset_url / preview_url` 引用，避免在 `sessionStorage` 中反复堆积原始大图 `data_url`；assistant 最终回复里的 markdown 外链图片则在服务端结果出站前下载到同一 Session 附件目录并改写成本地 URL，让当前回复和历史回放都复用同一套本地图片资产；`chat` 与 `agent-runtime` 空态都会抑制重复的 workspace summary row，把标题与说明完全交给欢迎区承接。运行页本身输出 `data-conversation-view / data-conversation-session-pane / data-conversation-workspace / data-conversation-chat-screen / data-conversation-inspector` 等稳定锚点，不再通过 bridge 或 snapshot store 回写业务状态。
...
- Web 上传的图片附件经 `internal/interfaces/web/server.go` 与 `session_attachment_store.go` 规范化后写入 `alter0.user_input.image_attachments` 元数据；Conversation runtime 会先通过 `/api/sessions/{session_id}/attachments` 把原图与预览图落到 `.alter0/workspaces/sessions/<session_id>/attachments/<asset_id>/`，消息接口随后只携带 `id + asset_url + preview_url` 引用，服务端再解析出工作区内的原图路径写入元数据。assistant 最终回复中的 markdown 外链图片则由 `internal/orchestration/application/session_output_image_assets.go` 在 SessionPersistenceService 中做结果后处理：仅对可下载的 `http(s)` 图片做抓取，写入同一 Session 附件目录，并把 `result.Output / result.ProcessSteps[].Detail` 里的图片地址改写为 `/api/sessions/{session_id}/attachments/{asset_id}/original`。`/api/messages`、`/api/agent/messages`、Terminal 输入与 Control Task follow-up 输入都会复用同一附件目录与交付 URL。`internal/execution/infrastructure/hybrid_nl_processor.go` 会把这些附件解码成 `llmdomain.Message.Parts`，若元数据里携带的是工作区路径则在执行前读盘恢复为模型可消费的 `data:` URL，与文本 part 一起交给 ReAct / 直连模型链。带图请求不进入异步 Task，也不会在模型链失败后静默回退到 Codex CLI，避免把视觉请求错误降级为纯文本执行。
...
- `internal/interfaces/web/frontend/src/features/shell/components/ReactManagedTerminalRouteBody.tsx` 与 `ReactManagedTasksRouteBody.tsx` 共同复用 `composerImageAttachments.ts` 处理图片读取、缩放和 data URL 生成；两条链路在前端选图后都会先调用 `POST /api/sessions/{session_id}/attachments` 把图片写入当前 Session 工作区，再在草稿、预览与提交体里优先保留 `id + asset_url + preview_url` 引用，避免 Terminal 首发会话与 Task follow-up 把原始 `data_url` 长期留在浏览器状态里。`internal/interfaces/web/server_terminal.go` 直接把 Terminal `attachments[]` 转成 `terminalapp.InputRequest.Attachments`，而 `controlTaskTerminalInputHandler` 则把 follow-up 输入编码进统一消息元数据后交给 Task 服务。

...
func TestSnapshotIncludesProviderPolicyIncidentsAndAlert(t *testing.T) {
	now := time.Now().UTC()
	traceBase := t.TempDir()
	dayDir := filepath.Join(traceBase, now.Format("2006-01-02"))
	if err := os.MkdirAll(dayDir, 0755); err != nil {
	...
	entries := []string{
		fmt.Sprintf(`{"timestamp":"%s","channel_id":"telegram","event":"agent_process","status":"error","detail":"oauth token revoked: account restricted"}`, now.Add(-2*time.Minute).Format(time.RFC3339Nano)),
		fmt.Sprintf(`{"timestamp":"%s","channel_id":"telegram","event":"agent_process","status":"error","detail":"rate limit exceeded: quota reached"}`, now.Add(-90*time.Second).Format(time.RFC3339Nano)),
		fmt.Sprintf(`{"timestamp":"%s","channel_id":"slack","event":"agent_process","status":"error","detail":"policy denied by provider compliance"}`, now.Add(-30*time.Second).Format(time.RFC3339Nano)),
	}
	if err := os.WriteFile(filepath.Join(dayDir, "gateway_events.jsonl"), []byte(strings.Join(entries, "\n")+"\n"), 0644); err != nil {
		t.Fatalf("write trace file failed: %v", err)
	}
	...
	if policy.Status != "critical" {
		t.Fatalf("expected policy incident status critical, got %#v", policy.Status)
	}
	if policy.Signals != 3 {
		t.Fatalf("expected 3 provider policy signals, got %#v", policy.Signals)
	}
	if policy.ByCategory["access_blocked"] != 1 || policy.ByCategory["quota_limited"] != 1 || policy.ByCategory["policy_denied"] != 1 {
		t.Fatalf("unexpected policy categories: %#v", policy.ByCategory)
	}
	if policy.ByChannel["telegram"] != 2 || policy.ByChannel["slack"] != 1 {
		t.Fatalf("unexpected policy channels: %#v", policy.ByChannel)
	}

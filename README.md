...
当前优先级以 [`features.md`](./features.md) 为准，发布节奏按 release-gate 管控（回归矩阵、风险基准、回滚演练、部署检查、文档同步）；当前队列位于 P2（下一项 N18：场景基准矩阵与竞品追踪自动化）。
...
该检查默认对比 `origin/master...HEAD`，若只改了 `docs/features.md` 会直接失败，避免能力矩阵与总览文档漂移。`make integration-matrix` 用于执行网关级集成矩阵（多通道、多 Agent、子代理、定时任务、工具与故障注入）；`make risk-benchmark` 用于执行 `provider_policy/supply_chain` 风险巡检基准并生成 `output/risk/benchmark-latest.json`。`make release-gate` 会串联配置边界门禁（`make check-config-boundary`）、测试稳定性门禁（含 `make test-stability` 与 Windows 编译回归）、集成矩阵、风险基准门禁、回滚演练、部署资产检查与文档门禁。

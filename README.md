...
当前桌面工作台基线收敛为两层：左侧品牌导航保持全高固定栏，右侧主面板统一承载运行页和控制页。`Chat / Agent Runtime` 的产品体验向 `Terminal` 靠齐，统一采用「会话列 + 主时间线工作区 + 底部 Composer + 可切换 Inspector」的工作台结构；进入 `1100px` 及以下窄屏后，`Chat / Agent Runtime` 与 `Terminal` 都在工作区头部固定提供 `Menu / Sessions / New` 操作行，会话列切为独立抽屉，不再和正文上下堆叠。三条运行页共享同一套 terminal 风格的会话列、标题区、滚动区和 icon submit composer 语义；`Terminal` 继续保持原有布局关系、交互逻辑和 `terminal-*` DOM class 契约不变，仅内部实现改为更纯粹的 React state 驱动。控制类页面继续复用近白主表面、浅灰说明层和浅蓝选中态，不再派生独立的高装饰卡片系统；头部导航入口仅在 `1100px` 及以下切换为抽屉式交互，`760px` 及以下再压缩按钮与间距，保证真手机宽度下的可触达性。

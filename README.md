...
当前桌面基线是浅蓝轻科幻风格的三栏 cockpit：左侧主导航仅保留 `Alter0` 服务名与模块组，中栏承载新建入口与会话历史，右侧主工作区承载欢迎区/消息区、页面 hero 与固定底部输入面板；整体采用浅蓝渐变背景、低对比玻璃感面板与冷色高亮。为避免重构后信息重复，当前壳层收敛为单层信息架构：主导航品牌区不再展示图形标识、品牌口号或实现状态；会话栏不再重复输出整块路由概览，Chat 主区不再额外叠加 workspace hero，页面类路由只保留一层主标题说明。桌面端聊天阅读区与底部输入区继续统一限制在 `960px` 最大宽度内并居中，但外层三栏 cockpit 会按主视口宽度自适应收缩侧栏、间距与工作区宽度，避免只剩内容列限宽而整体壳层失衡；头部 `Menu / Sessions / New` 入口在桌面宽度默认隐藏，仅在主视口宽度 `1100px` 及以下时才收敛为主工作区头部按钮，把主导航与会话历史切换为贴边抽屉工作台，避免在低宽度桌面分辨率下把会话列表悬浮成独立桌面卡片；`Agent` 运行页也在同一阈值同步收敛为单列，避免壳层与页面主体断点错位。继续缩到 `760px` 及以下时，抽屉宽度、头部按钮高度与间距进一步压缩，保持真手机宽度下的可触达性。
...
- `Chat / Agent` 的前端壳层采用统一 cockpit 信息架构：左侧品牌导航仅保留 `Alter0` 服务名，中列承载会话列表，工作区 hero 用于承接当前会话定位，欢迎区内提供 target picker 与 prompt deck，输入区独立固定在底部；欢迎区、消息区与输入区共享居中的 `960px` 阅读宽度，欢迎区内容超出高度时只在内容区内部滚动，不与输入面板叠层。
...
- 会话侧栏中的 Session 卡片采用紧凑单卡布局：标题允许两行截断，摘要信息与短 hash 作为辅助信息收纳在同一卡片内，复制与删除操作收敛为卡片底部的轻量图标按钮，避免长标题把列表结构挤坏。

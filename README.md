...
当前桌面基线是浅蓝轻科幻风格的三栏 cockpit：左侧主导航仅保留 `Alter0` 服务名与模块组，中栏承载新建入口与会话历史，右侧主工作区承载欢迎区/消息区、页面 hero 与固定底部输入面板；整体采用浅蓝渐变背景、低对比玻璃感面板与冷色高亮。为避免重构后信息重复，当前壳层收敛为单层信息架构：主导航品牌区不再展示图形标识、品牌口号或实现状态；会话栏不再重复输出整块路由概览，Chat 主区不再额外叠加 workspace hero，页面类路由只保留一层主标题说明。桌面端阅读区与底部输入区统一限制在 `960px` 最大宽度内并居中，优先保证长对话可读性；主视口宽度高于 `960px` 时继续保留桌面三栏布局，只有在 `960px` 及以下时才切换为主导航抽屉 + 会话抽屉的单列工作台，继续复用同一 React 壳层和 bridge 契约。
控制类与资产类页面默认采用更高信息密度的管理视图：`Profiles` 使用短字段并排的紧凑表单栅格与显式启用开关，`Tasks` 使用左侧任务列表 + 右侧运行详情的主从布局，`Memory` 的任务历史使用表格 + 详情侧栏，`Environments` 使用配置表 + 详情面板查看键、值、模块与说明，`Codex Accounts` 使用账号卡片列表 + 导入/登录表单查看套餐、额度状态与当前活动账号；这些页面在较窄宽度下再回落为单列堆叠布局。

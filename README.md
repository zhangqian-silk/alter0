...
- Chat 会话接口统一返回 `Cache-Control: no-store`，浏览器会话数据只由应用缓存管理。列表接口固定返回 `id / title / status / created_at / updated_at / pinned`；详情的 summary、最新 turns 页和分页边界来自同一个原子快照。输入可携带 `client_request_id`，服务端会把它原样保存在对应 turn，供刷新后的 optimistic message 对账。

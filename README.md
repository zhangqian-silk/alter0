...
当前 Web Shell 已由 React 接管稳定运行链路：`LegacyWebShell` 负责主导航、会话栏、工作区壳层与 DOM 契约，`ReactRuntimeFacade` 负责 Chat / Agent 的会话状态、消息流、运行时面板、移动端 runtime sheet、草稿恢复与 bridge 事件处理，`agent / terminal / products / memory / channels / skills / mcp / models / environments / codex-accounts / cron-jobs / sessions / tasks` 十三类路由页也由 React 直接请求控制台或会话 API 并渲染；其中 Terminal 已改为 React 原生页面实现，不再通过 legacy host 挂载旧控制器。Terminal 当前仅沿用旧版 `terminal-*` DOM class 与布局关系作为样式基线，会话栏、工作区头、Process、输出区和 Composer 仍全部由 React state 驱动。React 继续在 `routeBody` 上输出当前页 `data-react-managed-route` 标记，并在 `appShell` 上输出稳定的 `data-react-managed-routes` 清单，确保 shell 状态更新不会清空页面主体 DOM；`legacy` 资源仅保留样式兼容层，不再参与 `/chat` 的脚本启动。
当前桌面基线对齐主仓库的 legacy shell：左侧主导航采用全高固定栏，品牌区仅保留 `Alter0` 服务名与折叠入口；中栏承载 `New Chat` 与会话历史；右侧主工作区保留旧版 header、欢迎区/消息区、路由页 `route-head` 与独立 Composer。React 不再把三栏壳体渲染成独立胶囊面板，也不再额外挂载 workspace hero、route hero 或 prompt deck 容器；Chat 欢迎区直接输出 welcome tag、标题、副标题、target 列表与 prompt pills，页面类路由直接使用 `route-head` 标题区。桌面端聊天阅读区与底部输入区继续统一限制在 `960px` 最大宽度内并居中，但外层三栏保持主仓库式全宽工作台；头部 `Menu / Sessions / New` 入口在桌面宽度默认隐藏，仅在主视口宽度 `1100px` 及以下时切换为贴边抽屉入口，`760px` 及以下再进一步压缩抽屉宽度与按钮尺寸，保证真手机宽度下的可触达性。
`/chat` 与 `/login` 默认以英文文案和 `html[lang="en"]` 启动；Web Shell 内可通过语言切换入口改为中文，运行时文案与 `document.documentElement.lang` 同步更新。
控制类与资产类页面默认采用更高信息密度的管理视图：`Profiles` 使用短字段并排的紧凑表单栅格与显式启用开关，`Tasks` 使用左侧任务列表 + 右侧运行详情的主从布局，`Memory` 的任务历史使用表格 + 详情侧栏，`Environments` 使用运行态工具栏 + 模块卡片栅格展示配置项，并在同页提供敏感值显隐、保存、重载、重启与审计视图，`Codex Accounts` 使用运行时概览条 + 托管账号卡片列表 + 导入/登录操作侧栏展示当前 auth 路径、托管数量、套餐、额度状态与登录会话；大屏保留“列表 + 侧栏”结构，中屏切换为全宽账号区 + 双侧栏，小屏回落为单列卡片，确保额度信息与切换按钮始终可见。
...
- `Chat / Agent` 的前端壳层对齐主仓库 legacy shell：左侧品牌导航仅保留 `Alter0` 服务名与折叠入口，中列承载新建入口与会话列表，主工作区使用旧版 header、欢迎区/消息区与独立 Composer；欢迎区直接输出标题、副标题、target picker 与 prompt pills，不再额外挂载 workspace hero、route hero 或 prompt deck 包裹层；欢迎区、消息区与输入区共享居中的 `960px` 阅读宽度。
- `/chat`、登录页和主工作区品牌文案对外统一展示为 `Alter0`，浏览器标题、登录标题、导航品牌位、会话栏标题与欢迎区 tag 不再混用小写服务名。
- `Chat / Agent` 头部的 `Menu / Sessions / New` 入口只在窄屏壳层中承担收敛后的主操作：桌面三栏时隐藏，`1100px` 及以下时作为主导航与会话历史抽屉入口显示，`760px` 及以下再切到更紧凑的按钮和抽屉宽度；`info-mode` 页面默认隐藏 `Sessions` 按钮，Terminal 页面保留会话入口。
- 会话侧栏中的 Session 卡片沿用主仓库式紧凑结构：标题保持单行省略，摘要信息单独占一行，短 hash 与复制/删除入口收纳在卡片下缘，避免把列表拆成额外的胶囊操作区。
...
- 移动端 `Chat / Agent` 在输入框与“会话设置”之间切换时，会主动消除键盘与设置面板的双重叠层：打开设置前先释放输入焦点并回收键盘占位，回到主输入框时则先收起设置面板，再贴底对齐输入区。
...
- Chat 与各路由消息流统一采用浅蓝轻科幻阅读主题：用户消息保留右对齐并限制在消息区宽度的 80% 内，消息气泡、流程卡与正文区统一使用冷色低对比边框、浅蓝玻璃感底色和更克制的阴影层级；正文排版优先于装饰层级，用户消息与其后续回复继续保持更紧凑的同轮分组间距。
...
- Web Shell 主工作区的首屏内容保持紧凑起始区：桌面与中宽度下的 `Chat` 空态欢迎区会在 header 与 composer 之间沿主工作区中轴做竖向居中，欢迎 tag、标题、描述与 prompt 统一围绕欢迎区中线排布；真窄屏继续贴近头部下沿起排，输入区不再依赖大块弹性留白把首屏内容拉散。普通 `page-mode` 路由页 `route-head` 与 `Terminal` 工作区继续沿用“两行头部 + 贴顶正文起始区”基线，而 `Chat` 空态在中窄屏仅保留 `Menu / Sessions / New` 操作行，由欢迎区承接标题与说明，避免头部标题与欢迎区内容重复叠层。
- Terminal 路由直接进入工作区，不再在工作区上方额外渲染页面级说明 hero；顶部通用栏保留 `Menu`，并在同一行右侧提供 `Sessions / New`，工作区头部收敛为会话标题、状态点与紧凑操作工具栏；发送区使用自适应输入框与紧凑发送按钮，运行态退出或中断提示以内嵌状态条贴合输入区上缘展示；消息与 `Process` 头部保持自然文档流滚动，不再启用吸顶导航；阅读定位由右侧圆形四键组承担，支持回到顶部、上一条、下一条与回到底部，并统一为浅色低对比滚动条与阅读主题；软键盘收起或浏览器底部工具栏回弹后，底部输入条需立即回贴可见底边，不保留额外占位空白。
...
- Terminal 的整体视觉语言与 Chat 收敛：会话侧栏、工作区容器、输入区与输出块统一使用浅蓝轻科幻主题下的低对比边框、柔和玻璃感背景和更大的圆角节奏；用户输入不再额外展示命令前缀符号或强调色，最终回复直接按 Chat 助手消息样式渲染；`Process` 保留低对比虚线提示区和左侧纵向时间线，内部步骤压缩为单行摘要，展开后只展示详细内容，不再重复状态标签。

...
当前桌面工作台基线收敛为两层：左侧品牌导航保持全高固定栏，右侧主面板统一承载运行页和控制页。`Chat / Agent Runtime` 的产品体验向 `Terminal` 靠齐，统一采用「会话列 + 主时间线工作区 + 底部 Composer + 可切换 Inspector」的工作台结构；进入 `1100px` 及以下窄屏后，`Chat / Agent Runtime` 与 `Terminal` 都在工作区头部固定提供 `Menu / Sessions / New` 操作行，会话列切为独立抽屉，不再和正文上下堆叠；普通 `page-mode` 路由页也会在标题上方补一行 `Menu` 入口，保证进入 `Tasks / Sessions / Models` 等信息页后仍能直接拉起主导航。三条运行页共享同一套 terminal 风格的会话列、标题区、滚动区和 icon submit composer 语义；会话侧栏按最近时间分组为 `Today / Yesterday / Earlier`，条目统一收敛为「状态 / 标题 / 摘要 / 底部短 hash / 尾侧删除」的工作台列表项；`Terminal` 继续保持原有布局关系、交互逻辑和 `terminal-*` DOM class 契约不变，仅内部实现改为更纯粹的 React state 驱动。控制类页面继续复用近白主表面、浅灰说明层和浅蓝选中态，不再派生独立的高装饰卡片系统；头部导航入口仅在 `1100px` 及以下切换为抽屉式交互，`760px` 及以下再压缩按钮与间距，保证真手机宽度下的可触达性。
...
- `Chat / Agent Runtime` 统一使用 terminal-style workspace：左侧主导航之外，运行页内部固定为会话列、主时间线工作区、底部 Composer 与可切换 Inspector；消息、过程步骤与最终输出都落在同一时间线中推进，不再保留桥接期的欢迎区 / runtime sheet 双轨壳层。会话列、workspace、chat screen 与 composer 直接复用 `terminal-*` 语义 class，保证与 `Terminal` 新版 UI 保持同一表面、标题区和输入区节奏。
...
- `Chat / Agent Runtime` 首页 Composer 收敛为单一紧凑输入面：桌面与移动端都压缩外层留白、输入高度和发送按钮体量，发送按钮直接复用 Terminal 的圆形 icon submit 皮肤，输入区、计数 meta、发送按钮、会话卡片和 Inspector 面板沿同一套浅色 terminal-runtime 皮肤出图，不再混用默认 terminal footer slab 与旧式轻表单观感；空态工作区不允许保留可拖拽滚动，把头部和输入区顶离可视区。
...
- 预览短哈希 host 与主域工作台共用同一套登录保护；访问 `https://<session_short_hash>.alter0.cn` 时可直接打开该 host 自身的 `/login` 登录页，后续稳定行为会把登录 cookie 共享到 `*.alter0.cn`，避免主域与预览 host 各自重复登录。
...
- Terminal 窄屏工作区头部不再重复渲染第二枚 `Sessions` 按钮；会话抽屉入口统一由壳层头部承接，工作区操作栏只保留 `Details` 等当前会话阅读动作。
- Terminal 工作区头部在真手机宽度下允许标题、状态与 `Details` 工具栏自适应换行；长标题优先保留可读性，不允许把操作按钮挤出可见宽度。
...
- Terminal 桌面端继续沿用旧版 master-detail 布局关系作为阅读基线：左侧会话列表显示标题、最近输出与状态徽标，右侧工作区头部收敛为会话标题、状态点与 `Details` 工具栏；这套布局由 React 组件直接渲染，不再依赖旧版脚本控制。

...
- 移动端 `Chat` 输入区在软键盘弹起、收起与可视视口高度变化期间，会基于 `VisualViewport` 同步有效视口高度；App Shell 在键盘弹起时保持稳定基线高度，底部输入 form 只消费 `--keyboard-composer-offset` 贴住可见底部，Composer 外壳、正文滚动区和背景层不再按键盘高度动态位移或收缩。浏览器工具栏状态切换、输入聚焦或正 `offsetTop` 不会把整个工作区重复顶起；键盘抬起动画中的临时 `offsetTop` 会先保持输入 form 贴底偏移，再通过短延迟复核切到稳定位置。移动端根文档禁止页面级滚动，键盘对齐期会复位运行页祖先容器和正文滚动容器的自动 `scrollTop` 改写。输入框实际聚焦后，只要可视高度开始收缩并超过同步阈值，就追加对应键盘底部偏移；浏览器工具栏伸缩或键盘收起后不保留额外底部留白。

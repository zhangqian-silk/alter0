...
- `Chat / Agent Runtime` 首页 Composer 收敛为单一紧凑输入面：桌面与移动端都压缩外层留白、输入高度和发送按钮体量，发送按钮直接复用 Terminal 的紧凑 icon submit 皮肤；PC 端上传、发送、状态、详情和短标识控件都采用低圆角矩形，输入区、计数 meta、发送按钮、会话卡片和 `Details` 面板沿同一套浅色 terminal-runtime 皮肤出图，不再混用默认 terminal footer slab 与旧式轻表单观感；空态工作区不允许保留可拖拽滚动，把头部和输入区顶离可视区。

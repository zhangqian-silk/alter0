...
- 所有 Agent 会额外自动维护当前 Session 的私有画像文件 `.alter0/agents/<agent_id>/sessions/<session_id>.md`，并以只读 `Agent Session Profile` 注入执行链路；该文件用于沉淀当前 Agent 在该 Session 内的稳定工作上下文。`coding` 场景会自动写入源仓库路径、独立 repo worktree 路径、远端地址、当前分支、Session 工作区与预览地址等关键属性。

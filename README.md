...
- Agent 默认可使用 `list_dir`、`read`、`write`、`edit`、`bash`、`codex_exec` 等工具，并依据执行结果决定继续推进还是完成收口；支持 `delegate_agent` 时可向其他可委派 Agent 下发子任务并回收结果；文件型 Skill 可进一步暴露 `read_skill`、`write_skill`，用于读取或维护可复用规则文件。
...
- Terminal 默认使用共享工作区 `.alter0/workspaces/terminal/shared`，不再为每个会话创建独立目录。
- Terminal 会持久化 Codex CLI 线程标识与会话状态；运行态退出后保留原会话历史，继续发送即可在同一会话内恢复。
- Terminal 工作区头部同时提供 `Close` 与 `Delete`；会话列表中的历史会话也支持直接删除：`Close` 仅退出当前运行态并保留会话历史与线程标识，`Delete` 会移除会话记录与持久化状态文件，但不会清空共享工作区。
...
- Chat 移动端输入区默认隐藏装饰性附注与字数计数，底部只保留输入框，以及同排的发送按钮与可展开会话设置入口；会话设置里的模型、工具与技能文案统一使用当前语言口径，避免中英混排，减少软键盘弹起后输入区占屏。
- Terminal 移动端采用紧凑 `Header + 独立滚动消息区 + 底部输入条` 结构；顶部通用栏保留 `Menu`，并在同一行右侧提供 `Sessions / New`，工作区头部收敛为会话标题、状态点与紧凑操作工具栏；发送区使用自适应输入框与紧凑发送按钮，运行态退出或中断提示以内嵌状态条贴合输入区上缘展示；消息与 `Process` 头部保持自然文档流滚动，不再启用吸顶导航；阅读定位由右侧圆形四键组承担，支持回到顶部、上一条、下一条与回到底部，并统一为浅色低对比滚动条与阅读主题。
...
11. `travel` 额外内置文件型 `travel-page` Skill，默认落在 `.alter0/skills/travel-page.md`，作为城市页生成规则与 HTML 页面呈现约定的可复用规则簿；`travel-master` 默认携带 `read_skill`、`write_skill`，会在用户提出稳定可复用偏好时更新该 Skill，而不会把一次性行程约束写入规则簿。
...
默认运行策略保持 `danger-full-access`，当前默认执行目录策略统一为“仓库根目录 + Terminal 共享工作区”：

1. `Chat / Agent`
- 默认执行目录：仓库根目录
- 不再默认为每个会话创建独立工作区；若历史版本或显式配置仍产生 `.alter0/workspaces/sessions/<session_id>`，`DELETE /api/sessions/{session_id}` 仍会同步清理

2. `Async Task`
- 默认执行目录：仓库根目录
- 不再默认为每个任务创建独立工作区；若历史版本遗留 `.alter0/workspaces/sessions/<session_id>/tasks/<task_id>`，会在删除所属会话时一并清理

3. `Terminal`
- 终端共享工作区：`.alter0/workspaces/terminal/shared`
...
3. 原生工具的相对路径默认按仓库根目录解析；默认模式下 `base=workspace` 与仓库根目录一致，仅在显式启用 session workspace 模式时才会落到 `.alter0/workspaces/sessions/...`。
...
4. 当前内置原生工具为 `list_dir`、`read`、`write`、`edit`、`bash`；Agent 额外支持 `codex_exec`，系统会自动补充收口工具 `complete`。支持委派的 Agent 还可启用 `delegate_agent`；带文件型 Skill 的 Agent 还可启用 `read_skill`、`write_skill`，按 Skill ID 直接维护对应规则文件。

...
- `Chat / Terminal` 在页面从后台回到前台、浏览器重新把当前页激活为可见工作页、bfcache 恢复、网络恢复在线或后端服务重启后，共享 Conversation runtime page-activation 刷新链路：当前 route 先刷新 owner 会话列表并按 update cursor 读取增量；若缓存不完整或收到 `resync_required`，再补拉对应 owner 的当前活动会话详情和进行中会话详情，避免后台期间或服务重启后的最新输出、标题或状态停留在旧视图。

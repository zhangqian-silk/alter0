...
- Terminal 工作区头部仅保留 `Details` 等阅读辅助工具；会话删除统一从会话列表触发，`Delete` 会移除会话记录、持久化状态文件与该会话对应的独立工作区。删除成功后，当前会话列表面板保持现状不自动收起，便于连续清理多条会话；前端同时会在后续列表轮询和 page-activation 补拉中继续屏蔽该会话，避免服务端短暂返回旧列表时已删除项又回弹到侧栏。

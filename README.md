...
- `Chat / Agent Runtime` 在消息发送与当前轮流式补丁期间，必须同步刷新浏览器中的活动会话快照，而不是只等异步 effect 或页面空闲时再写入；这样即使用户在刚发出消息后立刻刷新页面，本地恢复也仍能先看到已发送的 user message 与当前 assistant 占位，再等待服务端详情补齐最终结果。

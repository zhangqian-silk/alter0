...
当前桌面基线对齐主仓库的 legacy shell：左侧主导航采用全高固定栏，品牌区仅保留 `Alter0` 服务名与折叠入口；中栏承载 `New Chat` 与会话历史；右侧主工作区保留旧版 header、欢迎区/消息区、路由页 `route-head` 与独立 Composer。React 不再把三栏壳体渲染成独立胶囊面板，也不再额外挂载 workspace hero、route hero 或 prompt deck 容器；Chat 欢迎区直接输出 `Alter0 workspace` tag、面向任务入口的默认标题与说明、target 列表与 prompt pills，页面类路由直接使用 `route-head` 标题区。桌面端聊天阅读区与底部输入区继续统一限制在 `960px` 最大宽度内并居中，但外层三栏保持主仓库式全宽工作台；壳层视觉收敛为更克制的工作台基线，默认使用中性冷灰表面、低对比边框、有限强调色与更少的玻璃感/渐变装饰，`Control / Sessions / Tasks / Memory / Terminal / Codex Accounts` 等 React 托管 route 页统一复用近白表面、浅灰表头/说明层和浅蓝选中态，不再各自维持独立的高装饰卡片语言；头部 `Menu / Sessions / New` 入口在桌面宽度默认隐藏，仅在主视口宽度 `1100px` 及以下时切换为贴边抽屉入口，`760px` 及以下再进一步压缩抽屉宽度与按钮尺寸，保证真手机宽度下的可触达性。
...
控制类与资产类页面默认采用更高信息密度的管理视图：`Profiles` 使用短字段并排的紧凑表单栅格与显式启用开关，`Tasks` 使用左侧任务列表 + 右侧运行详情的主从布局，`Memory` 的任务历史使用表格 + 详情侧栏，`Environments` 使用运行态工具栏 + 模块卡片栅格展示配置项，并在同页提供敏感值显隐、保存、重载、重启与审计视图，`Codex Accounts` 使用运行时概览条 + 托管账号卡片列表 + 导入/登录操作侧栏展示当前 auth 路径、托管数量、套餐、额度状态与登录会话；`Agent` 与 `Products` 的列表卡片、管理表单、工作区详情和对话区也统一使用近白表面、浅灰说明层与浅蓝选中态。大屏保留“列表 + 侧栏”结构，中屏切换为全宽账号区 + 双侧栏，小屏回落为单列卡片，确保额度信息与切换按钮始终可见。
...
- `Chat / Agent` 头部的 `Menu / Sessions / New` 入口只在窄屏壳层中承担收敛后的主操作：桌面三栏时隐藏，`1100px` 及以下时作为主导航与会话历史抽屉入口显示，`760px` 及以下再切到更紧凑的按钮和抽屉宽度；`info-mode` 页面默认隐藏 `Sessions` 按钮，Terminal 页面保留会话入口。窄屏主导航抽屉内的菜单区保持独立纵向滚动，小高度视口下所有导航分组、底部设置项与语言切换入口都必须可触达。

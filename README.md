...
当前桌面基线是浅蓝轻科幻风格的三栏 cockpit：左侧主导航承载品牌与模块组，中栏承载当前路由上下文与会话历史，右侧主工作区承载欢迎区/消息区、页面 hero 与固定底部输入面板；整体采用浅蓝渐变背景、低对比玻璃感面板、冷色高亮与更明确的未来感品牌区。为避免重构后信息重复，当前壳层收敛为单层信息架构：会话栏不再重复输出整块路由概览，Chat 主区不再额外叠加 workspace hero，页面类路由只保留一层主标题说明。移动端切换为主导航抽屉 + 会话抽屉的单列工作台，继续复用同一 React 壳层和 bridge 契约。
...
- `Chat / Agent` 的前端壳层采用统一 cockpit 信息架构：工作区 hero 用于承接当前会话定位，欢迎区内提供 target picker 与 prompt deck，输入区独立固定在底部，欢迎区内容超出高度时只在内容区内部滚动，不与输入面板叠层。
...
- Chat 桌面宽屏会按可用主工作区宽度自适应放宽消息列与底部输入区，不再在大分辨率下长期锁定为窄列；同时保留阅读上限，避免正文无限拉长。
...
对应 Nginx 配置与运行权限方案见：`docs/deployment/nginx.md`。若需要会话级前端预览，请把 `alter0.cn` 与 `*.alter0.cn` 一并反向代理到同一共享运行时，再用 `scripts/deploy_session_preview.sh <session_id> [repo_path]` 注册当前会话构建。

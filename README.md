...
- Web 登录后，`Chat` 的已发送会话历史落到服务端 Session history，并在同一 Web 登录态下跨设备共享。未发送文本草稿、附件草稿、GitHub 仓库选择草稿与当前页局部选择继续按浏览器隔离；切换 Chat / Settings、切换会话或点击 New 时不再弹出丢弃草稿确认，原会话草稿按 route 与 session 继续缓存，返回后恢复。
...
- `Chat` 新会话标题以底层 ChatRuntime session store 为唯一事实源；前端创建真实会话时不把首条 prompt 作为 create title，也不在本地乐观消息阶段改写标题。底层会话先使用统一占位标题 `New`；当 Codex/Claude 等外部 CLI Runtime 暴露自身 thread title 时，ChatRuntime 直接采用该 title 并通过会话更新同步，后续 runtime 多次改名也继续覆盖。未收到外部 thread title 时，服务端保留更具体输入自动升级标题的兜底策略；已由外部 runtime 接管的标题不会被后续 prompt 自动标题抢回。`/chat?session_id=<chat_session_id>` 入口用于恢复对应会话，`chat_session_id` 使用 `c_` 加 16 位小写字母数字的短 canonical id。
- `Chat` 使用 runtime workspace：会话列表由左侧主导航直接展示，置顶会话单独进入 `Pinned / 置顶` 分组并位于 `Today / 今天` 上方；距离 7 天不活跃清理阈值还剩不超过 2 天的未置顶会话进入 `Expiring Soon / 即将清理` 提醒分组，其余会话按内容更新时间分组，缺少更新时间时才回退到创建时间；主工作区固定为主消息工作区、底部 Composer 与固定 workspace header；真实会话条目右侧固定提供三点更多按钮，展开后提供置顶、详情与删除操作，删除需二次确认。空白 `New` 草稿会话尚未进入 Session history 时只作为输入入口，不显示三点菜单，也不支持置顶、详情或删除；同一路由内只允许存在一个空白虚拟会话，重复点击 `New` 会回到现有空白草稿，首次发送后才升级为真实会话。header 统一只保留当前会话标题、状态按钮与 `Details` 入口，草稿会话的 `Details` 禁用。消息、过程步骤与最终输出都在同一轻量 IM 式消息流中推进。
- `Chat` Composer 的 GitHub 图标是独立于附件回形针的轻量仓库选择入口：发送前搜索并勾选仓库只生成本地可移除 chip，不会创建会话或拉取代码；首次消息发送时才把稳定仓库引用与用户语义一起提交。服务端复用当前个人 `gh` 登录解析仓库并 clone 到会话工作区 `repo/`，一个会话只绑定一个仓库；已绑定 chip 不可替换，准备失败可原地重试。该绑定不会自动 pull、commit 或 push，也不会改写 Git author。
...
- 运行页的可见交互按后端业务状态驱动：提交期间只禁用当前 Composer；收到后端接受结果后，当前活动会话、移动标题和左侧会话列表才按返回状态显示处理中；同一 owner 下其他本机 `local_running / recovering` 会话也通过同一 owner 增量轮询更新，不为每个会话单独建立实时连接。跨设备或历史会话的服务端 `busy / running` 摘要不触发后台自动轮询，页面激活补偿或显式手动刷新会恢复最新服务端事实。transport failure 只在 Composer 工具栏显示短暂弱提示并静默回读当前详情；结构化 HTTP 后端错误可在同一位置展示服务端消息。两者都不改变会话状态、消息或缓存；只有服务端详情或事件明确返回失败、中断、删除或不存在时，才收口为对应业务状态。
...
- 左侧主导航内的 Session 列表保持工作台式紧凑结构：置顶会话单独维护在 `Pinned / 置顶` 分组并固定在 `Today / 今天` 上方；距离 7 天不活跃清理阈值还剩不超过 2 天的未置顶会话进入 `Expiring Soon / 即将清理` 分组，其余会话按内容更新时间分组，并与主导航 `menu` 复用同一套分组外壳、hover、激活态语言和桌面会话列宽；条目采用独立卡片，主体只保留标题且在可用宽度内单行截断，长标题只能在条目内部省略，新增会话插入或列表刚好跨过滚动阈值时也不改变 `Sessions / New` 区块的宽度或纵向位置，尾侧保留单个三点更多按钮，展开后承载置顶、详情与删除操作，不再展示摘要、Skill 标签、完整会话 id、状态灯或额外 footer 区块。
...
- 执行过程通过 runtime session `turns[].runtime_trace_events` 收口；Chat 前端直接消费轻量 `RuntimeTraceEvent`，并按 `important_text / reasoning / plan / tools / commands / system` 披露类型渲染。
...
- 浏览器本地缓存里的历史消息若残留旧版 `streaming`、`Thinking...` 或 `Load failed` 客户端占位，页面恢复时以当前会话详情与 owner 增量覆盖；客户端占位本身不得作为失败事实继续写回会话。
...
- 运行时执行过程会在运行时产出结构化 `RuntimeTraceEvent`，并通过 Chat session `turns[].runtime_trace_events`、Task 结果与历史恢复一并返回；轻量摘要不携带 `blocks`，完整详情只在 event detail 接口返回。前端按可控事件类型渲染可折叠 `Process` 区块，不再维护 Chat 两套过程数据结构。
- Chat 回复中的 `Process` 与最终正文在收口后继续同时保留；刷新页面或从服务端会话历史恢复时，结构化步骤不会因最终正文已落库而丢失。
- `Chat` 在浏览器刷新前会把当前 route 的活动会话写入独立 `sessionStorage` key，并把完整已加载消息或 turns 写入当前 route 的 24 小时 `localStorage` 缓存；同时额外写入不含消息正文的会话信息缓存，避免完整消息缓存超限时会话列表也丢失。两类缓存恢复时只接受 canonical session id 并按 session 合并：完整缓存保住消息、过程事件与分页状态，轻量信息缓存补齐缺失的会话摘要。旧 `active snapshot / recent snapshot` sessionStorage key 会在启动时清理，不恢复为当前会话。刷新后前端先用本地快照保住当前会话与消息时间线，再按 `session_id` 补拉当前 owner 的单会话详情校准最新正文、状态和过程事件；即使服务端会话列表暂时还没返回该会话，也不得让活跃会话短暂消失或被新的空白 `New` 会话顶替。
...
- Web `Chat` 独立入口与独立消息接口已移除；当前 Chat 会话只恢复后端短 canonical id 对应的 runtime session，不再迁移旧本地会话快照或旧 id 状态。
...
- Chat 的消息流统一采用克制的冷灰工作台阅读主题：用户消息保留右对齐并使用浅灰低对比紧凑气泡，助手消息左对齐并弱化为无边框阅读块；Chat 正文工作区白底无框，靠阅读宽度、留白和角色对齐建立层级，不在消息区叠加明显边框、背景分界或卡片容器。同一条 assistant 消息同时存在过程与最终回复时，`Thinking / 已思考` 披露入口位于最终 Markdown 上方，思考过程只保留一行轻量入口并在展开后承载步骤内容；最终正文和复制动作位于其下方。正文排版优先于装饰层级，用户消息与其后续回复继续保持更紧凑的同轮分组间距，长历史按最新优先和顶部渐进加载处理。
- Chat 已加载的完整历史、过程详情和分页耗尽状态会随本地运行态缓存一起保留；最新 bounded detail 或增量更新只能补丁新 turn、最终正文和过程步骤，不会把已展示全量历史刷新成短页，也不会在没有更早历史时重新显示 `Load earlier messages / 加载更早消息`。
...
- Chat 的左侧会话列表与 Chat 共享同一套列表视觉语义：条目主体只展示标题，详情与删除入口收在列表项尾侧，处理中条目在标题旁显示 loading；会话运行状态保留在 workspace header 与 `Details` 摘要中，不在列表项内额外渲染状态灯、元信息或会话标识。
...
- `POST /api/chat/sessions/updates`：当前 owner 的增量轮询通道。客户端携带 `after_update_id`、`limit`、`byte_limit` 与 `visible_event_kinds`；`byte_limit` 默认/最大为 `1MiB`。服务端返回 `latest_update_id`、`resync_required`、`has_more` 与 `updates[]`。运行过程由应用层发布 `turn.started / turn.event.appended / turn.event.updated / turn.completed / turn.failed / turn.interrupted` 语义事件，单条 update 使用 `update_id / type / session_id / turn_id / created_at / runtime_event` 顶层结构；turn/event id 统一使用 `turn-* / event-*` 字符串。`runtime_event` 是轻量过程事件，只保留 `id / kind / status / text / detail_available / created_at / completed_at / duration_ms`，完整 `blocks` 仅通过 event detail 接口读取。`turn.completed` 只携带当前 turn 的最终状态与 `final_output`。`session.created / session.updated / session.deleted` 只承载会话摘要和用户可见状态，不再作为常规 runtime step 增长的全量载体。不可见 command/tool/system 等过程事件可被跳过，`latest_update_id` 仍前进，可见 `update_id` 不保证连续；前端只合并缺失 step，不用增量数组替换同 turn 已有过程。
- Chat 会话接口统一返回 `Cache-Control: no-store`，浏览器会话数据只由应用缓存管理。列表接口固定返回 `id / title / status / created_at / updated_at / pinned`；详情的 summary、最新 turns 页和分页边界来自同一个原子快照。输入携带 `client_request_id` 供服务端关联 turn 与后续快照对账，不用于在浏览器提前创建会话消息或状态。
- 移动端页面隐藏时停止轮询；回前台不足 5 分钟时稳定会话不发列表或详情请求，本机未完成会话只恢复 updates；达到 5 分钟、首次进入或手动刷新时拉取轻量全量列表和当前会话最新详情。切换会话不拉列表，仅在内容未加载、缓存损坏、列表 `updated_at` 更新或未完成会话没有活跃同步时拉详情。

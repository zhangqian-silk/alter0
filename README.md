...
- `Chat / Terminal` 移动端输入区由 `VisualViewport` 同步键盘偏移；输入框聚焦后可视高度开始收缩即写入键盘占位，避免 App Shell 在键盘开场阶段先缩短再回弹。软键盘弹出期间若浏览器短暂回报完整视口高度，前端会保留上一帧已确认的键盘占位，避免 Composer 闪回底边；若浏览器在抬起动画中临时回报正 `offsetTop`，Composer 会先保持上一帧贴底偏移，并在短窗口结束后自动复核到稳定偏移，避免键盘上沿出现突变。键盘期间只有 Composer form 的 `bottom` 跟随可见底边，Composer shell、workspace header、正文滚动区、空态与配置浮层保持原位置和原高度；输入框真实聚焦期间不主动改写 `window.scrollY`、运行页祖先容器或正文滚动容器的 `scrollTop`，避免浏览器键盘动画与前端反向滚动补偿叠加成可见突变。真手机宽度下共享 runtime Composer 通过底部偏移贴住可见底边，不使用 transform 合成层承载键盘位移；键盘路径上的 runtime composer shell/form、workspace、mobile header、阅读定位条和状态信号统一静止化，不保留装饰性 transition、animation、filter、backdrop-filter、阴影或脉冲，避免 iOS Safari 在键盘动画与输入框 surface 回收时出现位移动画或灰色残影。
...
- 移动端 `Chat` 输入区在软键盘弹起、收起与可视视口高度变化期间，会基于 `VisualViewport` 同步有效视口高度；App Shell 与 `workbench-main` 在键盘弹起时保持稳定基线高度，底部输入 form 只消费 `--keyboard-composer-offset` 贴住可见底部，Composer 外壳、正文滚动区和背景层不再按键盘高度动态位移或收缩。浏览器工具栏状态切换、输入聚焦或正 `offsetTop` 不会把整个工作区重复顶起；键盘抬起动画中的临时 `offsetTop` 会先保持输入 form 贴底偏移，再通过短延迟复核切到稳定位置。移动端根文档禁止页面级滚动，但键盘对齐期不主动复位运行页祖先容器和正文滚动容器的 `scrollTop`。输入框实际聚焦后，只要可视高度开始收缩并超过同步阈值，就追加对应键盘底部偏移；浏览器工具栏伸缩或键盘收起后不保留额外底部留白。
...
- 移动端 `Chat / Terminal` 的主输入框首次触摸保留浏览器原生聚焦与软键盘手势，不在 `pointerdown / touchstart` 捕获阶段取消默认行为或抢先调用 `focus()`；程序化回焦仅用于 slash command、创建新会话后回到 Composer 等非直接输入框触摸场景，并使用 `preventScroll` 避免页面跳动。输入框保持真实焦点期间，前端只同步 composer 与遮挡高度，不再额外锚定页面级滚动或运行页内部滚动容器，避免滚动补偿与软键盘动画叠加。

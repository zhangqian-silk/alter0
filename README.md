...
- Chat 的整体视觉语言与 Chat 收敛：左侧会话列表、工作区容器、输入区与输出块统一使用扁平白底、必要分割线、有限强调色和 Composer 胶囊；用户输入使用共享右侧紧凑气泡，不再额外展示命令前缀符号或强调色，最终回复直接按共享助手消息阅读流渲染；`Process` 保留低对比提示区和左侧纵向时间线，内部步骤压缩为单行摘要，展开后只展示详细内容，不再重复状态标签。

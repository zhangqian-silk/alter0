...
- Terminal 移动端采用紧凑 `Header + 独立滚动消息区 + 底部输入条` 结构；顶部通用栏保留 `Menu`，并在同一行右侧提供 `Sessions / New`，工作区头部收敛为单行会话摘要操作栏；发送区使用自适应输入框与紧凑发送按钮，长输出支持展开/收起、用户问题吸顶、`Process / Final Output` 头部吸顶折叠与快速回到底部。

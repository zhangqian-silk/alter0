...
- `Chat / Agent Runtime` 统一使用 terminal-style workspace：左侧主导航之外，运行页内部固定为会话列、主时间线工作区、底部 Composer 与固定 workspace header；header 统一只保留当前会话标题、状态按钮和 `Details` 入口，`Details` 面板顶部先用紧凑摘要栅格承载会话元数据，再由各页面继续展开专属配置内容。消息、过程步骤与最终输出都落在同一时间线中推进，不再保留桥接期的欢迎区 / runtime sheet 双轨壳层。会话列、workspace、chat screen 与 composer 直接复用 `terminal-*` 语义 class，保证与 `Terminal` 新版 UI 保持同一表面、标题区和输入区节奏。
...
- `Codex Accounts` 控制面位于 `Settings`，支持在同页查看运行时摘要、当前 Codex 运行时状态、当前活动 profile、活动 model、思考深度、配置来源、导入已有 `auth.json`、启动独立 `codex login` 会话、查看账号套餐/额度状态，并切换当前运行时生效账号；model 与思考深度的可选项来自 Codex app-server 的 `model/list`，当前生效值与来源来自 `config/read`，保存时通过 `config/batchWrite` 写回当前用户配置。当前运行中的 `auth.json` 若尚未纳入托管，会在概览与空态中明确标记为“已生效但未托管”，加载阶段则保留完整面板骨架而不是退化成单行提示。

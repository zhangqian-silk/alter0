...
- `Agent Runtime / Terminal` 会把当前活动会话同步写入 URL query，用于刷新后的精确恢复：`agent-runtime` 使用 `agent_session_id`，`terminal` 使用 `terminal_session_id`。`Chat` 固定回到 `alter0-chat`，不会再把 `chat_session_id` 作为多会话切换入口写回 URL。

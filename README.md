...
该检查默认对比 `origin/master...HEAD`，若只改了 `docs/features.md` 会直接失败，避免能力矩阵与总览文档漂移。`make integration-matrix` 用于执行网关级集成矩阵（多通道、多 Agent、子代理、定时任务、工具与故障注入）；`make release-gate` 会串联配置边界门禁（`make check-config-boundary`）、测试稳定性门禁（含 `make test-stability` 与 Windows 编译回归）、集成矩阵、回滚演练、部署资产检查与文档门禁。

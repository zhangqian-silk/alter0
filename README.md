...
9. `travel` Product 在 Workspace 中提供“主 Agent 对话 -> 城市页同步”链路：`POST /api/products/travel/workspace/chat` 会由 `travel-master` 解析用户意图，并将结果落到具体城市页空间；适合直接创建或修改如武汉、成都、北京等页面。

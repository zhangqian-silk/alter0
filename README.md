...
移动端 `Chat / Terminal` 输入区通过共享 `VisualViewport` 状态机维护 `--mobile-viewport-height`、`--keyboard-offset` 与 `--keyboard-composer-offset`。软键盘弹起、失焦回弹和浏览器工具栏变化期间，`--keyboard-offset` 只作为键盘占位事实，`--keyboard-composer-offset` 只负责底部 Composer 贴住可见底边；移动端 document/root 禁止页面级滚动，App Shell 保持可视高度加键盘占位形成的稳定基线，workspace header、正文滚动区、空态、阅读定位条、命令候选与配置面板不消费键盘偏移，也不随键盘过程改变高度或位置。当浏览器在键盘动画中同时上报收缩后的 `height` 与正 `offsetTop` 时，仍以收缩高度保持键盘占位，并在短过渡窗口内保持 Composer 贴边偏移，待视口报告稳定后再扣除 `offsetTop`，避免输入区在键盘抬起过程中突跳或被重复上移。
...
- `Chat / Terminal` 移动端输入区由 `VisualViewport` 同步键盘偏移；软键盘弹出期间若浏览器短暂回报完整视口高度，前端会保留上一帧已确认的键盘占位，避免 Composer 闪回底边；若浏览器在抬起动画中临时回报正 `offsetTop`，Composer 会先保持上一帧贴底偏移，并在短窗口结束后自动复核到稳定偏移，避免键盘上沿出现突变。键盘期间只有 Composer 的 `bottom` 跟随可见底边，workspace header、正文滚动区、空态与配置浮层保持原位置和原高度；主输入框首触时会先捕获页面级与运行页滚动锚点，再保留浏览器原生聚焦和软键盘手势；输入框保持真实焦点时，前端会在 `window.scroll`、`VisualViewport resize/scroll` 与短延迟复核中清理页面级 `window.scrollY`，并复位 App Shell、Chat pane、workspace body 与正文滚动区在键盘动画中被浏览器自动改写的 `scrollTop`，避免移动端浏览器把整个工作台顶起。真手机宽度下共享 runtime Composer 通过底部偏移贴住可见底边，不使用 transform 合成层承载键盘位移，避免 iOS Safari 在输入框阴影层回收时留下灰色残影。
...
- 移动端 `Chat` 输入区在软键盘弹起、收起与可视视口高度变化期间，会基于 `VisualViewport` 同步有效视口高度；App Shell 在键盘弹起时保持稳定基线高度，底部输入区只消费 `--keyboard-composer-offset` 贴住可见底部，正文滚动区不再按键盘高度动态收缩。浏览器工具栏状态切换、输入聚焦或正 `offsetTop` 不会把整个工作区重复顶起；键盘抬起动画中的临时 `offsetTop` 会先保持 Composer 贴底偏移，再通过短延迟复核切到稳定位置。移动端根文档禁止页面级滚动，键盘对齐期会复位运行页祖先容器和正文滚动容器的自动 `scrollTop` 改写。仅在输入框实际聚焦且软键盘占位达到阈值时才追加键盘底部偏移，浏览器工具栏伸缩或键盘收起后不保留额外底部留白。
...
- 移动端 `Chat / Terminal` 的主输入框首次触摸保留浏览器原生聚焦与软键盘手势，不在 `pointerdown / touchstart` 捕获阶段取消默认行为或抢先调用 `focus()`；触摸捕获阶段只记录页面级滚动和运行页内部滚动锚点，待真实焦点建立后立即复位浏览器自动改写的滚动位置；程序化回焦仅用于 slash command、创建新会话后回到 Composer 等非直接输入框触摸场景，并使用 `preventScroll` 避免页面跳动。输入框保持真实焦点期间，前端监听 `window.scroll + VisualViewport resize/scroll` 同步 composer 与遮挡高度，并锚定页面级滚动和运行页内部滚动容器，避免工作台被整页顶起。
...
- 移动端 `Terminal` 在输入框聚焦且软键盘抬起后，底部 Composer 会按 `VisualViewport` 推导出的 `--keyboard-composer-offset` 直接贴住可见底边；Terminal 工作区主体、workspace header、输出区高度和配置浮层保持原位，键盘弹起不会把页面整体向上推出，也不会压缩长历史输出区；长历史输出继续留在 `terminal-chat-screen` 内独立滚动，不允许通过增大 footer padding 把输入区整体挤出屏幕。
...
- Chat 与 Terminal 在移动端键盘弹起和收回期间，只有 composer 自身允许跟随 `VisualViewport` 派生的 `--keyboard-composer-offset` 做贴底位移；顶部操作行与紧凑 workspace header 保持原位不跟着键盘位移一起跳动；阅读定位条在输入框聚焦期间隐藏，失焦后再回到原有停靠位。

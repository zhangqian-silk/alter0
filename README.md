...
- 移动端运行页的 `Menu / Sessions` 抽屉在真机上优先保证稳定性：遮罩保留淡入淡出，抽屉本体仅保留一层轻量侧滑，不再叠加容易闪烁的多层位移、淡出或条目级顺序动画；抽屉内会话项统一展示状态文本、标题、摘要与短 hash，删除动作固定在条目尾侧。
...
- Terminal 工作区头部保留 `Close` 与 `Details` 工具；会话删除统一从会话列表触发：`Close` 仅退出当前运行态并保留会话历史与线程标识，`Delete` 会移除会话记录、持久化状态文件与该会话对应的独立工作区。

...
- 四键阅读定位条仅保留在 `Agent` 运行态消息区与 `Terminal` 对话区，采用与主仓库 Terminal 一致的右侧箭头四键：`回到顶部 / 上一条 / 下一条 / 回到底部`。普通 `Chat` 消息区与其他 `page-mode` 路由页不再展示这组按钮；滚动超出阈值后显示顶部与底部入口，并按当前视口中的可见消息块或 Terminal turn 动态计算上下目标。Terminal 移动端的四键定位条固定停靠在工作区右侧、输入区上沿之上，四个按钮统一为独立圆形，避免落回正文流内或压住输入区。

...
- `Chat / Terminal` 移动端输入区由 `VisualViewport` 同步键盘偏移；软键盘弹出期间若浏览器短暂回报完整视口高度，前端会保留上一帧已确认的键盘占位，避免 Composer 闪回底边；若浏览器在抬起动画中临时回报正 `offsetTop`，Composer 会先保持上一帧贴底偏移，并在短窗口结束后自动复核到稳定偏移，避免键盘上沿出现突变。键盘期间只有 Composer 的 `bottom` 跟随可见底边，workspace header、正文滚动区、空态与配置浮层保持原位置和原高度；输入框保持真实焦点时，前端会在 `window.scroll`、`VisualViewport resize/scroll` 与短延迟复核中清理页面级 `window.scrollY`，并复位 App Shell、Chat pane、workspace body 与正文滚动区在键盘动画中被浏览器自动改写的 `scrollTop`，避免移动端浏览器把整个工作台顶起。真手机宽度下共享 runtime Composer 通过底部偏移贴住可见底边，不使用 transform 合成层承载键盘位移，避免 iOS Safari 在输入框阴影层回收时留下灰色残影。
